...
function HeroButtons({ isBright, copy, className = "" }) {
  // Keep current CTA size; allow wrap within the content column only (never under the map).
...
/**
 * Marketing hero — text + CTAs share a wider column; compact coverage card on the side.
...
export default function HomeHero({ isBright }) {
  ...
  const mapWrap = isBright
    ? "w-full min-w-0 max-w-full overflow-hidden rounded-2xl border border-sky-100 bg-white/90 p-2.5 shadow-xl shadow-sky-300/35 ring-1 ring-white/50 sm:p-3 lg:rounded-3xl lg:shadow-2xl"
    : "w-full min-w-0 max-w-full overflow-hidden rounded-2xl border border-white/10 bg-black/35 p-2.5 shadow-2xl shadow-black/45 ring-1 ring-white/10 sm:p-3 lg:rounded-3xl";

  return (
    <section data-testid="home-hero" className="w-full" data-home-hero-layout="content-priority">
      <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        {/* Wider content/CTA column; constrained map column so long localized CTAs never sit under the map. */}
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(0,0.72fr)] lg:gap-8 xl:grid-cols-[minmax(0,1.65fr)_minmax(0,0.7fr)] xl:gap-10">
          ...
          <div
            className={`${mapWrap} lg:max-w-[22rem] xl:max-w-[24rem] lg:justify-self-end w-full`}
