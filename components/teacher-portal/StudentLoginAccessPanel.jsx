...
export default function StudentLoginAccessPanel({ accessToken, studentId }) {
  ...
  const onRotatePin = async (accessId) => {
    if (!window.confirm(globalBurnDownCopy("components__teacher-portal__StudentLoginAccessPanel", "the_current_code_will_be_revoked_and_a_new_one_generated_save_the_new_co"))) {
      return;
    }
    setBusy(true);
    const res = await teacherAuthFetch(
  ...
  const onRotateUsername = async (accessId) => {
    if (!window.confirm(globalBurnDownCopy("components__teacher-portal__StudentLoginAccessPanel", "the_current_username_will_be_revoked_the_new_username_will_be_shown_only"))) {
      return;
    }
    setBusy(true);
    const res = await teacherAuthFetch(
