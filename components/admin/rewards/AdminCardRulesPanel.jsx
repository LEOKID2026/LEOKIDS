...
import { apiErrorMessageHe } from "../../../lib/admin-portal/admin-ui.js";
import { formatRuleTypeHe } from "../../../lib/admin-portal/admin-rewards-ui.js";
...
export default function AdminCardRulesPanel({ accessToken, cardId, cardName, embedded = false }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!rulesRes.ok) {
      setMessage(apiErrorMessageHe(rulesBody?.error, "  "));
      setPhase("error");
      return;
    }
  ...
  if (phase === "loading") {
    return <p className="text-white/50 text-xs text-right">  ...</p>;
  }
  ...
  return (
    <div className={shellClass}>
      {!embedded ? <h4 className="font-bold text-sm">  - {cardName || ""}</h4> : null}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
      <label>
        ...
        >
          {(ruleTypes.length ? ruleTypes : [{ value: "total_questions", label: " " }]).map((t) => (
      ...
      {(rt === "grade_band_only" || rt === "event_window") && (
        <label>
          ...
          >
            <option value=""> </option>
            <option value="g12">–</option>
            <option value="g34">–</option>
            <option value="g56">–</option>
          </select>
        </label>
      )}
      <label className="sm:col-span-2">
        ...
        <input
          ...
          onChange={(e) => setDraft((d) => ({ ...d, requirementText: e.target.value }))}
        />
      </label>
