...
const EMPTY_SERIES = { name: "", slug: "", display_order: 0, is_active: true };
...
export default function AdminSeriesTab({ accessToken }) {
  ...
  const editTitle = editingRow?.name ? ` : ${editingRow.name}` : " ";
          ...
          <tbody>
            {series.map((row) => (
              <tr key={row.id} className="border-b border-white/5">
                ...
                <td className="py-2 px-2">{row.is_active !== false ? "" : ""}</td>
