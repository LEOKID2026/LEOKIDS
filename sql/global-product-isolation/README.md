...
2. **Authenticated client** — treated as **IL legacy path**. RESTRICTIVE policies hide Global rows (`product_id = 'leokids_il'` only; **NULL is not IL**).
3. **Arcade / Tier D** — no product isolation.
4. **Shared catalogs** — `coin_reward_rules`, `coin_spend_rules`, `shop_items`, `teacher_plans` stay shared (no product restrict).
5. **Legacy tables** — `parent_account_settings`, `guest_mode_settings` keep single-row PK for IL.
6. **Product-scoped settings** — `product_parent_account_settings`, `product_guest_mode_settings` (Global app only).
...
| 1 | `A_students_product_id.sql` | `students.product_id` NOT NULL after IL backfill; access_code denorm |
...
| 7 | `F_rls_restrictive_il_only.sql` | RESTRICTIVE IL-only for authenticated Tier A/B/C (+ preflight gaps) |
...
Rollback: `rollback/R_*.sql` per stage with confirm GUC (`v3.rollback_confirm`).
