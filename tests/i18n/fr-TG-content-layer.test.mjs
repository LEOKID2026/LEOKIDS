/**
 * fr-TG (Togo — French schooling layer) sparse content-layer checks.
...
/** France CP…6e, Senegal CI, Cameroon SIL, DR Congo EBEP must not leak. */
const FOREIGN_GRADE_RE = /\b6e\b|\bSIL\b|(?<![A-Za-z])CI(?![A-Za-z0-9])|\bEBEP\b|\b7e\b/;
const TG_GRADES = ["CP1", "CP2", "CE1", "CE2", "CM1", "CM2"];
/** Other-country leakage (not Togo). */
const SCOPE_LEAK_RE =
  /\b(Sénégal|Senegal|Côte d’Ivoire|Cote d'Ivoire|Cameroun|Cameroon|RDC|Congo|Bénin|Benin|Guinée|Guinea|France)\b/i;
/** Claims that French is the only language spoken in Togo. */
...
function assertNoScopeLeak(blob, label) {
  ...
  assert.equal(ALL_FRENCH_CLAIM_RE.test(blob), false, `all-French/Togo claim in ${label}`);
}

test("fr-TG locale namespaces parse and stay sparse vs fr-FR", () => {
...
test("fr-TG grade terminology CP1–CM2 (authority-backed; not France/SN/CM)", () => {
  ...
  assert.match(seo.homeTitle, /Togo/);
  assert.match(seo.learningDescription, /CP1 à CM2/);
  assert.doesNotMatch(seo.homeTitle, ALL_FRENCH_CLAIM_RE);
  ...
  assert.doesNotMatch(allLocaleText, /(?<![A-Za-z])CI(?![A-Za-z0-9])/);
  assert.match(allLocaleText, /\bCP1\b/);
  assert.match(allLocaleText, /\bCP2\b/);
  assert.match(allLocaleText, /\bCM2\b/);
});

test("fr-TG content packs sparse contract vs fr-FR", () => {
  ...
  for (const rel of listJsonRel(countryRoot)) {
    ...
    if (isBurnDownIndexPath(rel)) {
      ...
      for (const [key, value] of indexAudit.countryLeaves) {
        ...
        if (SCOPE_LEAK_RE.test(value) || ALL_FRENCH_CLAIM_RE.test(value)) {
    ...
    for (const [key, value] of countryLeaves) {
      ...
      if (
        typeof value === "string" &&
        (SCOPE_LEAK_RE.test(value) || ALL_FRENCH_CLAIM_RE.test(value))
      ) {
        scopeHits.push(`${rel}:${key}`);
      }
  ...
  assert.deepEqual(scopeHits, [], "no other-country or all-French claims");
});

test("fr-TG pack grade labels use CP1–CM2 bands", () => {
  const booksUi = JSON.parse(
    fs.readFileSync(path.join(ROOT, "content-packs", LOCALE, "books/ui.json"), "utf8")
  );
  assert.deepEqual(Object.values(booksUi.grades), TG_GRADES);
...
test("fr-TG help overlays parse and remap France grade span", async () => {
  const help = await import(`../../data/help-center/fr-TG/index.js?t=${Date.now()}`);
  const parentsBase = await import("../../data/help-center/fr-FR/parents.js");
  assert.equal(
    help.ALL_ARTICLES_FR_TG.length,
  ...
  for (const a of help.BY_SECTION_FR_TG.parents) {
    assert.ok(parentSlugs.has(a.slug), a.slug);
  }
  const welcome = help.BY_SECTION_FR_TG.parents.find((a) => a.slug === "welcome-and-overview");
  assert.match(JSON.stringify(welcome), /CP1 au CM2/);
  assert.match(JSON.stringify(welcome), /Togo/);
  assert.doesNotMatch(JSON.stringify(welcome), /\b6e\b/);
  assert.doesNotMatch(JSON.stringify(welcome), ALL_FRENCH_CLAIM_RE);

  const addStudents = help.BY_SECTION_FR_TG.parents.find((a) => a.slug === "add-students");
  assert.match(JSON.stringify(addStudents), /CP1 à CM2/);
  assert.match(JSON.stringify(addStudents), /un niveau/);
...
test("fr-TG demo false friends: niveau / joueur / accueil", () => {
  ...
  assert.deepEqual(Object.values(demo.grades), TG_GRADES);
});

test("fr-TG UI back-home and primary terminology", () => {
  ...
  assert.match(ui.home?.subhead || "", /enseignement primaire au Togo/);
  assert.doesNotMatch(JSON.stringify(ui), /Retour à la maison/);
  assert.doesNotMatch(JSON.stringify(ui), ALL_FRENCH_CLAIM_RE);
});

test("fr-TG report detailed surface grade chrome = Niveau", () => {
...
test("fr-TG help students: tu conjugations and niveau wording", async () => {
  const help = await import(`../../data/help-center/fr-TG/index.js?t=${Date.now() + 1}`);
  const students = help.BY_SECTION_FR_TG.students;
...
test("fr-TG grade-aware report pack: math terms and CP1–CM2 ranges", () => {
  ...
  assert.doesNotMatch(blob, /(?<![A-Za-z])CI(?![A-Za-z0-9])–/);
...
test("fr-TG school portal: niveau scolaire vs groupe-classe", () => {
...
test("fr-TG student/teacher/parent terminology stays Togo-appropriate", () => {
  const blobs = [
  ...
  // MEPS usage: élève, enseignant / maître, parents, directeur d'école.
...
test("fr-TG does not ship word-meanings, science overlay, or mutate English targets", () => {
  ...
  const scienceOverlay = path.join(ROOT, "data/science-questions-fr-TG-overlay.js");
  assert.equal(fs.existsSync(scienceOverlay), false);
  const learningContent = path.join(ROOT, "utils/learning-content-fr-TG");
...
test("fr-TG child/adult register: no tu/vous mix introduced in overlays", () => {
  const files = [
    ...listJsonRel(path.join(ROOT, "locales", LOCALE)).map((rel) =>
...
test("fr-TG auth/validation: adresse e-mail and téléphone portable", () => {
...
test("fr-TG teacher portal distinguishes niveau from groupe-classe", () => {
...
test("fr-TG teacher content-pack chrome uses groupe-classe (not cours)", () => {
...
test("fr-TG layer has no active Hebrew/homeland/Israel-curriculum local overrides", () => {
  const roots = [
  ...
  assert.deepEqual(hits, [], "dead curriculum residue in fr-TG overlays");
});

test("fr-TG no foreign Francophone leakage and no sole-language French claim", () => {
  const blobs = [
    ...listJsonRel(path.join(ROOT, "locales", LOCALE)).map((rel) =>
      fs.readFileSync(path.join(ROOT, "locales", LOCALE, rel), "utf8")
    ),
  ...
  assert.doesNotMatch(blobs, /(?<![A-Za-z])CI(?![A-Za-z0-9])–/);
  assert.doesNotMatch(blobs, /\b6e\b/);
  assert.doesNotMatch(blobs, /\bEBEP\b/);
  assert.doesNotMatch(blobs, SCOPE_LEAK_RE);
  assert.doesNotMatch(blobs, ALL_FRENCH_CLAIM_RE);
  assert.match(blobs, /\bCP1\b/);
  assert.match(blobs, /\bTogo\b/);
});

test("fr-TG other locales and shared runtime remain untouched", () => {
  ...
  assert.equal(fs.existsSync(path.join(ROOT, "public/tg")), false);
  assert.equal(fs.existsSync(path.join(ROOT, "locales/tg")), false);
  assert.equal(fs.existsSync(path.join(ROOT, "data/english-questions/word-meanings/fr-TG.js")), false);
});
