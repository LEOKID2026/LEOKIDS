/**
 * ar-AE (United Arab Emirates) sparse content-layer checks vs ar-001.
 * No registry wiring, build, or full suite.
 *
 * Authority: UAE MoE — التعليم الأساسي; الحلقة الأولى = 1–4; الحلقة الثانية = 5–8.
 * LEO covers grades 1–6 → حلقة أولى + جزء من الحلقة الثانية (not full cycle 2).
 * Physical class group: الشعبة الصفية. Academic grade: الصف.
 * Fallback target (not wired here): ar-AE → ar-001 → en.
 */
...
const CROSS_COUNTRY_RE =
  /مصر|المصرية|المملكة العربية السعودية|السعودية(?! العربية)|المغرب|الجزائر|تونس|الأردن|الاردن|عمّان|الكويت|قطر|البحرين|جنيه|ريال سعودي/;
...
const MONOLINGUAL_CLAIM_RE =
  /اللغة الوحيدة|فقط بالعربية في (الإمارات|الامارات)|العربية وحدها|لا توجد لغة أخرى|اللغة الرسمية الوحيدة(?!\s+للواجهة)/;
/** Wrong claim that cycle 2 ends at grade 6, or that 1–6 is all حلقة أولى. */
const BAD_CYCLE_CLAIM_RE =
  /الحلقة الثانية[^.؛\n]{0,40}(حتى|إلى)\s*الصف السادس(?![^.؛\n]{0,40}الثامن)|الصفوف?\s*(?:الأول|1)\s*[–\-إلى]+\s*(?:السادس|6)[^.؛\n]{0,30}الحلقة الأولى|الحلقة الأولى[^.؛\n]{0,40}(?:الأول|1)[^.؛\n]{0,20}(?:السادس|6)/;
...
/** Official UAE stage label that must not describe the whole LEO 1–6 audience. */
...
test("ar-AE locale namespaces parse and stay sparse vs ar-001", () => {
  ...
  assert.ok(files.includes("school.json"), "school has الشعبة الصفية overrides");
...
test("ar-AE grade mapping الصف الأول–السادس and UAE education-stage wording", () => {
  ...
  assert.match(seo.homeTitle, /دولة الإمارات العربية المتحدة/);
  assert.match(seo.homeDescription, /النسخة العربية لدولة الإمارات العربية المتحدة/);
  ...
  assert.match(ui.home?.subhead || "", /دولة الإمارات العربية المتحدة/);
  ...
  assert.match(ui.public?.about?.intro1 || "", /حتى الصف الثامن/);
...
test("ar-AE effective surfaces: no misleading المرحلة الابتدائية for LEO grades 1–6", async () => {
  /** @type {string[]} */
  const hits = [];

  // Locales: deep-merge ar-001 ← ar-AE
  ...
  const help = await import("../../data/help-center/ar-AE/index.js");
  const helpBlob = JSON.stringify(help.ALL_ARTICLES_AR_AE);
  if (MARHALA_IBTIDAIYYA_RE.test(helpBlob)) hits.push("help-effective:ALL_ARTICLES_AR_AE");
...
test("ar-AE UAE cycle regression: حلقة أولى 1–4; 5–6 part of حلقة ثانية to 8", async () => {
  const ui = JSON.parse(fs.readFileSync(path.join(ROOT, "locales", LOCALE, "ui.json"), "utf8"));
  ...
  assert.match(about, /الحلقة الثانية تمتد رسميًا حتى الصف الثامن/);
  ...
  const help = await import("../../data/help-center/ar-AE/index.js");
  const welcome = help.BY_SECTION_AR_AE.parents.find((a) => a.slug === "welcome-and-overview");
  ...
  assert.match(welcomeBlob, /حتى الصف الثامن/);
...
test("ar-AE grade/class-group distinction: الصف ≠ الشعبة الصفية; no الفصل الدراسي for physical group", () => {
  const school = JSON.parse(fs.readFileSync(path.join(ROOT, "locales", LOCALE, "school.json"), "utf8"));
  assert.equal(school.portal.choosePhysicalClass, "اختر الشعبة الصفية");
  assert.equal(school.portal.classLabel, "الشعبة الصفية");
  assert.equal(school.portal.colClass, "الشعبة");
  assert.equal(
    school.portal.classesSubtitle,
    "اختر الصف والشعبة الصفية والمادة - التقارير والإدارة حسب الصف"
  );
  ...
  assert.match(JSON.stringify(school), /الشعبة الصفية/);
...
test("ar-AE content packs sparse contract vs ar-001", () => {
...
test("ar-AE help overlays parse on ar-001 base and keep slugs", async () => {
  const help = await import("../../data/help-center/ar-AE/index.js");
  ...
  assert.equal(
    help.ALL_ARTICLES_AR_AE.length,
  ...
  assert.equal(help.SECTIONS_AR_AE.students.title, "دليل للطلاب");
  assert.doesNotMatch(help.SECTIONS_AR_AE.students.title, TALMEEDH_RE);

  const parentSlugs = new Set(baseParents.PARENT_ARTICLES.map((a) => a.slug));
  for (const a of help.BY_SECTION_AR_AE.parents) {
    assert.ok(parentSlugs.has(a.slug), a.slug);
  }

  const welcome = help.BY_SECTION_AR_AE.parents.find((a) => a.slug === "welcome-and-overview");
  const welcomeBlob = JSON.stringify(welcome);
  assert.match(welcomeBlob, /دولة الإمارات العربية المتحدة/);
  ...
  const login = help.BY_SECTION_AR_AE.students.find((a) => a.slug === "student-login");
  assert.match(JSON.stringify(login), /تسجيل دخول الطالب/);
  assert.doesNotMatch(JSON.stringify(login), TALMEEDH_RE);

  const blob = JSON.stringify(help.ALL_ARTICLES_AR_AE);
  assert.doesNotMatch(blob, HEBREW_RE);
  assert.doesNotMatch(blob, CROSS_COUNTRY_RE);
});

test("ar-AE does not ship word-meanings overlay; English learning IDs preserved", () => {
...
test("ar-AE other locales untouched; shared wiring present for Wave 2", () => {
  ...
  assert.match(registry, /"ar-AE"/);
  assert.match(registry, /pathPrefix:\s*"ae"/);
...
test("ar-AE Copilot chrome uses Arabic Master product name مساعد الطيار", () => {
...
test("ar-AE academic-grade surfaces use الصف not درجة; score/angle درجة may remain inherited", async () => {
  /** @type {string[]} */
  const academicHits = [];
  ...
  const helpBlob = JSON.stringify({
    sections: help.SECTIONS_AR_AE,
    articles: help.ALL_ARTICLES_AR_AE,
  });
  if (hasAcademicDarajah(helpBlob)) academicHits.push("data/help-center/ar-AE");
  ...
  const edit = help.BY_SECTION_AR_AE.parents.find((a) => a.slug === "edit-or-delete-student");
