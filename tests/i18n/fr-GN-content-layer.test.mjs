/**
 * fr-GN (Guinée) sparse content-layer checks.
 * No registry wiring, build, or full suite.
 *
 * Grade mapping CP1–CM2: MENA/MEPU-A programmes, GPE ESP 2020, IFADEM.
 * French UI for primary schooling — must not claim French is the only language spoken.
 */
...
/** France 6e / bare CP, Senegal CI year-1, Cameroon SIL, DR Congo EBEP must not leak. */
const FOREIGN_GRADE_RE =
  /\b6e\b|(?<![A-Za-z0-9])CI(?![A-Za-z0-9])|\bSIL\b|\bEBEP\b|\b7e\b|(?<![A-Za-z0-9])CP(?![12A-Za-z0-9])/;
const GN_GRADES = ["CP1", "CP2", "CE1", "CE2", "CM1", "CM2"];
const SCOPE_LEAK_RE =
  /\b(Sénégal|Senegal|Côte d’Ivoire|Cote d'Ivoire|Cameroun|Cameroon|RDC|Congo|Benin|Bénin|France)\b/i;
...
function assertNoScopeLeak(blob, label) {
  ...
  assert.equal(ALL_GUINEA_FRENCH_CLAIM_RE.test(blob), false, `all-Guinea/French-only claim in ${label}`);
}

test("fr-GN locale namespaces parse and stay sparse vs fr-FR", () => {
...
test("fr-GN grade terminology CP1–CM2 (authority-backed; not France/SN/CM/CD)", () => {
  ...
  assert.match(seo.homeTitle, /Guinée/);
  assert.match(seo.learningDescription, /CP1 à CM2/);
  assert.doesNotMatch(seo.homeTitle, ALL_GUINEA_FRENCH_CLAIM_RE);
  ...
  assert.doesNotMatch(allLocaleText, /\bSIL\b/);
  assert.doesNotMatch(allLocaleText, /(?<![A-Za-z0-9])CI(?![A-Za-z0-9])/);
  assert.doesNotMatch(allLocaleText, /(?<![A-Za-z0-9])CP(?![12A-Za-z0-9])/);
  assert.match(allLocaleText, /\bCP1\b/);
  assert.match(allLocaleText, /\bCP2\b/);
  assert.match(allLocaleText, /\bCM2\b/);
});

test("fr-GN content packs sparse contract vs fr-FR", () => {
  ...
  for (const rel of listJsonRel(countryRoot)) {
    ...
    if (isBurnDownIndexPath(rel)) {
      ...
      for (const [key, value] of indexAudit.countryLeaves) {
        ...
        if (SCOPE_LEAK_RE.test(value) || ALL_GUINEA_FRENCH_CLAIM_RE.test(value)) {
    ...
    for (const [key, value] of countryLeaves) {
      ...
      if (
        typeof value === "string" &&
        (SCOPE_LEAK_RE.test(value) || ALL_GUINEA_FRENCH_CLAIM_RE.test(value))
      ) {
        scopeHits.push(`${rel}:${key}`);
      }
  ...
  assert.deepEqual(foreignGradeHits, [], "France/SN/CM/CD grade labels must not remain");
  assert.deepEqual(scopeHits, [], "no other-country or French-only-Guinea claims");
});

test("fr-GN pack grade labels use CP1–CM2 bands", () => {
  const booksUi = JSON.parse(
    fs.readFileSync(path.join(ROOT, "content-packs", LOCALE, "books/ui.json"), "utf8")
  );
  assert.deepEqual(Object.values(booksUi.grades), GN_GRADES);
  ...
  assert.doesNotMatch(JSON.stringify(titles), /\bSIL\b/);
});

test("fr-GN help: no empty parent-report file; inherits fr-FR; overlays win", async () => {
  ...
  assert.ok(fs.existsSync(path.join(helpDir, "index.js")));
  assert.ok(fs.existsSync(path.join(helpDir, "merge-overlays.js")));

  const help = await import(`../../data/help-center/fr-GN/index.js?t=${Date.now()}`);
  ...
  assert.deepEqual(
    Object.keys(help.BY_SECTION_FR_GN).sort(),
    ["parent-report", "parents", "students", "subjects"].sort()
  );
  assert.equal(
    help.BY_SECTION_FR_GN["parent-report"],
    parentReportBase.PARENT_REPORT_ARTICLES,
    "parent-report must be the fr-FR article array by reference (no empty merge)"
  );
  assert.equal(help.BY_SECTION_FR_GN["parent-report"].length, 12);
  assert.equal(
    help.BY_SECTION_FR_GN["parent-report"].length,
    parentReportBase.PARENT_REPORT_ARTICLES.length
  );
  assert.equal(
    help.ALL_ARTICLES_FR_GN.length,
  ...
  const slugs = help.ALL_ARTICLES_FR_GN.map((a) => a.slug);
  assert.equal(new Set(slugs).size, slugs.length, "no duplicate article slugs/IDs");

  const baseSlugs = [
    ...parentsBase.PARENT_ARTICLES,
    ...studentsBase.STUDENT_ARTICLES,
    ...parentReportBase.PARENT_REPORT_ARTICLES,
    ...subjectsBase.SUBJECT_ARTICLES,
  ].map((a) => a.slug);
  for (const slug of baseSlugs) {
    assert.ok(slugs.includes(slug), `missing Help article ${slug}`);
  }

  for (const base of parentReportBase.PARENT_REPORT_ARTICLES) {
    const hit = help.BY_SECTION_FR_GN["parent-report"].find((a) => a.slug === base.slug);
  ...
  for (const a of help.BY_SECTION_FR_GN.parents) {
    assert.ok(parentSlugs.has(a.slug), a.slug);
  }

  const welcome = help.BY_SECTION_FR_GN.parents.find((a) => a.slug === "welcome-and-overview");
  assert.match(JSON.stringify(welcome), /CP1 au CM2/);
  assert.match(JSON.stringify(welcome), /Guinée/);
  assert.doesNotMatch(JSON.stringify(welcome), /\b6e\b/);
  assert.doesNotMatch(JSON.stringify(welcome), ALL_GUINEA_FRENCH_CLAIM_RE);

  const addStudents = help.BY_SECTION_FR_GN.parents.find((a) => a.slug === "add-students");
  assert.match(JSON.stringify(addStudents), /CP1 à CM2/);
  assert.match(JSON.stringify(addStudents), /CP1 — grade_1/);
  assert.match(JSON.stringify(addStudents), /CP2 — grade_2/);
  assert.doesNotMatch(JSON.stringify(addStudents), /\b6e\b/);

  const choose = help.BY_SECTION_FR_GN.students.find((a) => a.slug === "choose-subject-and-grade");
...
test("fr-GN demo false friends: niveau / joueur / accueil", () => {
  ...
  assert.deepEqual(Object.values(demo.grades), GN_GRADES);
});

test("fr-GN UI back-home and Guinea primary terminology", () => {
  ...
  assert.match(ui.home?.subhead || "", /élèves du primaire en Guinée/);
  assert.doesNotMatch(JSON.stringify(ui), /Retour à la maison/);
  assert.doesNotMatch(JSON.stringify(ui), ALL_GUINEA_FRENCH_CLAIM_RE);
  assert.doesNotMatch(JSON.stringify(ui), SCOPE_LEAK_RE);
});

test("fr-GN report detailed surface grade chrome = Niveau", () => {
...
test("fr-GN help students: tu conjugations (no vous forms)", async () => {
  const help = await import(`../../data/help-center/fr-GN/index.js?t=${Date.now() + 1}`);
  const students = help.BY_SECTION_FR_GN.students;
...
test("fr-GN grade-aware report pack: math terms and CP1–CM2 ranges", () => {
  ...
  assert.doesNotMatch(blob, /\bSIL\b/);
...
test("fr-GN school portal: niveau scolaire vs groupe-classe", () => {
...
test("fr-GN student/teacher/parent terminology stays Guinea-appropriate", () => {
  const blobs = [
  ...
  // MEPU-A / arrêté 6982: élève, enseignant/maître, parents d’élèves, directeur d’école.
  ...
  assert.doesNotMatch(blobs, /\bapprenant\b/i);
  assert.match(blobs, /élève/);
  assert.match(blobs, /fiche d’exercices|fiche d'exercices/);
  assert.match(blobs, /directeur d'école|directeur d’école/);
});

test("fr-GN does not ship word-meanings, science overlay, or mutate English targets", () => {
  ...
  const scienceOverlay = path.join(ROOT, "data/science-questions-fr-GN-overlay.js");
  assert.equal(fs.existsSync(scienceOverlay), false);
  const learningContent = path.join(ROOT, "utils/learning-content-fr-GN");
  ...
  assert.equal(skills.grades.g2.sentence_base.title, "Phrases courtes — CP2");
});

test("fr-GN child/adult register: no tu/vous mix introduced in overlays", () => {
  const files = [
    ...listJsonRel(path.join(ROOT, "locales", LOCALE)).map((rel) =>
...
test("fr-GN auth/validation: adresse e-mail and téléphone portable", () => {
  ...
  assert.match(auth.registration?.intent?.school_representative || "", /directeur d'école/);
...
test("fr-GN teacher portal distinguishes niveau from groupe-classe", () => {
  ...
  assert.match(dash.createClassPlaceholder || "", /\b(CP1|CP2|CE1|CE2|CM1|CM2)\b/);
...
test("fr-GN teacher content-pack chrome uses groupe-classe (not cours)", () => {
  ...
  assert.match(dash.copy?.remove_this_student_from_the_class || "", /élève/);
...
test("fr-GN layer has no active Hebrew/homeland/Israel-curriculum local overrides", () => {
  const roots = [
  ...
  assert.deepEqual(hits, [], "dead curriculum residue in fr-GN overlays");
});

test("fr-GN no foreign Francophone leakage in overlays", () => {
  const blobs = [
    ...listJsonRel(path.join(ROOT, "locales", LOCALE)).map((rel) =>
      fs.readFileSync(path.join(ROOT, "locales", LOCALE, rel), "utf8")
    ),
  ...
  assert.doesNotMatch(blobs, /\bSIL\b/);
  assert.doesNotMatch(blobs, /(?<![A-Za-z0-9])CI(?![A-Za-z0-9])–/);
  assert.doesNotMatch(blobs, /\b6e\b/);
  assert.doesNotMatch(blobs, /\bEBEP\b/);
  assert.doesNotMatch(blobs, SCOPE_LEAK_RE);
  assert.doesNotMatch(blobs, ALL_GUINEA_FRENCH_CLAIM_RE);
  assert.match(blobs, /\bCP1\b/);
  assert.match(blobs, /\bGuinée\b/);
});

test("fr-GN other locales and shared runtime remain untouched", () => {
  ...
  assert.equal(fs.existsSync(path.join(ROOT, "public/gn")), false);
  assert.equal(fs.existsSync(path.join(ROOT, "locales/gn")), false);
  assert.equal(
    fs.existsSync(path.join(ROOT, "data/english-questions/word-meanings/fr-GN.js")),
    false
  );
});
