...
test.describe("School physical class report", () => {
  ...
  test("manager opens physical class report with actions", async ({ page }) => {
    ...
    await page.getByPlaceholder(" ").fill(SCHOOL_MANAGER_EMAIL);
    ...
    await expect(physicalDialog.getByText("   …")).toHaveCount(0);
    ...
    await expect(subjectDetail.getByRole("button", { name: " " }).first()).toBeVisible();
    await expect(subjectDetail.getByRole("button", { name: " " }).first()).toBeVisible();

    await subjectDetail.getByRole("button", { name: " " }).first().click();
    ...
    await expect(teacherModal.getByRole("heading", { name: "  " })).toBeVisible();
    ...
    await expect(actDetail.getByRole("button", { name: " " }).first()).toBeVisible();
    ...
    await expect(studentDetail.getByRole("button", { name: " /" }).first()).toBeVisible();
    await studentDetail.getByRole("button", { name: " /" }).first().click();
  ...
  test("physical report API returns scoped data", async ({ request }) => {
    ...
    expect(body.physicalClass?.name).toBe("  1");
