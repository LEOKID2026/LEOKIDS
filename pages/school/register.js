...
export default function SchoolRegisterPage() {
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto px-3 md:px-4 py-3 md:py-8" data-testid="school-register-page">
        <PortalLoginHeading title={t(REG_SCHOOL_TITLE)} className="md:!mb-4" />
        ...
        <div className="rounded-2xl border border-white/15 bg-black/40 p-3 md:p-5">
          ...
          <p className="mt-2.5 md:mt-4 text-xs text-white/60 leading-snug">
            {t("auth.schoolRegisterAlreadyAccount")}{" "}
            <Link href="/teacher/login" className="text-amber-300 underline">
              {t("auth.schoolRegisterTeacherLogin")}
            </Link>
          </p>
        </div>
      </div>
