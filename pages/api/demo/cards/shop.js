...
export default async function handler(req, res) {
  ...
  if (!isCardRewardsEnabled()) {
    return res.status(200).json({ ok: true, ...buildDemoCardsShopFixture(DEMO_COIN_BALANCE), demo: true, fixture: true });
  }
  ...
  try {
    ...
    return res.status(200).json({ ok: true, ...view, demo: true });
  } catch (_err) {
    return res.status(200).json({ ok: true, ...buildDemoCardsShopFixture(DEMO_COIN_BALANCE), demo: true, fixture: true });
  }
}
