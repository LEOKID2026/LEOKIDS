...
export default async function handler(req, res) {
  ...
  const fromRow = body.fromRow;
  const fromCol = body.fromCol;
  const toRow = body.toRow;
  const toCol = body.toCol;
  ...
  try {
    const result = await applyCheckersAction(auth.supabase, {
      roomId,
    ...
    return res.status(200).json({
      ok: true,
      checkers: snap.checkers,
      room: snap.room,
      players: snap.players,
      gameSession: snap.gameSession,
    });
  } catch (err) {
    console.error("[api/arcade/games/checkers/action] unexpected error", err);
