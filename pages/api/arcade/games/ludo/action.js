...
export default async function handler(req, res) {
  ...
  const pieceIndex = body.pieceIndex;
  ...
  try {
    const result = await applyLudoAction(auth.supabase, {
      roomId,
      studentId: auth.studentId,
      action,
      pieceIndex: action === "move" ? pieceIndex : null,
    ...
    console.error("[api/arcade/games/ludo/action] unexpected error", err);
    return res.status(500).json({
      ok: false,
      code: "server_error",
      error: "שגיאת שרת בלודו",
    });
  }
}
