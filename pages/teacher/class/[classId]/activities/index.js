...
function actCopy(key) {
  return globalBurnDownCopy(ACT, key);
}
...
export default function TeacherClassActivitiesPage({ classId }) {
  ...
  return (
    <Layout>
      <TeacherPortalShell
        title={actCopy("class_activities")}
        backHref="/teacher/dashboard"
        backLabel={actCopy("back_to_dashboard")}
      >
        <TeacherClassActivitiesNav classId={classId} />

        <div
          className="flex flex-wrap items-center justify-between gap-3 mb-4"
          data-testid="teacher-class-activities-ready"
        >
          <p className="text-white/70 text-sm">{actCopy("subtitle")}</p>
          <Link
          ...
          >
            {actCopy("new_activity")}
          </Link>
        </div>

        {phase === "loading" ? (
          <p className="text-white/70" data-testid="teacher-class-activities-loading">
            {actCopy("loading")}
          </p>
        ) : null}
        {phase === "error" ? (
        ...
        {phase === "forbidden" ? (
          <p className="text-red-200">{actCopy("forbidden")}</p>
        ) : null}

        {phase === "ready" && activities.length === 0 ? (
          <p className="text-white/60 text-sm" data-testid="teacher-class-activities-empty">
            {actCopy("empty")}
          </p>
        ) : null}
        ...
        {phase === "ready" && activities.length > 0 ? (
          <div
            ...
            data-testid="teacher-class-activities-list"
          >
            <table className="w-full text-sm text-start">
              <thead className="bg-white/5 text-white/70">
                <tr>
                  <th className="px-3 py-2">{actCopy("col_title")}</th>
                  <th className="px-3 py-2">{actCopy("col_status")}</th>
                  <th className="px-3 py-2">{actCopy("col_type")}</th>
                  <th className="px-3 py-2">{actCopy("col_progress")}</th>
                  <th className="px-3 py-2">{actCopy("col_actions")}</th>
                </tr>
              </thead>
              <tbody>
                {activities.map((a) => {
                    ...
                    <tr key={a.activityId} className="border-t border-white/10 hover:bg-white/[0.03]">
                      ...
                      <td className="px-3 py-2">
                        <Link href={href} className="text-amber-300 hover:underline">
                          {a.status === "draft" ? actCopy("edit_launch") : actCopy("view")}
                        </Link>
                      </td>
                    </tr>
                  );
