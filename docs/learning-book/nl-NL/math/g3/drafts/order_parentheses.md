# Parentheses in Calculations

## Metagegevens

| Veld | Waarde |
|-------|-------|
| **learning_page_id** | `math:g3:order_parentheses` |
| **skill_id** | `math:kind:order_parentheses` |
| **subject** | rekenen |
| **groep** | g3 |
| **age_band** | groepen_5_6 |
| **page_type** | step_by_step_procedure |
| **approval_status** | launch_ready |
| **title_english** | Parentheses in Calculations |

**Bronverwijzingen:**
- `data/curriculum-spine/v1/skills.json`
- `docs/learning-book/MATH_GRADE_3_LEARNING_BOOK_PLAN.md`
- `docs/learning-book/MATH_LEARNING_PAGE_TEMPLATE.md`

**Inhoudsbereik:** What's **inside parentheses** first. Short problems with ( ).

---

## 1. Wat gaan we leren?

Vandaag leren we parentheses — ( ) — in a calculation problem.
What's inside the parentheses — first!

---

## 2. Eenvoudige uitleg

(8 + 2) × 3
Parentheses first: 8 + 2 = 10
Then multiply: 10 × 3 = 30
(8 + 2) × 3 = 30
Without parentheses it would be: 8 + 2 × 3 = 8 + 6 = 14 — different!
Parentheses = "do this first."

---

## 3. Visueel / concreet voorbeeld

(12 − 4) ÷ 2
12 − 4 = 8
8 ÷ 2 = 4
(12 − 4) ÷ 2 = 4

---

## 4. Laten we het samen oplossen

Calculate: (5 + 3) × 4 = ?
5 + 3 = 8
8 × 4 = 32
(5 + 3) × 4 = 32

---

## 5. Probeer het zelf

Calculate: (15 − 6) × 4 = ?
Probeer het zelf op te lossen.
Op de volgende pagina kijken we samen de stappen en het antwoord na.

---

## 6. Veelgemaakte fout — let op!

Calculate: (15 − 6) × 4
Parentheses first: 15 − 6 = 9
Then multiply: 9 × 4 = 36
(15 − 6) × 4 = 36

---

## 7. Laten we oefenen!

Nu ken je: parentheses first.
In de oefening vind je ( ).
Calculate inside the parentheses before the rest!
