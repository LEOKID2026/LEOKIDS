# Groep 3 Math Learning Book — Drafts
...
| Crocodile metaphor (`cmp.md`) | **Keep for Groep 3 draft:** `תנין רעב` — child-friendly, draft content only (not final owner-approved product copy) |
...
| `add_tens_only` scope | **Groep 3 cap: 30** — use 10, 20, 30 only |
...
| `mul` example 4 × 3 = 12 | **Accepted for draft use** — within Groep 3 scope (product ≤ 20) |
...
| Scope wording | Negative-number note scoped to Groep 3 page |
...
| `add_tens_only` scope | Explicit **Groep 3 cap 30**; removed ellipsis implying 40/50/100 |
...
- `eq_add_simple`: ontbrekende getal as puzzle; `__` / `מספר חסר`; links to `מסגרת עשר` where helpful
- `eq_sub_simple`: ontbrekende getal in subtraction; concrete number line / objects; no formal algebra
...
- `wp_coins`: coin values added together; "how much in totaal?"; repeated addition only; no spending/change, no multiplication
...
**Groep 3 Math Learning Book:** All **19** skill pages now exist as drafts in this folder.
...
| `utils/math-constants.js` | Groep 3 number range (0–30 max) |
...
| `4 × 3 = 12` in `mul.md` | Accepted — within Groep 3 scope (product ≤ 20) |
...
1. **Create the Groep 3 decisions/signoff document** — consolidate all owner draft decisions (Batches A–D), open questions, and sign-off checklist for `draft` → `review`.
2. **Owner review** of Batch D polish pass and full Groep 3 draft set.
3. After Groep 3 signoff, consider a **Hebrew style guide addendum**, then begin Groep 4 or implementation planning.

**Do not proceed to Groep 4 or implementation until the Groep 3 signoff document is reviewed.**
