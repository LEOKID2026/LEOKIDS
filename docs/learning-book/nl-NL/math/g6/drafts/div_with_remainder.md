# Division with a Remainder

## Metagegevens

| Veld | Waarde |
|-------|-------|
| **learning_page_id** | `math:g6:div_with_remainder` |
| **skill_id** | `math:kind:div_with_remainder` |
| **subject** | rekenen |
| **groep** | g6 |
| **age_band** | groepen_7_8 |
| **page_type** | practice_bridge |
| **approval_status** | launch_ready |
| **title_english** | Division with a Remainder |

**Bronverwijzingen:**
- `data/curriculum-spine/v1/skills.json`
- `docs/learning-book/MATH_GRADE_6_LEARNING_BOOK_PLAN.md`
- `docs/learning-book/MATH_LEARNING_PAGE_TEMPLATE.md`

**Inhoudsbereik:** quotient and remainder; large numbers (850÷7). Check equation: dividend = divisor × quotient + remainder.

---

## 1. Wat gaan we leren?

Vandaag zullen we strengthen division with a remainder — with full multi-step checking.
850 ÷ 7 — does not divide evenly.
A remainder is left.
We bouwen een volledige controlevergelijking.

---

## 2. Eenvoudige uitleg

126,000 ÷ 9 — Step A: find the quotient
9 × 14,000 = 126,000 — divides evenly, no remainder.
Check equation: 126,000 = 9 × 14,000
850 ÷ 7 (for comparison — with remainder): 7 × 121 = 847; 850 − 847 = 3 → 121 with remainder 3.

---

## 3. Visueel / concreet voorbeeld

78,400 ÷ 11 — check:
11 × 7,127 = 78,397 — close; 11 × 7,128 = 78,408 — too big.
11 × 7,127 = 78,397; 78,400 − 78,397 = 3 → 7,127 with remainder 3.
Check: 11 × 7,127 + 3 = 78,400

---

## 4. Laten we het samen oplossen

Calculate: 92,500 ÷ 7 = ? (quotient and remainder)
7 × 13,214 = 92,498
92,500 − 92,498 = 2
Step 3 — check equation: 92,500 = 7 × 13,214 + 2
13,214 with remainder 2

---

## 5. Probeer het zelf

Calculate: 968 ÷ 9 — wat is the quotient and wat is the remainder?
Probeer het zelf op te lossen.
Op de volgende pagina kijken we samen de stappen en het antwoord na.

---

## 6. Veelgemaakte fout — let op!

Calculate: 968 ÷ 9
Find 9 × ? close to 968: 9 × 107 = 963.
Remainder: 968 − 963 = 5
Step 3 — check equation: 9 × 107 + 5 = 963 + 5 = 968
Check:
Remainder 5 < divisor 9
The answer: 107 with remainder 5

---

## 7. Laten we oefenen!

Remainders appear in word problems — hoeveel is left over.
In de oefening vind je division with a remainder — build a check equation and make sure the remainder is smaller than the divisor!
