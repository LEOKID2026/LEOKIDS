 ...
 * Hebrew wording: parent_report_hebrew_copy_spec.md only (via parent-report-copy-spec.js).
...
} from "./parent-report-language/engine-decision-parent-copy.js";
import { buildTopicDiagnosticExplainSectionsHe } from "./parent-report-ui-explain.js";
import {
...
export function buildParentInsightsFromTopicEngineHe(report, apiPayload = null) {
  ...
  for (const [ slot] of bySubject) {
    ...
    if (
      ...
      const mixed = mixedSubjectStrongWeakHe(slot.weak.subjectLabel, strongLabel, weakLabel);
...
function dedupeDiagnosticOverviewAgainstInsights(report, insightLines) {
  ...
  const numbersOnlyLine = (line) => {
    ...
    for (const topic of insightTopics) {
      ...
      const subj = subjectLabel(String(row.subjectId || "")) || String(row.subjectLabel || "");
