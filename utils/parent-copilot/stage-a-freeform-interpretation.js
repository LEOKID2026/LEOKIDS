...
const INTENT_PARAPHRASES = {
  ...
  what_is_going_well: [
    ...
    /what\s+(?:is\s+)?going\s+well/i,
    /where\s+(?:is\s+)?(?:he|she|the\s+child)?\s*strong/i,
    /what\s+(?:are\s+)?(?:the\s+)?strengths?/i,
    /(?:which|what)\s+subject\s+is\s+(?:the\s+)?strongest/i,
    /(?:the\s+)?strongest\s+subject/i,
    /what\s+is\s+(?:he|she|the\s+child)\s+good\s+at/i,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
  ...
  what_not_to_do_now: [
    /what\s+(?:should\s+(?:we|i)\s+)?not\s+(?:do|practice)\s+now/i,
    /what\s+to\s+avoid\s+(?:now|this\s+week)/i,
    /what\s+(?:is\s+)?not\s+worth\s+doing\s+now/i,
    /what\s+should\s+(?:we|i)\s+avoid/i,
    /(?!)/u,
  ],
  what_is_still_difficult: [
    /what\s+is\s+still\s+(?:difficult|hard|weak)/i,
    /where\s+(?:is\s+)?(?:the\s+)?(?:difficulty|weakness)/i,
    /what\s+(?:is\s+)?(?:still\s+)?(?:hard|difficult|weak)/i,
    /what\s+needs?\s+(?:reinforcement|strengthening|work)/i,
    /where\s+(?:is\s+)?(?:he|she|the\s+child)\s+struggling/i,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
  ...
  how_to_tell_child: [
    /how\s+(?:to\s+|do\s+i\s+|can\s+i\s+)?(?:tell|explain|say)\s+(?:this\s+)?(?:to\s+)?(?:the\s+)?child/i,
    /how\s+(?:should\s+i\s+)?explain\s+(?:this\s+)?to\s+(?:my\s+)?(?:son|daughter|kid|child)/i,
    /how\s+to\s+say\s+(?:this\s+)?(?:to\s+)?(?:the\s+)?child\s+at\s+home/i,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
  ...
  question_for_teacher: [
    /what\s+(?:should\s+i\s+)?ask\s+(?:the\s+)?teacher/i,
    /question\s+for\s+(?:the\s+)?teacher/i,
    /what\s+to\s+ask\s+(?:the\s+)?teacher/i,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
  ...
  is_intervention_needed: [
    /not\s+clear|unclear|uncertainty|not\s+sure/i,
    /is\s+(?:an?\s+)?intervention\s+needed/i,
    /do\s+(?:we|i)\s+need\s+(?:extra\s+)?help/i,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
  ...
  clarify_term: [
    ...
    /what\s+do\s+(?:the\s+)?(?:numbers|figures)\s+mean/i,
    /what\s+does\s+this\s+say/i,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
    /(?!)/u,
