/**
 * Portuguese Brazil (pt-BR) rebuilders for geometry question stems.
 ...
 * @returns {string|null}
 */
export function rebuildGeometryStemPtBr(question) {
 ...
 * @returns {string}
 */
export function geometryFallbackStemPtBr(question) {
...
/**
 * Localize geometry question for Portuguese Brazil (pt-BR) display.
 * @param {Record<string, unknown>} question
 */
export function localizeGeometryQuestionPtBr(question) {
  ...
  const rebuilt = rebuildGeometryStemPtBr({ ...question, ...base, params: question.params });
  let resolvedStem = rebuilt && !containsHebrew(rebuilt) ? rebuilt : null;
  if (!resolvedStem) {
    resolvedStem = geometryFallbackStemPtBr(question);
  }
