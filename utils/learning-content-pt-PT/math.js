/**
 * Portuguese (Portugal) (pt-PT) rebuilders for math question stems.
 * English is the authority; params/numbers/operators unchanged.
 * Generated by scripts/i18n/bootstrap-pt-PT-stem-rebuilders.mjs — review instructional copy.
 */
...
export function rebuildMathStemPtPt(question) {
  ...
  if (kind === "mul_skip_count_g1") {
    ...
    return `Conta de ${p.perGroup} em ${p.perGroup}: ${head}, ${BLANK}`;
  }
  if (kind === "ns_number_line" || kind === "ns_number_line_g1") {
    ...
    return `Preenche o número que falta na reta numérica: ${line}`;
  }
  ...
  if (kind === "frac_simplify_intro_g4" || kind === "frac_simplify_intro_g3") {
    if (p.num != null && p.den != null) {
      return `Simplifica a fração ${p.num}/${p.den}: ${BLANK}`;
    }
  }
  if (kind === "frac_equivalent_expand" || kind === "frac_equivalent") {
    if (p.num != null && p.den != null && p.factor != null) {
      return `Encontra uma fração equivalente para ${p.num}/${p.den} (multiplique por ${p.factor}): ${BLANK}`;
    }
  }
  ...
  if (kind === "wp_simple_sub" || kind === "wp_simple_sub_g2") {
    ...
    return `Leo tem ${p.total} autocolantes. Ele dá ${p.give} para um amigo. Quantos autocolantes Leo ainda tem?`;
  }
  if (kind === "wp_pocket_money" || kind === "wp_pocket_money_g2") {
    return `A Emma tem ${p.money} euros. Ela compra um lanche por ${p.toy} euros. Quanto dinheiro resta?`;
  }
  ...
  if (kind === "wp_coins") {
    return `Leo tem ${p.coins1} moedas de 1 euro e ${p.coins2} moedas de 2 euros. Quanto dinheiro ele tem ao todo?`;
  }
  if (kind === "wp_coins_spent") {
    return `Leo tem € ${p.total} em moedas. Ele compra doces por € ${p.spent}. Quanto dinheiro resta?`;
  }
  ...
  if (kind === "wp_shop_discount") {
    return `Uma camisa custa € ${p.price} com um desconto de ${p.discPerc}%. Quanto pagas após o desconto?`;
  }
  ...
  if (kind === "wp_multi_step" || kind === "wp_multi_step_g6") {
    return `Leo tem € ${p.money}. Ele compra ${p.a} canetas e ${p.b} lápis, e cada item custa € ${p.price}. Quanto dinheiro resta depois das compras?`;
  }
  ...
  return applyMathLevelPresentationPtPt(
...
export function applyMathLevelPresentationPtPt(question, ctx) {
  ...
  if (kind === "ns_complement100") {
    ...
    if (b != null && Number.isFinite(c)) {
      if (mathLevelKey === "easy") {
        return `Faz ${c}: o que acrescentas a ${b} para chegar a ${c}? = ${BLANK}`;
      }
  ...
  if (kind === "ns_complement10") {
    ...
    if (b != null && Number.isFinite(c)) {
      if (mathLevelKey === "easy") {
        return `Até ${c}: o que acrescentas a ${b} para terminar em ${c}? = ${BLANK}`;
      }
      if (mathLevelKey === "medium") {
  ...
  if (kind === "scale_find") {
    ...
    if (ml != null && rl != null) {
      if (mathLevelKey === "easy") {
        return `Em um mapa, um segmento tem ${ml} cm de comprimento e na vida real tem ${rl} cm. Completa a escala como 1:${BLANK}`;
      }
      if (mathLevelKey === "medium") {
  ...
  if (selectedOp === "compare" || kind === "cmp") {
    ...
    if (mathLevelKey === "easy") {
      const opts = [
        `Compara os dois números e preencha (<, =, >): ${raw}`,
        `Sinal de comparação entre os números: ${raw}`,
        `Escolhe < , = ou > — compare: ${raw}`,
        `Compara os valores e preencha o sinal: ${raw}`];
      return opts[pv].trim();
    }
    if (mathLevelKey === "medium") {
      const opts = [
        `Preenche o sinal de comparação correto: ${raw}`,
        `Qual signo compara o par? ${raw}`,
        `Combine o sinal de comparação correto: ${raw}`,
        `Preenche o sinal entre as expressões numéricas: ${raw}`];
      return opts[pv].trim();
    }
    const opts = [
      `Preenche o sinal de comparação — verifique antes de escolher: ${raw}`,
  ...
  if (selectedOp === "powers" && (kind === "power_base" || kind === "power_calc")) {
    ...
    if (kind === "power_base") {
      ...
      if (mathLevelKey === "medium") return `Puzzle de poder — ${q0}`;
  ...
  if (kind === "dec_add" || kind === "dec_sub") {
    ...
    if (a != null && b != null) {
      ...
      if (kind === "dec_add") {
        if (mathLevelKey === "easy") {
          return pv === 0
            ? `Adiciona decimais: ${af} + ${bf} = ${BLANK}`
            : `Soma direta: ${af} + ${bf} = ${BLANK}`;
        }
        return `Adiciona decimais: ${af} + ${bf} = ${BLANK}`;
      }
      if (mathLevelKey === "easy") {
        return pv === 0
          ? `Subtrai decimais: ${af} − ${bf} = ${BLANK}`
          : `Diferença direta: ${af} − ${bf} = ${BLANK}`;
      }
      return `Subtrai decimais: ${af} − ${bf} = ${BLANK}`;
    }
  }

  if (selectedOp === "sequences") {
    if (mathLevelKey === "easy") {
      return q0.replace(/^Continue the sequence\b/i, "Continua o padrão numérico");
    }
    return q0;
  }
...
function localizeMathField(_field, value, question) {
  ...
  const rebuilt = rebuildMathStemPtPt(question);
  if (rebuilt && !containsHebrew(rebuilt) && (_field === "question" || _field === "exercícioTexto" || _field === "questionLabel")) {
    return rebuilt;
  }

  const presented = applyMathLevelPresentationPtPt(text, {
    selectedOp: question?.operation || inferSelectedOp(question),
...
/**
 * Localize math question for Portuguese (Portugal) (pt-PT) display.
 ...
 */
export function localizeMathQuestionPtPt(question) {
  ...
  const resolvedStem = stem || "Resolve.";
