...
import { SUBJECT_OWNER_COPY_TEMPLATE_IDS } from "./parent-report-language/parent-report-owner-copy-templates.js";
import {
...
} from "./parent-report-language/grade-context-parent.js";
...
function buildSubjectOpeningLineHe(sp, lab) {
  ...
  if (domRisk && domRisk !== globalBurnDownCopy("utils__detailed-report-parent-letter", "sparse_data") && w0) {
...
function buildSubjectDiagnosisLineHe(sp, lab) {
  ...
  if (trendLine && domRisk && domRisk !== globalBurnDownCopy("utils__detailed-report-parent-letter", "sparse_data")) {
...
function applySubjectNarrativeGuardrails(sp, letter) {
  ...
  const clearWeak = findClearWeakTopicInSubject(sp);
  if (clearWeak) {
    const lab = sp?.subjectLabel || "the subject";
    const topicCore = displayTopicCoreHe(clearWeak.label) || clearWeak.label;
    return {
      ...letter,
...
export function buildSubjectParentLetter(sp, opts = {}) {
  ...
  const base = {
    opening: normalizeParentFacing(stripGuillemetsHe(opening)),
    diagnosisHe: normalizeParentFacing(stripGuillemetsHe(diagnosisHe)),
    homeAction: normalizeParentFacing(String(homeAction || "")),
    closing: normalizeParentFacing(stripGuillemetsHe(closing)),
...
export function buildTopicRecommendationNarrative(tr) {
  ...
  return {
    snapshot: normalizeParentFacing(stripGuillemetsHe(snapshotOut)),
    homeLine: normalizeParentFacing(stripGuillemetsHe(homeFromContract)),
    cautionLineHe: cautionFromContract ? normalizeParentFacing(stripGuillemetsHe(cautionFromContract)) : "",
  };
}

/** Phase 10–11 — short lines for parent-facing wording (mapped from parent-report-ui-explain) */
export {
