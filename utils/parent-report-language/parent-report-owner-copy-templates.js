...
import { parentReportOwnerTopicCopyTemplatesHe } from "./parent-report-owner-topic-copy-templates.js";
 ...
 */
export function buildSubjectOwnerCopySlots(contract, subjectLabel = "") {
  ...
  return {
    subjectName: str(subjectLabel || contract.subjectLabelKey),
 ...
 * @returns {string|null}
 */
export function resolveSubjectOwnerCopyFromContract(contract, templateId, subjectLabel = "") {
  if (!contract?.blockedLegacySummary) return null;
  const slots = buildSubjectOwnerCopySlots(contract, subjectLabel);
