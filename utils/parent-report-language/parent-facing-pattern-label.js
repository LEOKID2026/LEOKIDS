...
export function parentFacingPatternLabelHe(unit) {
  ...
  if (unitTaxonomyId(unit) === "M-10") {
    return normalizeParentFacing(parentFacingM10PatternLabelHe(unit));
  }
  ...
  if (remapped) return normalizeParentFacing(remapped);
  return normalizeParentFacing(rewriteEngineTaxonomySnippetForParentHe(raw));
}
...
export function parentFacingDiagnosisSnippetHe(unit, rawText) {
  ...
  if (unitTaxonomyId(unit) === "M-10" && text.includes(M10_ENGINE_PATTERN_HE)) {
    ...
    return normalizeParentFacing(text.split(M10_ENGINE_PATTERN_HE).join(mapped));
  }
  const rewritten = rewriteEngineTaxonomySnippetForParentHe(text);
  if (rewritten.includes(M10_ENGINE_PATTERN_HE)) {
    return normalizeParentFacing(rewritten.split(M10_ENGINE_PATTERN_HE).join(parentFacingM10PatternLabelHe(unit)));
  }
  return normalizeParentFacing(rewritten);
}
