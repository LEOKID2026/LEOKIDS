 ...
 * Centralizes replacement for the old generic "" line.
 */
...
export function withholdSummaryCopyHe(mode, ctx) {
  ...
  const subjectLabel = String(ctx.subjectLabel || "").trim();
  const mediumSubjectAware = subjectLabel
    ? `There's a volume of practice in ${subjectLabel}; to reduce misinterpretation - it's worth continuing focused practice and watching for what repeats.`
    : "There's a certain volume of practice; to reduce misinterpretation - it's worth continuing focused practice and watching for what repeats.";
  const accMaybe =
  ...
  if (
    ...
    if (m === "subject" && subjectLabel) {
      if (weakLabel) {
        return `In ${subjectLabel} a clear reinforcement point is visible in ${weakLabel} - it's worth reinforcing it with short, targeted practice.`;
      }
      return `In ${subjectLabel} a clear reinforcement point is visible based on the practice - it's worth focusing on topics that need reinforcement.`;
    }
  ...
  if (volume > 0 && volume < 35) {
    if (globalVolumeSupportsRichCopy) {
      ...
      if (reportAcc != null && reportAcc > 0 && reportAcc <= 55 && volume >= 15) {
        if (m === "subject") {
          return subjectLabel
            ? `In ${subjectLabel} a relative difficulty appears based on the accuracy in the practice summary - it's worth focusing on short, targeted practice.`
            : mediumSubjectAware;
        }
  ...
  if (reportAcc != null && reportAcc > 0 && reportAcc <= 55 && volume >= 25) {
    if (m === "subject" && subjectLabel) {
      return `In ${subjectLabel} a relative difficulty appears based on the accuracy in the practice summary - it's worth focusing on short, targeted practice.`;
    }
    if (m === "topic") {
  ...
  if (volume >= 90 && su === 0) {
    if (m === "subject" && subjectLabel) {
      return `There's a significant volume of practice in ${subjectLabel}; no sharp recurring difficulty is visible right now - it's worth continuing focused practice and watching for what repeats.`;
    }
...
export function withholdConfidenceSummaryFallbackHe(ctx) {
  ...
  if (volume >= 40) {
    return subjectLabel
      ? `There's a volume of practice in ${subjectLabel} - it's worth continuing focused practice and watching for what repeats.`
      : "There's a certain volume of practice - it's worth continuing focused practice and watching for what repeats.";
  }
  return subjectLabel
    ? `There's a practice foundation in ${subjectLabel} - it's worth continuing focused practice and watching how it develops.`
...
export function isGenericCautiousPracticeLineHe(text) {
  const t = String(text || "");
  const hasPractice = /\s+\s+|there'?s?\s+(?:some\s+)?practice\s+data/i.test(t);
  const hasCautious = /\s+\s+\s+\s+|what\s+the\s+practice\s+shows\s+is\s+still\s+tentative/i.test(t);
