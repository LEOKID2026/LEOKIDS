...
/**
 * Qatar public locale prefix is `/qa` (ar-QA). Must not be treated as QA tooling.
 * @param {string} pathname
 */
export function isQatarPublicLocalePath(pathname) {
  const p = String(pathname || "").toLowerCase();
  if (!p) return false;
  return p === "/qa" || p.startsWith("/qa/");
}

/**
 * Internal QA tooling routes (not Qatar locale `/qa`).
 * Authority samples: `/tools/qa`, nested `/tools/qa/*`, and `*-qa-*` path segments.
 * @param {string} pathname
 */
export function isInternalQaToolingPath(pathname) {
  const lower = String(pathname || "").toLowerCase();
  if (!lower) return false;
  if (isQatarPublicLocalePath(lower)) return false;
  if (lower.includes("-qa-")) return true;
  if (lower === "/tools/qa" || lower.startsWith("/tools/qa/")) return true;
  // Legacy non-locale paths that embed a `/qa` segment (e.g. `/internal/qa/foo`).
  if (lower.includes("/qa/") || lower.endsWith("/qa")) return true;
  return false;
}

/**
 * Language switcher in shared Layout HUD — product surfaces only.
 * Hidden on admin, internal dev tools, prototypes, POC, and QA tooling routes.
 * Not gated by environment: always shown on allowed product paths while the site is in development.
 * Qatar locale `/qa` remains a normal product surface.
 * @param {string} pathname
 */
export function shouldShowLayoutLanguageSwitcher(pathname) {
  const path = String(pathname || "");
  if (!path) return false;

  if (path.startsWith("/admin")) return false;
  if (path.startsWith("/dev/")) return false;
  if (path.startsWith("/learning/dev")) return false;
  if (path === "/learning/dev-student-simulator") return false;
  if (path === "/learning/dev-db-report-preview") return false;

  const lower = path.toLowerCase();
  if (lower.includes("prototype")) return false;
  if (lower.includes("/poc/") || lower.endsWith("/poc") || lower.includes("-poc")) return false;
  if (isInternalQaToolingPath(path)) return false;
...
export function shouldShowLayoutThemePicker(pathname) {
  ...
  if (
    ...
    (path.startsWith("/school") && path !== "/schools") |
...
const PUBLIC_NAV = [
  ...
  { href: "/schools", labelKey: "ui.nav.schoolPortal" },
...
const PURE_PUBLIC_MARKETING_PATHS = new Set(["/", "/kids", "/parents", "/teachers", "/schools"]);
...
/**
 * Area-aware home route for HUD logo / "" when not on the public marketing shell.
 * @param {string} pathname
 * @param {{ authPortal?: string, activePortal?: string || null }} [options]
 */
