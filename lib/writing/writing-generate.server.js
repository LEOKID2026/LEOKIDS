 ...
 * } || { ok: false, status: number, code: string, message?: string }}
 */
export function generateReadyWritingPayload(entry, localeOpts = {}) {
  if (!entry?.slug || !entry.catalogNumber) {
    return { ok: false, status: 400, code: "invalid_ready_entry" };
  }

  const worksheetPayload = buildReadyWritingPayload(entry, localeOpts);
  const integrity = validateWritingAnswerIntegrity(worksheetPayload);
  if (!integrity.pass) {
    return {
      ok: false,
...
/**
 * @param {string} slug
 * @param {{ publicOnly?: boolean, interfaceLocale?: string, contentLocale?: string, instructionLocale?: string }} [opts]
 * @returns {{
 *   ok: true,
 *   worksheetPayload: WritingWorksheetPayload,
 *   generation: Record<string, unknown>,
...
export function generateReadyWritingBySlug(slug, opts = {}) {
  ...
  return generateReadyWritingPayload(entry, {
    ...
    contentLocale: opts.contentLocale || opts.instructionLocale,
  });
}
