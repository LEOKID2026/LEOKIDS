 ...
 * @returns {"en"}
 */
function languageForRequest(_request) {
  return "en";
}
...
export function buildWritingMeta(request, titleHeOverride) {
  ...
  const presetTitle =
    ...
    REFERENCE_SHEET_PRESETS[/** @type {keyof typeof REFERENCE_SHEET_PRESETS} */ (presetKey)]
      ...
      : null;
  return {
    titleHe: titleHeOverride || presetTitle || WRITING_CATEGORY_LABELS_EN[request.writingCategory] || "Writing practice",
    categoryHe: WRITING_CATEGORY_LABELS_EN[request.writingCategory] || "",
...
function buildReferenceSheetBlocks(request, pageIdx) {
    ...
    return [
      {
        ...
        direction,
        textHe: "No practice content was found.",
      },
    ];
  }
  ...
  const presetTitle =
    typeof presetKey === "string" && REFERENCE_SHEET_PRESETS[/** @type {keyof typeof REFERENCE_SHEET_PRESETS} */ (presetKey)]
      ? REFERENCE_SHEET_PRESETS[/** @type {keyof typeof REFERENCE_SHEET_PRESETS} */ (presetKey)].titleHe
      : WRITING_CATEGORY_LABELS_EN[request.writingCategory] || "Writing practice";
  ...
  return [
    ...
    },
    {
      blockType: "instruction",
      direction,
      textHe: "Trace each character once.",
    },
    {
      blockType: "practice",
      direction,
  ...
  const blocks = [
    {
      blockType: "title",
      direction,
      textHe: WRITING_CATEGORY_LABELS_EN[request.writingCategory] || "Writing practice",
    },
    {
      blockType: "instruction",
      direction,
...
function instructionHeForRequest(request) {
  if (request.lineTemplate === "reference_sheet") {
    return "Trace each character once.";
  }
  if (request.tracingMode === "copy") return "Copy by hand";
  if (request.tracingMode === "independent") return "Write on your own";
  if (request.writingCategory === "prewriting") return "Follow the lines";
  if (
    request.writingCategory === "numbers" &&
    /** @type {import("./writing-worksheet-types.js").NumbersWritingRequest} */ (request).numberMode ===
      "quantity_match"
  ) {
    return "Count and match the quantity";
  }
  return "Trace the example and write";
}
