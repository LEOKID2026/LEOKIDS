...
export function validateWritingRequest(body) {
  ...
  const base = {
    ...
    ...(typeof body.referenceSheetPreset === "string" && body.referenceSheetPreset
      ...
      : {}),
    ...(typeof body.interfaceLocale === "string" && body.interfaceLocale.trim()
      ? { interfaceLocale: String(body.interfaceLocale).trim() }
      : {}),
    ...(typeof body.contentLocale === "string" && body.contentLocale.trim()
      ? { contentLocale: String(body.contentLocale).trim() }
      : {}),
    ...(typeof body.instructionLocale === "string" && body.instructionLocale.trim()
      ? { instructionLocale: String(body.instructionLocale).trim() }
      : {}),
  };
  ...
  if (writingCategory === "english_words") {
    ...
    if (wordPackId === "custom") {
      ...
      words = wordsFromPack(getWritingWordPacks("en"), wordPackId);
    }
    words = [...new Set(words)];
...
export function validatePublicWritingDemo(body) {
  ...
  if (request.writingCategory === "english_words") {
    const words =
      request.wordPackId === "custom" && request.words
        ? request.words
        : wordsFromPack(getWritingWordPacks("en"), request.wordPackId || "animals");
