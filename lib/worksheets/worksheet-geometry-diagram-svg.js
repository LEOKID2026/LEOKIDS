...
function dimLabel(x, y, label, st, opts = {}) {
  ...
  return `<text x="${x}" y="${y}" text-anchor="${anchor}" font-size="${fontSize}" font-weight="${weight}" fill="${st.text}" font-family="${globalBurnDownCopy("lib__worksheets__worksheet-geometry-diagram-svg", "arial_helvetica_sans_serif")}" style="font-weight:${weight};font-synthesis:none">${label}</text>`;
}

/**
 * Dimension as separate LTR pieces: number left, unit right (visual "13 cm").
 * Avoids BiDi reversing a single "13 cm" string inside SVG.
