...
export function validateReadyCatalogShape() {
  ...
  if (READY_WORKSHEET_CATALOG.length < 25) {
    errors.push(`catalog too small: ${READY_WORKSHEET_CATALOG.length} < 25`);
  }
...
export async function validateReadyCatalogEntryGenerates(entry) {
  ...
  if (pub.meta.level !== worksheetLevelLabel(entry.subjectId, entry.levelKey)) {
    return { pass: false, error: `${entry.slug}: level mismatch` };
  }
...
export function listReadyCatalogSummaryRows() {
  return READY_WORKSHEET_CATALOG.map((entry) => ({
    slug: entry.slug,
    subjectId: entry.subjectId,
    subject: worksheetSubjectLabel(entry.subjectId),
    gradeKey: entry.gradeKey,
    grade: worksheetGradeLabel(entry.subjectId, entry.gradeKey),
    topicKey: entry.topicKey,
    topic: worksheetTopicLabel(entry.subjectId, entry.topicKey),
    levelKey: entry.levelKey,
    level: worksheetLevelLabel(entry.subjectId, entry.levelKey),
    count: entry.count,
  }));
}
