...
export function listShapesBasicWorksheetPool(gradeKey, levelKey) {
  ...
  const lv = String(levelKey || "medium").toLowerCase();
  const shapes = [globalBurnDownCopy("lib__worksheets__worksheet-geometry-thin-pool.server", "square"), globalBurnDownCopy("lib__worksheets__worksheet-geometry-thin-pool.server", "rectangle"), "Triangle", "Parallelogram"];
  /** @type {Array<Record<string, unknown>>} */
  const out = [];
    ...
    for (let v = 0; v < 6; v += 1) {
      ...
      out.push({
        question: [
          ...
          `Identify angles in a ${shape} with side ${side}: (1 = 2, 2 = 3, 3 = 4, 4 = none)`][v % 6],
        correctAnswer: shape === globalBurnDownCopy("lib__worksheets__worksheet-geometry-thin-pool.server", "square") || shape === globalBurnDownCopy("lib__worksheets__worksheet-geometry-thin-pool.server", "rectangle") ? "4" : "2",
        answers: ["2", "3", "4", "none"],
        topic: "shapes_basic",
        params: {
          shape,
...
function listQuadrilateralsWorksheetPool(levelKey) {
  ...
  return listClassifyDiagramPool(
    stems,
    [globalBurnDownCopy("lib__worksheets__worksheet-geometry-thin-pool.server", "square"), globalBurnDownCopy("lib__worksheets__worksheet-geometry-thin-pool.server", "rectangle"), "Parallelogram", "Trapezoid"],
    "quadrilaterals",
    "quadrilaterals",
    lv
  );
