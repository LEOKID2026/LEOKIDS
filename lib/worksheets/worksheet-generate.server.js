...
/**
 * @param {Record<string, unknown> & { inkSave?: boolean, title?: string }} input
 * @returns {Promise<
...
export async function generateWorksheetForParent(input) {
  ...
  const meta = buildWorksheetPayloadMeta({
    ...
    instructionLocale:
      typeof input.instructionLocale === "string"
        ? input.instructionLocale
        : typeof input.contentLocale === "string"
          ? input.contentLocale
          : typeof input.interfaceLocale === "string"
      ...
      typeof input.contentLocale === "string"
        ...
        : typeof input.instructionLocale === "string"
          ...
          : typeof input.interfaceLocale === "string"
            ...
            : "en",
    interfaceLocale:
      typeof input.interfaceLocale === "string" ? input.interfaceLocale : "en",
  });

  const worksheetPayload = buildWorksheetPayload(questions, meta, {
    subjectId: selectorParams.subjectId,
    mathPracticeFormat: mathPracticeFormat || selectorParams.mathPracticeFormat,
    instructionLocale: meta.instructionLocale,
    contentLocale: meta.contentLocale,
    interfaceLocale: meta.interfaceLocale,
...
export async function generateAnswerKeyForParent(input) {
  ...
  if (expectedFp && !worksheetFingerprintsMatch(expectedFp, worksheetFingerprint)) {
    return {
      ...
      message:
        "The answer key was updated for the current questions. Try opening the answer key again.",
    };
  }
