/**
 * Topic options for parent worksheet hub — core subjects only.
...
import { worksheetTopicLabelEn } from "./worksheet-meta-labels-en.server.js";
...
export function worksheetTopicOptionsForGrade(subjectId, gradeKey) {
  if (subjectId === "geometry") {
    const topics = listGeometryTopicsForGrade(gradeKey)
      ...
      .map((key) => ({
        key,
        label: worksheetTopicLabelEn("geometry", key),
      }));
    if (canExposeWorksheetMixed(subjectId, gradeKey, topics)) {
      topics.push({ key: "mixed", label: worksheetTopicLabelEn("geometry", "mixed") });
    }
    return topics;
  }

  const topics = topicOptionsForSubject(subjectId, gradeKey)
    .filter(({ key }) => key !== "mixed")
    .map(({ key }) => ({
      key,
      label: worksheetTopicLabelEn(subjectId, key),
    }));
  ...
  if (
    ...
    topics.push({ key: "mixed", label: worksheetTopicLabelEn(subjectId, "mixed") });
  }
