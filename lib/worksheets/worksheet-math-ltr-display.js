...
import { splitLearningMixedHebrewMathRuns } from "../../utils/learning-mixed-rtl-math-render.js";
...
/**
 * @param {import("../../utils/learning-mixed-rtl-math-render.js").LearningMixedRun[]} runs
 * @returns {boolean}
 */
...
export function splitWorksheetStemProseAndMath(text) {
  ...
  const trailingFrac = value.match(/(?!)/u);
  if (trailingFrac) {
  ...
  const mixedToFrac = value.match(/(?!)/u);
  if (mixedToFrac) {
    return {
