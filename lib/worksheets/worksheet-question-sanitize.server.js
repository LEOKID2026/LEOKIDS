...
export function toPrintableWorksheetQuestion(raw, ctx) {
  const localizedRaw =
    ctx.subject === "english"
      ? localizeEnglishQuestionEn(raw, {
          instructionLocale: ctx.instructionLocale || ctx.contentLocale || ctx.interfaceLocale || "en",
          interfaceLocale: ctx.interfaceLocale || ctx.contentLocale || "en",
        }) || raw
      : raw;
  const rawStem = extractStemHe(localizedRaw, { subject: ctx.subject });
  const optionsFromRaw = extractOptionsHe(localizedRaw);
  const sanitized =
    sanitizeQuestionForStudentDisplay(localizedRaw) || localizedRaw;
  ...
  // Global English: prefer rebuilt LTR stem over any residual Hebrew/RTL-ordered prompt.
  if (ctx.subject === "english") {
    const localizedStem = extractStemHe(localizedRaw, { subject: "english" });
    if (localizedStem) stemHe = finalizeWorksheetStemHe(localizedStem);
  }
...
export function toAnswerKeyEntry(raw, ctx) {
  const localizedRaw =
    ctx.subject === "english"
      ...
      : raw;
  const sanitized = sanitizeQuestionForStudentDisplay(localizedRaw) || localizedRaw;
  let correctAnswerHe = "";
