...
export function buildQuestionCatalogItems() {
  return READY_WORKSHEET_CATALOG.map((entry) => ({
    ...
    subject: worksheetSubjectLabelEn(entry.subjectId),
    gradeKey: entry.gradeKey,
    grade: worksheetGradeLabelEn(entry.subjectId, entry.gradeKey),
    ...
    level: worksheetLevelLabelEn(entry.subjectId, entry.levelKey),
    count: entry.count,
...
export function buildUnifiedWorksheetCatalogItems(locale = "en") {
  return [...buildQuestionCatalogItems(), ...buildWritingCatalogItems(locale)];
}
