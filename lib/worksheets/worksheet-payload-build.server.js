...
export function worksheetPayloadToPreviewHtml(payload) {
  ...
  const header = `
    <header class="worksheet-header worksheet-header-centered">
      <div class="worksheet-header-top">
        ...
        <h1 class="worksheet-title">${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "worksheet")}</h1>
      </div>
      ...
      <div class="worksheet-fields worksheet-fields-centered">
        <span>${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "name")}</span>
        <span>${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "date")}</span>
      </div>
    </header>`;
  ...
  const renderQuestionArticle = (q, index = 0) => {
      ...
      const options =
        q.optionsHe?.length
          ? `<div class="worksheet-options-section"><p class="worksheet-options-heading">${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "options")}</p><ol class="worksheet-options worksheet-options-grid">${q.optionsHe
              .map((o, i) => {
      ...
      const answerLine =
        ...
        (useMathPrintPages |
          ...
          ? `<div class="worksheet-math-answer-line" aria-hidden="true"><span class="worksheet-math-answer-line-label">${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "answer")}</span><span class="worksheet-math-answer-line-blank"></span></div>`
          : "";
      return `
      <article class="${cardClass}" data-index="${q.displayIndex}">
        <h2 class="worksheet-question-title">${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "question")}<span class="worksheet-question-number">${q.displayIndex}</span></h2>
...
export function answerKeyPayloadToPreviewHtml(payload) {
  ...
  const rows = answers
    .map((a) => {
      ...
      return `<li class="answer-key-item"><div class="answer-key-item-head">${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "answer_2")}<span class="worksheet-question-number">${a.displayIndex}</span></div>${bodyHtml}${
        a.explanationHe
  ...
  return `<!DOCTYPE html>
<html lang="${htmlLang}" dir="${htmlDir}">
<head><meta charset="utf-8"><title>Answer key - ${escapeWorksheetHtml(meta.titleHe)}</title></head>
...
<header class="worksheet-header worksheet-header-centered">
  <div class="worksheet-header-top">
    ...
    <h1 class="worksheet-title">${globalBurnDownCopy("lib__worksheets__worksheet-payload-build.server", "answer_key")}</h1>
  </div>
  <p class="worksheet-meta">${escapeWorksheetHtml(meta.subjectHe)} · ${escapeWorksheetHtml(meta.gradeHe)} · ${escapeWorksheetHtml(meta.topicHe)} · ${escapeWorksheetHtml(meta.levelHe)}</p>
</header>
