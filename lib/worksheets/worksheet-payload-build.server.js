...
function worksheetDocumentLocaleAttrs(meta) {
  const def = resolveLocaleDefinition(meta?.interfaceLocale || meta?.instructionLocale || "en");
  const htmlLang = def.id === "ar-001" ? "ar" : def.intlLocale || def.id || "en";
  const htmlDir = def.direction || "ltr";
  ...
  const questions = rawQuestions
    .map((raw, i) =>
      toPrintableWorksheetQuestion(raw, {
        ...
        instructionLocale: opts.instructionLocale || meta.instructionLocale || "en",
        contentLocale: opts.contentLocale || meta.contentLocale || opts.instructionLocale || "en",
        interfaceLocale: opts.interfaceLocale || meta.interfaceLocale || "en",
...
function worksheetPayloadCopy(meta, key) {
  const locale = meta?.interfaceLocale || meta?.instructionLocale || meta?.contentLocale || "en";
  return globalBurnDownCopyForLocale(locale, "lib__worksheets__worksheet-payload-build.server", key);
}
...
export function worksheetPayloadToPreviewHtml(payload) {
  ...
  const { htmlLang, htmlDir } = worksheetDocumentLocaleAttrs(meta);
  ...
  const header = `
    <header class="worksheet-header worksheet-header-centered">
      <div class="worksheet-header-top">
        ...
        <h1 class="worksheet-title">${worksheetPayloadCopy(meta, "worksheet")}</h1>
      </div>
      <p class="worksheet-meta">${escapeWorksheetHtml(meta.subject)} · ${escapeWorksheetHtml(meta.grade)} · ${escapeWorksheetHtml(meta.topic)} · ${escapeWorksheetHtml(meta.level)}</p>
      <div class="worksheet-fields worksheet-fields-centered">
        <span>${worksheetPayloadCopy(meta, "name")}</span>
        <span>${worksheetPayloadCopy(meta, "date")}</span>
      </div>
    </header>`;
  ...
  const renderQuestionArticle = (q, index = 0) => {
      ...
      const options =
        q.optionsHe?.length
          ? `<div class="worksheet-options-section"><p class="worksheet-options-heading">${worksheetPayloadCopy(meta, "options")}</p><ol class="worksheet-options worksheet-options-grid">${q.optionsHe
              .map((o, i) => {
                ...
                const cls =
                  !useFraction && useLtr
                    ...
                    : "";
                return `<li class="worksheet-option-cell"><span class="worksheet-option-label">${["A", "B", "C", "D"][i] || i + 1}.</span><span class="worksheet-option-text${caseClass}">${useFraction ? inner : `<span${cls}>${inner}</span>`}</span></li>`;
              })
              .join("")}</ol></div>`
          : "";
      ...
      const remainderPrompt =
        ...
        /(?!)/.test(String(q.stemHe)) &&
        !/(?!)/.test(String(q.mathExpressionLtr));
      ...
      const remainderPromptHe = remainderPrompt
        ? `<p class="worksheet-remainder-prompt" dir="ltr">${escapeWorksheetHtml(q.stemHe)}</p>`
        : "";
      ...
      const answerLine =
        ...
        (useMathPrintPages |
          ...
          ? `<div class="worksheet-math-answer-line" aria-hidden="true"><span class="worksheet-math-answer-line-label">${worksheetPayloadCopy(meta, "answer")}</span><span class="worksheet-math-answer-line-blank"></span></div>`
          : "";
      return `
      <article class="${cardClass}" data-index="${q.displayIndex}">
        <h2 class="worksheet-question-title">${worksheetPayloadCopy(meta, "question")}<span class="worksheet-question-number">${q.displayIndex}</span></h2>
...
<head><meta charset="utf-8"><title>${escapeWorksheetHtml(meta.title)}</title></head>
<body class="worksheet-root${inkClass}" lang="${htmlLang}" dir="${htmlDir}">
${header}
...
export function answerKeyPayloadToPreviewHtml(payload) {
  ...
  const rows = answers
    .map((a) => {
      ...
      return `<li class="answer-key-item"><div class="answer-key-item-head">${worksheetPayloadCopy(meta, "answer_2")}<span class="worksheet-question-number">${a.displayIndex}</span></div>${bodyHtml}${
        a.explanationHe
  ...
  return `<!DOCTYPE html>
<html lang="${htmlLang}" dir="${htmlDir}">
<head><meta charset="utf-8"><title>${worksheetPayloadCopy(meta, "answer_key")} - ${escapeWorksheetHtml(meta.title)}</title></head>
<body class="worksheet-root answer-key-root" lang="${htmlLang}" dir="${htmlDir}">
<header class="worksheet-header worksheet-header-centered">
  <div class="worksheet-header-top">
    <div class="worksheet-brand-center">
      <img src="/images/coin.png" alt="LEO KIDS" width="56" height="56" class="worksheet-brand-logo" />
    ...
    <h1 class="worksheet-title">${worksheetPayloadCopy(meta, "answer_key")}</h1>
  </div>
  <p class="worksheet-meta">${escapeWorksheetHtml(meta.subject)} · ${escapeWorksheetHtml(meta.grade)} · ${escapeWorksheetHtml(meta.topic)} · ${escapeWorksheetHtml(meta.level)}</p>
</header>
