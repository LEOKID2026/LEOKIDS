...
export function worksheetPayloadToPreviewHtml(payload) {
  ...
  const renderQuestionArticle = (q, index = 0) => {
      ...
      <article class="${cardClass}" data-index="${q.displayIndex}">
        <h2 class="worksheet-question-title"><span class="worksheet-question-number">${q.displayIndex}</span> Question</h2>
...
export function answerKeyPayloadToPreviewHtml(payload) {
  ...
  const rows = answers
    .map((a) => {
      ...
      return `<li class="answer-key-item"><div class="answer-key-item-head"><span class="worksheet-question-number">${a.displayIndex}</span> Answer</div>${bodyHtml}${
        a.explanationHe
