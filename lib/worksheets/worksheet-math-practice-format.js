import { globalBurnDownCopy, globalBurnDownCopyForLocale } from "../../lib/i18n/global-burn-down-copy.js";
/**
...
/** Maps format id → burn-down pack key for UI labels. */
const PRACTICE_FORMAT_PACK_KEYS = {
  horizontal_add_sub: "horizontal_addition_and_subtraction",
  vertical_add_sub: "vertical_addition_and_subtraction",
  ...
  division_with_remainder: "basic_division",
  long_division_with_remainder: "long_division",
  fractions: "fractions",
  decimals: "decimals",
  percentages: "percentages",
  ratio_scale: "ratio_and_scale",
 ...
 * @returns {string}
 */
export function mathPracticeFormatLabelEn(formatId, locale = "en") {
  const packKey = PRACTICE_FORMAT_PACK_KEYS[formatId];
  if (!packKey) return String(formatId || "");
  return globalBurnDownCopyForLocale(locale, "lib__worksheets__worksheet-math-practice-format", packKey);
}
 ...
 * @returns {string}
 */
export function mathPracticeFormatTitleEn(formatId, topicKey, gradeKey, locale = "en") {
  const topic = String(topicKey || "");
  const fmt = (key) => globalBurnDownCopyForLocale(locale, "lib__worksheets__worksheet-math-practice-format", key);
  if (topic === "addition" && formatId === "horizontal_add_sub") return fmt("horizontal_addition");
  if (topic === "addition" && formatId === "vertical_add_sub") return fmt("vertical_addition");
  if (topic === "subtraction" && formatId === "horizontal_add_sub") return fmt("horizontal_subtraction");
  if (topic === "subtraction" && formatId === "vertical_add_sub") return fmt("vertical_subtraction");
  if (topic === "multiplication" && formatId === "basic_multiplication") return fmt("basic_multiplication");
  if (topic === "multiplication" && formatId === "long_multiplication") return fmt("long_multiplication");
  if (topic === "division" && formatId === "basic_division") return fmt("basic_division");
  if (topic === "division" && formatId === "long_division") return fmt("long_division");
  if (topic === "division_with_remainder" && formatId === "division_with_remainder") {
    return fmt("basic_division");
  }
  if (topic === "division_with_remainder" && formatId === "long_division_with_remainder") {
    return fmt("long_division");
  }
  if (topic === "scale" && formatId === "ratio_scale") {
    return globalBurnDownCopyForLocale(locale, "lib__worksheets__worksheet-meta-labels-en.server", "scale");
  }
  if (topic === "ratio" && formatId === "ratio_scale") {
    return globalBurnDownCopyForLocale(locale, "lib__worksheets__worksheet-meta-labels-en.server", "ratio");
  }
  if (topic === "fractions" && gradeKey === "g2") return fmt("unit_fractions_halves_quarters");
  return mathPracticeFormatLabelEn(formatId, locale);
}
