...
export function buildWorksheetRecommendationsFromReport(reportPayload, gradeKey) {
  ...
  for (const subjectId of CORE_SUBJECTS) {
    ...
    for (const [topicKey, topicAgg] of Object.entries(subjectBlock.topics)) {
      ...
      const topic = worksheetTopicLabel(subjectId, topicKey);
      const subject = worksheetSubjectLabel(subjectId);
      const level = worksheetPublicLevelLabelHe(levelKey);
      ...
      candidates.push({
        ...
        topic,
        level,
        reasonHe: buildReasonHe(subject, topic, answers, accuracyPct),
 ...
 * @param {number} accuracyPct
 */
function buildReasonHe(subject, topic, answers, accuracyPct) {
  const label = SUBJECT_LABEL_BY_ID.math === subject ? subject : subject;
  return `Practiced ${topic} in ${label} - ${answers} questions, accuracy ${accuracyPct}%`;
}
