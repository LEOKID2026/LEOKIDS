...
export function subjectLabel(subjectId) {
  return platformSubjectLabel(subjectId);
}

/** @deprecated admin companion — use subjectLabel */
...
export function resolveTopicLabelHe(subject, topicKey) {
  ...
  let label = topicBucketLabel(sid, topicKey);
  if (!label || label === "נושא זה" || label === baseKey || label === String(topicKey)) {
    label = topicBucketLabel(sid, baseKey);
  }
