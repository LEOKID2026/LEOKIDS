...
const SESSION_SLUG = "lib__teacher-portal__use-teacher-portal-session";
const sessionCopy = (key) => globalBurnDownCopy(SESSION_SLUG, key);
...
export function useTeacherPortalLoad({
  ...
  const [loadingHint, setLoadingHint] = useState(() => sessionCopy("verifying_connection"));
  ...
  runLoadRef.current = async ({ force = false, isManualRetry = false } = {}) => {
    ...
    if (isManualRetry && manualRetryCountRef.current >= MAX_MANUAL_RETRIES) {
      setPhase("error");
      setErrorMessage(sessionCopy("load_report_error_long"));
      return;
    }
    if (isManualRetry) {
    ...
    setPhase("loading");
    setLoadingHint(sessionCopy("verifying_connection"));
    setErrorMessage("");
    ...
    try {
      ...
      if (!session.ok) {
        if (session.code === "stale_session" || session.code === "session_timeout") {
          await redirectLogin(sessionCopy("session_expired"), isStaffCookie);
          return;
        }
      ...
      setLoadingHint(sessionCopy("loading_report_hint"));
      ...
      if (!validate?.(res.status, body)) {
        setPhase("error");
        setErrorMessage(sessionCopy("load_report_error"));
        return;
      }
      ...
      if (msg.includes("report_fetch_timeout")) {
        setPhase("error");
        setErrorMessage(sessionCopy("load_timeout"));
        return;
      }
