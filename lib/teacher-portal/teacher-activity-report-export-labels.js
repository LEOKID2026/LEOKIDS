...
import { resolveClassroomSkillLabelHe } from "../classroom-activities/classroom-skill-labels.js";
...
import { DEFAULT_CALENDAR_TIMEZONE, resolveCalendarTimeZone } from "../learning-supabase/locale-calendar.server.js";
...
const ISRAEL_TZ = DEFAULT_CALENDAR_TIMEZONE;

/**
 * @param {string|null|undefined} key
 */
export function activityExportsubjectLabel(key) {
  if (!key) return "";
  const k = String(key).trim().toLowerCase().replace(/-/g, "_");
  if (EXPORT_SUBJECT_LABEL_HE[k]) return EXPORT_SUBJECT_LABEL_HE[k];
  const fromGlobal = subjectLabel(k);
  return fromGlobal === "-" ? "" : fromGlobal;
}

/** @deprecated Alias for existing teacher export imports. */
export const activityExportSubjectLabelHe = activityExportsubjectLabel;
...
function replaceExportRawKeysInTitle(text, subject) {
  ...
  for (const sid of SUBJECT_ORDER) {
    const label = activityExportsubjectLabel(sid);
  ...
  if (subject) {
    const subjHe = activityExportsubjectLabel(subject);
    if (subjHe) {
...
/**
 * Format ISO timestamp for teacher-facing Excel (UTC default, or explicit IANA tz).
 * Output: DD/MM/YYYY HH:mm
 * @param {string|null|undefined} iso
 * @param {unknown} [explicitTimeZone]
 */
export function formatActivityExportDateTimeHe(iso, explicitTimeZone) {
  if (!iso) return "";
  const d = new Date(String(iso));
  if (Number.isNaN(d.getTime())) return "";

  const parts = new Intl.DateTimeFormat("en-GB", {
    timeZone: resolveCalendarTimeZone(explicitTimeZone ?? ISRAEL_TZ),
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
