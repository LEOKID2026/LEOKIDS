 ...
 * Full English parity with ./teacher-ui.js — export names kept compatible
...
import { getActiveBurnDownLocale } from "../i18n/global-burn-down-copy.js";
import { apiErrorMessageHe } from "../platform-ui/display-labels.js";

export { apiErrorMessageHe };

/** Active interface locale for locale-aware helper functions. */
let _teacherUiLocale = "en";

/**
 * Bind teacher-portal copy helpers to the active interface locale.
 * Call from the teacher dashboard component (mirrors bindSchoolUiLocale pattern).
 * @param {string|null|undefined} locale
 */
export function bindTeacherUiLocale(locale) {
  _teacherUiLocale = locale ? String(locale) : getActiveBurnDownLocale() || "en";
  refreshTeacherUiBindings();
}

/** Resolve the teacher JSON bundle for the active locale, with en fallback. */
function teacherRoot() {
  // Prefer explicit bindTeacherUiLocale; fall back to I18nProvider burn-down locale.
  const locale = _teacherUiLocale || getActiveBurnDownLocale() || "en";
  const bundles = loadLocaleBundles(locale);
  const t = bundles.teacher;
  return t && typeof t === "object" ? t : teacherEn;
}

/** Called on bind; refreshes exported let constants. */
function refreshTeacherUiBindings() {
  const t = teacherRoot();
  DASHBOARD_NO_CLASSES_TITLE = t.dashboard?.noClassesTitle ?? teacherEn.dashboard.noClassesTitle;
  DASHBOARD_NO_CLASSES_HINT = t.dashboard?.noClassesHint ?? teacherEn.dashboard.noClassesHint;
  DASHBOARD_CREATE_CLASS_LABEL = t.dashboard?.createClassLabel ?? teacherEn.dashboard.createClassLabel;
  DASHBOARD_CREATE_CLASS_BUTTON = t.dashboard?.createClassButton ?? teacherEn.dashboard.createClassButton;
  DASHBOARD_CREATE_CLASS_PLACEHOLDER = t.dashboard?.createClassPlaceholder ?? teacherEn.dashboard.createClassPlaceholder;
}
...
export function subjectLabel(subjectId) {
  ...
  const t = teacherRoot();
  const subjects = t.subjects || SUBJECT_LABEL_EN;
  const fallback = t.fallback || T.fallback;
  return subjects[key] || key || fallback.unknownSubject;
}
...
export function formatTeacherClassSuffixHe(classDisplayLabel) {
  ...
  if (!raw) return null;
  const fallback = (teacherRoot().fallback || T.fallback);
  if (/^Class(\s|$)/i.test(raw)) return raw;
  return fillTemplate(fallback.classSuffix, { label: raw });
}
...
export function formatTeacherAttentionStudentLineHe(studentName, classDisplayLabel) {
  const fallback = (teacherRoot().fallback || T.fallback);
  const name = String(studentName || "").trim() || fallback.defaultStudentName;
  const suffix = formatTeacherClassSuffixHe(classDisplayLabel);
  return suffix ? fillTemplate(fallback.attentionStudentLine, { name, suffix }) : name;
}
...
/** Live-bound dashboard constants — updated by bindTeacherUiLocale(). */
export let DASHBOARD_NO_CLASSES_TITLE = teacherEn.dashboard.noClassesTitle;
export let DASHBOARD_NO_CLASSES_HINT = teacherEn.dashboard.noClassesHint;
export let DASHBOARD_CREATE_CLASS_LABEL = teacherEn.dashboard.createClassLabel;
export let DASHBOARD_CREATE_CLASS_BUTTON = teacherEn.dashboard.createClassButton;
export let DASHBOARD_CREATE_CLASS_PLACEHOLDER = teacherEn.dashboard.createClassPlaceholder;
