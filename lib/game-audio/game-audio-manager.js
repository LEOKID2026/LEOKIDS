import { createStemPlaybackController, pickEnglishTtsVoice } from "../../utils/audio-playback-core.js";
import { sanitizeSpeechText } from "./sanitize-speech-text.js";
...
export function createGameAudioManager(manifestById, getSettings, onSettingsChange) {
  ...
  function playBrowserTts(text, locale, gen) {
    return new Promise((resolve, reject) => {
      ...
      const lang = locale || DEFAULT_SPEECH_LOCALE;
      u.lang = lang;
      u.volume = settings().voiceVolume;
      const voices = synth.getVoices();
      const voice = pickEnglishTtsVoice(voices, lang);
  ...
  async function playVoice(assetId, payload = {}, options = {}) {
    ...
    const locale = payload.locale || DEFAULT_SPEECH_LOCALE;
    ...
    try {
      if (payload.stem) {
        const stem = {
          ...payload.stem,
          locale: payload.stem.locale || locale,
          playback_kind: "tts",
        };
        const session = createStemPlaybackController(stem, {
          onEnded: finish,
        });
