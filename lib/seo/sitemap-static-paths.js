 ...
 * @param {string} origin
 * @param {string[]} paths
 * @param {{ localeIds?: string[] }} [opts]
 * Build sitemap URL list. Only pass status=enabled locales (ACTIVE_LOCALE_IDS).
 * Does not invent URLs for disabled/future languages.
 */
export function expandSitemapPathsForLocales(paths, localeIds = []) {
  const locales = (localeIds || []).filter(Boolean);
  if (!locales.length || (locales.length === 1 && locales[0] === "en")) {
    return paths.slice();
  }
  /** @type {string[]} */
  const out = [];
  for (const loc of locales) {
    for (const p of paths) {
      ...
      if (loc === "en") out.push(path);
      else out.push(`/${loc}${path === "/" ? "" : path}`);
    }
  }
  return out;
}
 ...
 * @param {{ localeIds?: string[] }} [opts]
 */
export function buildMultilingualSitemapXml(origin, paths, opts = {}) {
  const expanded = expandSitemapPathsForLocales(paths, opts.localeIds);
  return buildSitemapXml(origin, expanded);
}
