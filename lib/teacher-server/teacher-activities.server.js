...
import { decorateWeakSkillsForTeacherDisplay } from "../classroom-activities/classroom-skill-labels.js";
...
export function parseCreateActivityBody(body) {
  ...
  if (!title || title.length > 120) {
    return { ok: false, code: "validation_failed", message: "Please enter a title (up to 120 characters)" };
  }

  const classId = String(body.classId || "").trim();
  if (!isUuid(classId)) {
    return { ok: false, code: "validation_failed", message: "Invalid class ID" };
  }
  ...
  if (!isActivityPreviewSubjectSupported(subject)) {
    return {
      ...
      message: globalBurnDownCopy("lib__teacher-server__teacher-activities.server", "this_subject_is_not_supported_for_class_activities"),
    };
  }

  const topic = String(body.topic || "").trim();
  if (!topic || topic.length > 120) {
    return { ok: false, code: "validation_failed", message: "Please enter a topic (up to 120 characters)" };
  }

  const mode = normalizeActivityMode(body.mode);
  if (!mode) {
    return { ok: false, code: "validation_failed", message: "Invalid activity type" };
  }

  const questionSelection = normalizeQuestionSelection(body.questionSelection);
  if (!questionSelection) {
    return { ok: false, code: "validation_failed", message: "Invalid question selection mode" };
  }
  ...
  if (mode === "discussion" && (Math.floor(questionCount) < 1 || Math.floor(questionCount) > 5)) {
    return {
      ok: false,
      code: "validation_failed",
      message: "A discussion activity must include 1 to 5 questions",
    };
  }
  ...
  if (mode === "discussion") {
    ...
    if (!normalizedScope) {
      return { ok: false, code: "validation_failed", message: "Invalid audience scope" };
    }
    ...
    if (recipientScope === "selected_students") {
      ...
      if (!Array.isArray(rawIds) || !rawIds.length) {
        return {
          ok: false,
          code: "validation_failed",
          message: globalBurnDownCopy("lib__teacher-server__teacher-activities.server", "please_select_at_least_one_student"),
        };
      }
      ...
      if (!assignedStudentIds.length || assignedStudentIds.length !== rawIds.length) {
        return { ok: false, code: "validation_failed", message: "Invalid student ID" };
      }
    }
  }
