...
import { formatTopicLineHe, subjectLabel } from "../teacher-portal/teacher-ui.js";
import {
  ...
  try {
    ...
    if (latestSession?.subject) {
      latestSubjectLabel =
        ...
        subjectLabel(latestSession.subject);
    }
    ...
    if (!latestSubjectLabel && topicWrong.size > 0) {
      ...
      latestSubjectLabel =
        formatTopicLineHe(subject, topic) || subjectLabel(subject) || null;
    }
      ...
      if (!latestSubjectLabel && classroom.latestActivity?.subject) {
        if (isSubjectInPermittedScope(classroom.latestActivity.subject, permittedSubjects)) {
          latestSubjectLabel =
            ...
            subjectLabel(classroom.latestActivity.subject);
        }
      }
