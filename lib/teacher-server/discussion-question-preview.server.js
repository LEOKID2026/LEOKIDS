...
export async function buildDiscussionQuestionPreview(serviceRole, teacherId, input) {
  ...
  if (!topic || topic.length > 120) {
    return { ok: false, status: 400, code: "validation_failed", message: "Topic is required" };
  }
  ...
  try {
    ...
    return {
      ...
      message: err?.message || "Could not generate questions — try a different topic or difficulty",
    };
  }
}
