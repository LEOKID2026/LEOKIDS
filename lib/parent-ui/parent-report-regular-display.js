...
function rowNeedsProminentFinding(row) {
  ...
  if (lpd) {
    ...
    if ((lpd.practicedQuestions || 0) <= 2 && !/סדרות|sequences|series/i.test(String(row.label || ""))) return false;
  ...
  if (q <= 3 && /סדרות|sequences|series/i.test(String(row.label || ""))) return true;
  return false;
}
