...
export function buildRegularReportTopicStrengthLineHe(row, registeredGradeKey) {
  ...
  const q = Number(row.questions) || 0;
  const acc = Math.round(Number(row.accuracy) || 0);
  const line = `${topic}: practice looks stable - ${q} questions, ${acc}% accuracy.`;
...
function buildStructuredInsightFromTopicRows(report, topicRows) {
  ...
  for (const row of attention) {
    ...
    if (copy.homeAction && homeTips.length < 3) {
      homeTips.push(`In ${copy.title}: ${copy.homeAction}`);
    }
  }
  ...
  if (strengthTopics.length && focusTopics.length) {
    summary =
      "This report shows relatively stable practice, but there are a few topics worth paying attention to. " +
      `${strengthTopics[0]} looks more stable, while ${focusTopics.slice(0, 3).join(", ")} shows recurring mistakes.`;
  } else if (focusTopics.length) {
    summary = `This report has a few topics worth paying attention to: ${focusTopics.slice(0, 3).join(", ")}.`;
  } else if (strengthTopics.length) {
    summary = `This report shows relatively stable practice, mainly in ${strengthTopics[0]}.`;
  }
