...
import { formatParentReportGradeHe } from "../../utils/parent-report-language/parent-report-display-labels.js";
import {
...
export function formatRegularReportGradeCellHe(row) {
  ...
  return grade && grade !== reportPackCopy("lib__parent-ui__parent-report-regular-display", "not_available") ? grade : reportPackCopy("lib__parent-ui__parent-report-regular-display", "not_available");
}
...
function rowNeedsProminentFinding(row) {
  ...
  if (lpd) {
    ...
    if ((lpd.practicedQuestions || 0) <= 2 && !/(?!)/i.test(String(row.label || ""))) return false;
  ...
  if (q <= 3 && /(?!)/i.test(String(row.label || ""))) return true;
  return false;
}
...
export function buildRegularReportTopicStrengthLineHe(row, registeredGradeKey) {
  ...
  const q = Number(row.questions) || 0;
  const acc = Math.round(Number(row.accuracy) || 0);
  const line = reportPackCopy("lib__parent-ui__parent-report-regular-display", "topic_stable_line", {
    topic,
    q: String(q),
    acc: String(acc),
  });
...
function buildStructuredInsightFromTopicRows(report, topicRows) {
  ...
  const slug = "lib__parent-ui__parent-report-regular-display";
  const focusBullets = [];
  const homeTips = [];
  for (const row of attention) {
    ...
    if (copy.focusLine) {
      focusBullets.push(
        reportPackCopy(slug, "topic_colon_detail", { topic: copy.title, detail: copy.focusLine }),
      );
    } else if (copy.whatItMeans) {
      focusBullets.push(
        reportPackCopy(slug, "topic_colon_detail", { topic: copy.title, detail: copy.whatItMeans }),
      );
    } else {
      const line = buildRegularReportProminentFindingLineHe(row, registeredGradeKey);
    ...
    if (copy.homeAction && homeTips.length < 3) {
      homeTips.push(
        reportPackCopy(slug, "in_topic_home_tip", { topic: copy.title, tip: copy.homeAction }),
      );
    }
  }
  ...
  if (strengthTopics.length && focusTopics.length) {
    summary = reportPackCopy(slug, "summary_stable_with_focus", {
      strength: strengthTopics[0],
      focus: focusTopics.slice(0, 3).join(", "),
    });
  } else if (focusTopics.length) {
    summary = reportPackCopy(slug, "summary_focus_only", {
      focus: focusTopics.slice(0, 3).join(", "),
    });
  } else if (strengthTopics.length) {
    summary = reportPackCopy(slug, "summary_stable_only", {
      strength: strengthTopics[0],
    });
  }
...
export function rebuildSummaryFromFilteredReport(report) {
  ...
  const subjectStats = [math, geometry, english, science];
  ...
  totalTimeMinutes =
    math.timeMinutes + geometry.timeMinutes + english.timeMinutes + science.timeMinutes;
