...
export const PARENT_REPORT_WEEKLY_HOME_FALLBACK_HE =
  "Keep practicing a few short questions so we can identify a clearer direction.";
...
export function resolveParentReportWeeklyHomeActionHe(ctx) {
  ...
  if (Array.isArray(rows)) {
    for (const row of rows) {
      ...
      const legacy = sub.parentRecommendationsImprove?.[0]?.text;
      if (legacy) {
 ...
 * @param {{ parentActionHe?: string|null, parImp?: { text?: string }[] }} sub
 */
export function mergeParentReportHomeActionHe(sub) {
  ...
  const first = Array.isArray(legacy) ? legacy[0]?.text : null;
