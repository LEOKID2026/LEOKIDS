import { reportPackCopy } from "../reports/report-pack-copy.js";
/**
...
const DISPLAY_SLUG = "lib__parent-ui__learning-time-exclusive-breakdown-display";

function subjectLabel(key) {
  const k = String(key || "").trim().toLowerCase();
  const packKey = `subject_${k}`;
  const v = reportPackCopy(DISPLAY_SLUG, packKey);
  return v && v !== packKey ? v : key;
}
 ...
 *   bySubject: Array<{ subjectKey: string, subjectLabel: string, questionPracticeMinutes: number, bookReadingMinutes: number }>
 * }}
 */
...
export function formatLearningTimeDivisionLineHe(b) {
  ...
  return reportPackCopy(DISPLAY_SLUG, "learning_time_breakdown_line", {
    question: x,
    book: y,
    other: z,
  });
