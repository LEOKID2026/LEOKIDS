 ...
 * Global default narrative locale is English; Hebrew ratio checks apply only for he* locales.
 */
...
/** Global default narrative locale for English runtime. */
export const PARENT_REPORT_AI_DEFAULT_LOCALE = "en";

/**
 * Resolve narrative validation locale from options / report context.
 * @param {ParentReportAIValidateOptions} [options]
 * @returns {string}
 */
export function resolveParentReportAiNarrativeLocale(options = {}) {
  const fromOptions = typeof options.locale === "string" ? options.locale.trim() : "";
  if (fromOptions) return fromOptions;
  const ctx = options.narrativeReportContext;
  const fromCtx =
    ctx && typeof ctx === "object" && typeof ctx.locale === "string" ? ctx.locale.trim() : "";
  if (fromCtx) return fromCtx;
  return PARENT_REPORT_AI_DEFAULT_LOCALE;
}
 ...
 * @returns {boolean}
 */
function isHebrewNarrativeLocale(locale) {
  return /^he\b/i.test(String(locale || "").trim());
}
 ...
 * @property {string} [locale] — report / account locale (default `en`)
 * @property {number} [minHebrewRatio] — Hebrew-letter ratio required only for he* locales (default 0.48)
 ...
 * @property {import("../../utils/parent-narrative-safety/parent-narrative-safety-contract.js").ParentNarrativeReportContext & { locale?: string }} [narrativeReportContext]
 */
...
export function validateParentReportAIText(text, options = {}) {
  ...
  const locale = resolveParentReportAiNarrativeLocale(options);
  const hebrewLocale = isHebrewNarrativeLocale(locale);
  ...
  } else if (he / letters.length > 0.15) {
    // English/global runtime must not accept Hebrew-majority narrative copy.
    return { ok: false, reason: "unexpected_hebrew" };
  }
