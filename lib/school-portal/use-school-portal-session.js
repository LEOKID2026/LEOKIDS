...
import { apiErrorMessageHe, schoolAuthFetch, SCHOOL_PORTAL_LOAD_ERROR } from "./school-ui.js";
import {
  fetchSchoolJsonSWR,
  isSchoolCacheFresh,
  readSchoolCache,
...
export function useSchoolPortalLoad() {
  ...
  const load = useCallback(async ({ force = false } = {}) => {
    ...
    if (status === 403 && body?.error?.code === "not_school_portal_member") {
      setState("forbidden");
      setError("No school portal access");
      return;
    }

    if (status === 403 && body?.error?.code === "not_a_school_manager") {
      setState("forbidden");
      setError("No school manager access");
      return;
    }
    ...
    if (status !== 200) {
      if (!cachedSession?.data) {
        setState("error");
        setError(apiErrorMessageHe(body?.error, SCHOOL_PORTAL_LOAD_ERROR));
      }
      return;
    }
