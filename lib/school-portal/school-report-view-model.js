...
function studentRowFromGroupItem(s) {
  ...
  return {
    ...
    studentId: s.studentId,
    name: s.studentFullNameMasked || s.studentFullName || "Student",
    detail: answers > 0 ? `${answers} answers${acc ? ` · accuracy ${acc}` : ""}` : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_in_this_period"),
    learningStatusBadge,
  };
}
...
function buildFocusAreasFromWeaknessSource(weaknessSource, guidance, rosterById, subjectLabelFn) {
  ...
  for (const t of weaknessSource || []) {
    const subjectKey = String(t.subject || "");
    const topicLabel =
      t.topicLabelHe || t.headlineHe || resolveTopicLabelHe(subjectKey, t.topic) || null;
    if (!topicLabel) continue;

    const subjectLabel = subjectLabelFn(subjectKey);
    const headline =
      t.level === "subject"
        ? topicLabel
        : t.subtopicLabelHe
          ? `${topicLabel} - ${t.subtopicLabelHe}`
          : topicLabel;
    const line =
      subjectLabel && t.level !== "subject" ? `${subjectLabel} - ${headline}` : headline;
    ...
    const drilldownKey = `${subjectKey}::${normalizeTopicKeyForDrilldown(t.topic)}`;
...
function buildClassFocusSectionMeta({ focusAreas, guidance, report }) {
  ...
  return {
    title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "topics_to_strengthen"),
    preamble,
...
function studentInsightText(body) {
  ...
  if (strengths.length) {
    const s = strengths[0];
    const subj = schoolsubjectLabel(s.subject);
...
export function parseClassReportViewModel(body, cls, extras = {}) {
  ...
  const header = {
    ...
    subtitle: [
      schoolsubjectLabel(cls.subjectFocus || body?.class?.subjectFocus),
  ...
  const summaryCards = [
    { label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "students_in_class"), value: String(studentCount) },
    ...
    { label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "class_activities"), value: String(activityCount) },
  ...
  const classroomActs = (extras.recentClassroomActivities || []).map((a) => ({
    ...
    subject: schoolsubjectLabel(a.subject),
  ...
  const dailyActs = (body?.recentActivity?.daily || [])
    ...
    .map((d, i) => ({
      ...
      subject: schoolsubjectLabel(cls.subjectFocus),
      mode: globalBurnDownCopy("lib__school-portal__school-report-view-model", "practice"),
      status: safeNum(d.answers) > 0 ? "Active" : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data"),
  ...
  const inactiveStudents = rosterStudentsRaw
    .filter((s) => safeNum(s.summary?.totalAnswers) === 0)
    .map((s) => studentRowFromPayload(s, globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_in_this_period")));
  ...
  const distributionDrilldowns = {
    ...
    active: {
      title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "submitted_active"),
      items: activeStudents,
    },
    inactive: {
      title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_in_period"),
  ...
  if (studentCount > 0) {
    distribution.unshift(
      { tier: globalBurnDownCopy("lib__school-portal__school-report-view-model", "submitted_active"), count: submitted, drilldownKey: "active" },
      { tier: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_in_period"), count: notSubmitted, drilldownKey: "inactive" }
    );
  }
  ...
  const navigation = [
    {
      id: "activities",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "class_activities"),
      badge: activityItems.length ? `${activityItems.length} activities` : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_activities"),
    },
    {
      id: "students",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "students_in_class"),
      badge: rosterStudents.length ? `${rosterStudents.length} children` : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_children"),
    },
    {
      id: "focus",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "topics_to_strengthen"),
      badge: focusSection.navBadge,
    },
    {
      id: "attention",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "students_who_need_attention"),
      badge: attentionStudents.length ? `${attentionStudents.length} children` : "None",
    },
    {
      id: "distribution",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "student_distribution"),
      badge: distribution.some((d) => d.count > 0) ? "Details" : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data"),
    },
  ];
  ...
  return {
    ...
    sections: {
      activities: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "class_activities"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_class_activities_to_show_for_this_period"),
        items: activityItems,
      },
      students: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "students_in_class"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_children_are_registered_in_this_class"),
      ...
      distribution: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "student_distribution"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "not_enough_data_for_a_distribution_breakdown"),
        items: distribution.filter((d) => d.count > 0),
      },
      focus: focusSection,
      attention: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "students_who_need_attention"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_children_have_been_flagged_for_attention"),
...
export function parseStudentReportViewModel(body, student, ctx = {}) {
  ...
  const header = {
    ...
    subtitle: [
      ...
      ctx.schoolName || null,
      ctx.subjectFocus ? schoolsubjectLabel(ctx.subjectFocus) : null,
    ]
      .filter(Boolean)
      .join(" · "),
    chips: [],
  ...
  const summaryCards = [
    ...
    {
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "learning_status"),
      value:
            ...
            : safeNum(summary.accuracy) >= 75
              ...
              : safeNum(summary.accuracy) >= 65
                ...
                : safeNum(summary.accuracy) >= 50
                  ? "Needs support"
                  : globalBurnDownCopy("lib__school-portal__school-report-view-model", "needs_intervention"),
    },
  ];
  ...
  const subjectRows = SCHOOL_REPORT_SUBJECTS.map((sid) => {
    ...
    if (!answers && !sessions) {
      return { subject: schoolsubjectLabel(sid), status: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_yet"), detail: null };
    }
    return {
      subject: schoolsubjectLabel(sid),
  ...
  const recentFromDaily = (body?.dailyActivity || [])
    ...
    .map((d, i) => ({
      id: `d-${d.date || i}`,
      title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "practice_day"),
      subject: "-",
      mode: globalBurnDownCopy("lib__school-portal__school-report-view-model", "practice"),
      status: safeNum(d.answers) > 0 ? "Active" : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data"),
      date: formatDateHe(d.date),
  ...
  const recentFromMistakes = (body?.recentMistakes || []).slice(0, 5).map((m, i) => ({
    ...
    subject: schoolsubjectLabel(m.subject),
    mode: globalBurnDownCopy("lib__school-portal__school-report-view-model", "practice"),
    status: globalBurnDownCopy("lib__school-portal__school-report-view-model", "mistake"),
    ...
    guidance.version === "v2"
      ? dedupeTeacherRecommendationItems(guidance.recommendationUnits || [])
          ...
          .map((u) => {
            ...
            const subj = schoolsubjectLabel(u.subject);
      ...
      : (guidance.supportSuggestions || [])
          ...
          .map((s, i) => {
            ...
            const label =
              ...
              `Recommendation ${i + 1}`;
            if (!label || label === globalBurnDownCopy("lib__school-portal__school-report-view-model", "keep_monitoring_progress")) return null;
            return {
              label,
  ...
  const navigation = [
    {
      id: "subjects",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "breakdown_by_subject"),
      badge: subjectRows.some((r) => r.status !== globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_yet")) ? "Has data" : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data"),
    },
    {
      id: "activities",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "recent_activities"),
      badge: recentItems.length ? `${recentItems.length} records` : globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_activity"),
    },
    {
      id: "focus",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "topics_to_strengthen"),
    ...
    {
      id: "recommendations",
      label: globalBurnDownCopy("lib__school-portal__school-report-view-model", "follow_up_recommendations"),
  ...
  return {
    ...
    sections: {
      subjects: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "breakdown_by_subject"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_data_by_subject"),
      ...
      activities: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "recent_activities"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_learning_activity_recently"),
        items: recentItems,
      },
      focus: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "topics_to_strengthen"),
      ...
      recommendations: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "follow_up_recommendations"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_further_recommendations_at_this_time"),
...
function physicalClassInsightText(body) {
  ...
  if (tier && blocks.length) {
    ...
    const subjHe =
      worst?.subjectLabel || schoolsubjectLabel(worst?.subjectFocus || "");
...
export function parsePhysicalClassReportViewModel(body, ctx = {}) {
  ...
  const header = {
    title: physName,
    subtitle: globalBurnDownCopy("lib__school-portal__school-report-view-model", "general_class_report_all_subjects"),
    chips: [
  ...
  const subjectItems = subjectBreakdown.map((row) => {
    ...
    return {
      ...
      label: row.subjectLabel || schoolsubjectLabel(row.subjectFocus),
      detail: [
  ...
  const activityItems = recentActivities.map((a) => ({
    ...
    subject: a.subjectLabel || schoolsubjectLabel(a.subjectFocus || a.subject),
  ...
  return {
    ...
    sections: {
      subjects: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "breakdown_by_subject"),
        empty: globalBurnDownCopy("lib__school-portal__school-report-view-model", "no_subjects_to_display"),
        items: subjectItems,
      },
      activities: {
        title: globalBurnDownCopy("lib__school-portal__school-report-view-model", "recent_activities"),
