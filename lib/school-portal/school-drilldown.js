...
import { subjectLabel } from "../platform-ui/display-labels.js";
...
/** Stable grade IDs sent to the server — never localize these values. */
export const SCHOOL_GRADE_LEVELS = Object.freeze(["1", "2", "3", "4", "5", "6"]);

/**
 * English fallback labels. UI should prefer {@link getSchoolGradeOptions} with `t`.
 * @type {ReadonlyArray<{ level: string, label: string }>}
 */
export const SCHOOL_GRADE_OPTIONS = Object.freeze(
  SCHOOL_GRADE_LEVELS.map((level) => ({ level, label: `Grade ${level}` }))
);

/**
 * Locale-aware school-year options via common.grade1–grade6.
 * Values (`level`) stay numeric grade IDs; only labels localize.
 * @param {(key: string) => string} [t]
 * @returns {{ level: string, label: string }[]}
 */
export function getSchoolGradeOptions(t) {
  if (typeof t !== "function") {
    return SCHOOL_GRADE_OPTIONS.map((g) => ({ ...g }));
  }
  return SCHOOL_GRADE_LEVELS.map((level) => {
    const key = `common.grade${level}`;
    const label = t(key);
    return {
      level,
      label: label && label !== key ? label : `Grade ${level}`,
    };
  });
}
 ...
 * @param {(key: string) => string} [t] locale translator (`common.gradeN`)
 */
export function schoolGradeLabelHe(gradeLevel, t) {
  ...
  return SCHOOL_GRADE_OPTIONS.find((g) => g.level === level)?.label || `Grade ${level}`;
}
...
export function groupPhysicalClassesForGrade(classes, gradeLevel) {
  ...
  return [...map.values()].sort((a, b) => {
    ...
    return sectionA - sectionB || a.name.localeCompare(b.name, "en");
  });
}
...
export function physicalClassSubjectLabelsHe(subjectClasses) {
  return sortSubjectClasses(subjectClasses || [])
    .map((cls) => subjectLabel(String(cls.subjectFocus || "").trim()))
    .filter(Boolean);
}
