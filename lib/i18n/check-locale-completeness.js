...
export function checkLocaleCompleteness(localeId, opts = {}) {
  ...
  if (id === "en" || isPseudo) {
    ...
    const sci = computeScienceLocalizationCoverage(undefined, SCIENCE_ES_419_OVERLAY);
    findings.push({
      id: "science_overlay",
      status: sci.contractComplete ? "ok" : "missing",
      detail: `questions=${sci.totalQuestions} overlay=${sci.overlayCoveragePct}% contractComplete=${sci.contractComplete} incomplete=${sci.incompleteOverlayCount}`,
    ...
    const sci = computeScienceLocalizationCoverage(undefined, SCIENCE_PT_BR_OVERLAY);
    findings.push({
      id: "science_overlay",
      status: sci.contractComplete ? "ok" : "missing",
      detail: `questions=${sci.totalQuestions} overlay=${sci.overlayCoveragePct}% contractComplete=${sci.contractComplete} incomplete=${sci.incompleteOverlayCount}`,
    ...
    const sci = computeScienceLocalizationCoverage(undefined, SCIENCE_PT_PT_OVERLAY);
    findings.push({
      id: "science_overlay",
      status: sci.contractComplete ? "ok" : "missing",
      detail: `questions=${sci.totalQuestions} overlay=${sci.overlayCoveragePct}% contractComplete=${sci.contractComplete} incomplete=${sci.incompleteOverlayCount}`,
  ...
  // question stems: en templates exist; es-419 / pt-BR / pt-PT have native math/geometry rebuilders
  findings.push({
    id: "question_stems",
    status:
      id === "en" || isPseudo || id === "es-419" || id === "pt-BR" || id === "pt-PT"
        ? "ok"
        : "missing",
    detail:
      id === "en" || isPseudo
        ...
        : id === "es-419"
          ? "Params-based es-419 math/geometry stem rebuilders (utils/learning-content-es419)"
          : id === "pt-BR"
            ? "Params-based pt-BR math/geometry stem rebuilders (utils/learning-content-pt-BR)"
            : id === "pt-PT"
              ? "Params-based pt-PT math/geometry stem rebuilders (utils/learning-content-pt-PT)"
  ...
  findings.push({
    ...
    detail:
      id === "en" || isPseudo
        ...
        : id === "es-419"
          ? "Writing packs resolve via word-packs.locale (es-419) + writing sentence cues"
          : id === "pt-BR"
            ? "Writing packs resolve via word-packs.locale (pt-BR) + writing sentence cues"
            : id === "pt-PT"
              ? "Writing packs resolve via word-packs.locale (pt-PT) + writing sentence cues"
