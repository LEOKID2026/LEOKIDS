...
} from "../admin-portal/admin-analytics-labels.js";
 ...
 * @param {number} n
 */
function formatCoinAmountLabel(n) {
...
export async function buildStudentSupportTimeline(supabase, studentId, opts = {}) {
  ...
  for (const row of sessionsRes.data || []) {
    ...
    const subject = formatAnalyticsSubjectHe(row.subject) || "";
    const minutes = durationMinutesHe(row.duration_seconds);
    const parts = [`  ${subject}`];
    if (totalQuestions != null) parts.push(`${totalQuestions} `);
    if (accuracy != null) parts.push(`${accuracy}%`);
    if (minutes != null) parts.push(`${minutes} `);

    const lineHe = parts.join(" - ");
    const cardParts = [` ${subject}`];
    if (totalQuestions != null) cardParts.push(`${totalQuestions} `);
    if (accuracy != null) cardParts.push(`${accuracy}%`);
    if (minutes != null) cardParts.push(`${minutes} `);
  ...
  for (const row of coinsRes.data || []) {
    ...
    if (row.reason === ADMIN_MANUAL_COIN_REASON || meta.manual_admin_action === true) {
      ...
      lineHe = ` ${formatCoinAmountLabel(amount)}   - : ${cat}`;
    } else if (direction === "spend") {
      lineHe = ` ${formatCoinAmountLabel(amount)} `;
  ...
  for (const row of parentStatusRes.data || []) {
    ...
    const subject = formatAnalyticsSubjectHe(activity?.subject) || "";
    const answers = Number(row.answers_count) || 0;
    const score = row.score_pct != null ? Math.round(Number(row.score_pct)) : null;
    let lineHe = "  ";
    if (activity?.title) {
      lineHe = ` ${String(activity.title).slice(0, 40)}`;
    }
    const detailParts = [subject];
    if (answers > 0) detailParts.push(`${answers} `);
