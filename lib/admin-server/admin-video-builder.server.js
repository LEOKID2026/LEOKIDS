...
export const VB_FFMPEG_UNAVAILABLE_HE =
  "ffmpeg      . / ffmpeg /  -PATH   MP4.";

/** @param {unknown} raw */
export function sanitizeVideoProjectName(raw) {
  const name = String(raw ?? "").trim();
  if (!name) return { ok: false, code: "missing_name", message: "Video not found" };
  if (name.length > 120) return { ok: false, code: "name_too_long", message: "screen not" };
  return { ok: true, name };
}
...
export function parseAspectRatio(raw) {
  ...
  if (!VB_ASPECT_RATIOS[ratio]) {
    return { ok: false, code: "invalid_aspect_ratio", message: "not" };
  }
...
function parseScene(raw, index) {
  if (!raw || typeof raw !== "object") {
    return { ok: false, code: "invalid_scene", message: ` ${index + 1}  ` };
  }
  ...
  if (!Number.isFinite(durationSec) || durationSec < 1 || durationSec > 120) {
    return { ok: false, code: "invalid_duration", message: `  ${index + 1}    1 -120 ` };
  }
  const bgType = String(s.bgType ?? "light");
  if (!VB_BACKGROUND_IDS.includes(bgType)) {
    return { ok: false, code: "invalid_bg", message: `   ${index + 1}  ` };
  }
  const animation = String(s.animation ?? "none");
  if (!VB_ANIMATION_IDS.includes(animation)) {
    return { ok: false, code: "invalid_animation", message: `  ${index + 1}  ` };
  }
  ...
  if (!VB_TEXT_SIZE_IDS.includes(titleSize) || !VB_TEXT_SIZE_IDS.includes(subtitleSize)) {
    return { ok: false, code: "invalid_text_size", message: `   ${index + 1}  ` };
  }
  if (!VB_TEXT_ALIGN_IDS.includes(textAlign)) {
    return { ok: false, code: "invalid_text_align", message: `   ${index + 1}  ` };
  }
  if (!VB_TEXT_SHADOW_IDS.includes(textShadow)) {
    return { ok: false, code: "invalid_text_shadow", message: `   ${index + 1}  ` };
  }
  if (!VB_MEDIA_OVERLAY_IDS.includes(mediaOverlay)) {
    return { ok: false, code: "invalid_overlay", message: `   ${index + 1}  ` };
  }
  ...
  if (!VB_TRANSITION_IDS.includes(transitionOut)) {
    return { ok: false, code: "invalid_transition", message: `  ${index + 1}  ` };
  }
  if (!VB_TEXT_BG_IDS.includes(textBg)) {
    return { ok: false, code: "invalid_text_bg", message: `   ${index + 1}  ` };
  }
  if (!VB_FONT_IDS.includes(fontFamily)) {
    return { ok: false, code: "invalid_font", message: `  ${index + 1}  ` };
  }
  if (!VB_MEDIA_POSITION_IDS.includes(mediaPosition) || !VB_MEDIA_SCALE_IDS.includes(mediaScale)) {
    return { ok: false, code: "invalid_media_layout", message: `   ${index + 1}  ` };
  }
  if (!VB_MEDIA_FIT_IDS.includes(mediaFit)) {
    return { ok: false, code: "invalid_media_fit", message: `   ${index + 1}  ` };
  }
...
export function parseVideoProjectBody(body) {
  if (!body || typeof body !== "object") {
    return { ok: false, code: "invalid_body", message: "not" };
  }
...
export async function getVideoProject(id) {
  ...
  if (!existsSync(filePath)) return { ok: false, code: "not_found", message: "Video not found" };
  const project = await readJsonFile(filePath, null);
  if (!project) return { ok: false, code: "not_found", message: "Video not found" };
...
export async function saveMediaAsset(buffer, contentType, originalFilename) {
  await ensureVideoBuilderDirs();
  if (!buffer?.length) {
    return { ok: false, code: "missing_file", message: "New video" };
  }
  if (buffer.length > VB_MAX_UPLOAD_BYTES) {
    return { ok: false, code: "file_too_large", message: "   ( 50MB)" };
  }
  const mimeType = String(contentType || "").toLowerCase().split(";")[0].trim();
  if (!VB_ALLOWED_MIME.has(mimeType)) {
    return { ok: false, code: "invalid_type", message: "not" };
  }
  const id = randomUUID();
...
export async function deleteMediaAsset(assetId) {
  ...
  if (!id) {
    return { ok: false, code: "missing_id", message: "not" };
  }
  const assets = await listMediaAssets();
  const asset = assets.find((a) => a.id === id);
  if (!asset) {
    return { ok: false, code: "not_found", message: "to" };
  }
...
export async function parseMediaUpload(req) {
  ...
  if (!contentType.includes("multipart/form-data")) {
    return { ok: false, status: 400, code: "validation_failed", message: " multipart/form-data" };
  }
  const chunks = [];
  ...
  if (!boundary) {
    return { ok: false, status: 400, code: "validation_failed", message: "boundary " };
  }
  const parts = parseMultipartBuffer(body, boundary);
  const filePart = parts.find((p) => p.name === "file" || p.name === "media");
  if (!filePart?.data?.length) {
    return { ok: false, status: 400, code: "validation_failed", message: "  ( file)" };
  }
  return {
    ok: true,
...
export async function exportVideoProjectMp4(project) {
  ...
  if (!scenes.length) {
    return { ok: false, code: "no_scenes", message: "to" };
  }
...
export function createEmptyProjectPayload(name = "New video") {
  return {
    name,
    aspectRatio: "16:9",
    scenes: [defaultScene()],
