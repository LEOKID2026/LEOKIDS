...
export function calculateSoloGameDiamonds(diamondRules, metrics) {
  ...
  if (rules.onlyOnWin === true && !didWin) {
    return { diamonds: 0, breakdownHe: "No diamonds — win required" };
  }
  ...
  if (mode === "in_game_collect") {
    ...
    if (total > 0) breakdown.push(`In game: ${total}`);
  } else {
    const fixed = Math.floor(Number(rules.fixedAmount || 0));
    if (fixed > 0) {
      total += fixed;
      breakdown.push(`Base: ${fixed}`);
    }
    for (const tier of Array.isArray(rules.tiers) ? rules.tiers : []) {
      ...
      if (tierAmount > 0 && score >= minScore) {
        total += tierAmount;
        breakdown.push(`Score ${minScore}+: +${tierAmount}`);
      }
    }
  }
  ...
  return {
    diamonds: total,
    breakdownHe: breakdown.length ? breakdown.join(" · ") : total > 0 ? "Diamonds" : "-",
  };
}
