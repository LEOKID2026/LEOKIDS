...
function mapDailyMissionRow(row) {
  return {
    ...
    text: String(row.mission_type === "questions" ? ("Answer " + Math.floor(Number(row.target_value) || 0) + " questions today") : row.mission_type === "minutes" ? ("Learn for " + Math.floor(Number(row.target_value) || 0) + " minutes today") : row.mission_type === "subjects" ? (Math.floor(Number(row.target_value) || 0) <= 1 ? "Practice at least one subject" : ("Practice " + Math.floor(Number(row.target_value) || 0) + " different subjects")) : String(row.mission_key || "")),
...
function mapMonthlyTierRow(row) {
  return {
    ...
    label: (Math.floor(Number(row.reward_coins) || 0) ? Math.floor(Number(row.reward_coins) || 0).toLocaleString("en-US") + " coins" : String(row.minutes_threshold || "")),
    displayOrder: Math.floor(Number(row.display_order) || 0),
  };
}
/**
...
export async function buildStudentEconomyConfigPayload(supabase) {
  ...
  const monthlyTiers = snap.monthlyTiers.map((t) => ({
    minutes: t.minutes,
    coins: t.coins,
    label: `${t.coins.toLocaleString("en-US")} coins`,
  }));
  return {
    monthlyTiers,
