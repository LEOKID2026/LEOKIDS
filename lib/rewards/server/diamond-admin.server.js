...
export function validateDiamondRules(raw) {
  ...
  if (value.maxPerSession > 1000) {
    return { ok: false, message: "Max diamonds per game must be 1000 or less" };
  }
  if (value.fixedAmount > 1000) {
    return { ok: false, message: "Fixed diamond amount must be 1000 or less" };
  }
  if (value.inGameCollectMultiplier > 100) {
    return { ok: false, message: "In-game collect multiplier must be 100 or less" };
  }
  for (const tier of value.tiers) {
    if (tier.amount > 1000) {
      return { ok: false, message: "Tier diamond amount must be 1000 or less" };
    }
  }
...
export function validateSurpriseBoxDiamondRewards(raw) {
  ...
  if (rewards.length === 0) {
    return { ok: false, message: "At least one diamond prize is required (amount + weight)" };
  }
  if (rewards.length > 20) {
    return { ok: false, message: "Up to 20 diamond prizes" };
  }
...
export async function updateSoloDiamondRulesAdmin(supabase, adminUserId, gameKey, diamondRulesRaw) {
  ...
  if (!SOLO_GAME_KEYS.includes(key)) {
    return { ok: false, code: "invalid_game_key", message: "Invalid solo game" };
  }

  const validated = validateDiamondRules(diamondRulesRaw);
  if (!validated.ok) {
    return { ok: false, code: "invalid_rules", message: validated.message };
  }
  ...
  if (!existing?.game_key) {
    return { ok: false, code: "not_found", message: "Game rules not found" };
  }
...
export async function updateSurpriseBoxDiamondRewardsAdmin(supabase, adminUserId, rewardsRaw) {
  ...
  if (!validated.ok) {
    return { ok: false, code: "invalid_rewards", message: validated.message };
  }
