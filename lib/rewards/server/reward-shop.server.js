...
export async function purchaseShopCard(supabase, studentId, cardId) {
  ...
  if (card.card_type === "achievement" || !card.can_be_purchased) {
    return { ok: false, code: "not_purchasable", message: "This card can't be purchased" };
  }
  ...
  const { data: owned } = await supabase
    ...
    .maybeSingle();
  if (owned?.owned) {
    return { ok: false, code: "already_owned", message: "Already in your collection" };
  }

  const price = await resolveCardPrice(supabase, card);
  const balance = await getStudentCoinBalance(supabase, studentId);
  if (balance < price) {
    return { ok: false, code: "insufficient_coins", message: "You don't have enough coins for this card" };
  }
