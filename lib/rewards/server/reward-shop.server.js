 ...
 */
function globalCardResponseFields(card, contentLocale = "en") {
  if (isIsraelOnlyRewardCardKey(card.card_key)) {
    throw new Error(`israel_only_card_in_global_shop:${card.card_key}`);
  }
  const display = resolveGlobalCardCopy({
    productContext: "global",
    contentLocale,
    card,
    series: card.reward_card_series,
 ...
 * @param {string} cardId
 */
export async function purchaseShopCard(supabase, studentId, cardId, contentLocale = "en") {
  ...
  return {
    ok: true,
    card: {
      id: grant.card?.id || card.id,
      card_key: grant.card?.card_key || card.card_key,
      cardKey: grant.card?.card_key || card.card_key,
      ...globalCardResponseFields(card, contentLocale),
 ...
 */
export async function sellDuplicateShopCard(
  supabase,
  studentId,
  cardId,
  idempotencyKey,
 ...
 */
export async function getShopListing(supabase, studentId, balance, contentLocale = "en") {
  ...
  for (const card of cards || []) {
    ...
    if (isIsraelOnlyRewardCardKey(card.card_key)) continue;
