...
function mapCardForChild(card, series) {
  const seriesObj =
    series && typeof series === "object"
      ? series
      : card?.reward_card_series || (series ? { name_he: series } : null);
  const display = resolveGlobalRewardCardDisplay({
    cardKey: card.card_key,
    seriesSlug: seriesObj?.slug,
    nameHe: card.name_he,
    descriptionHe: card.description_he || "",
    seriesNameHe: seriesObj?.name_he || (typeof series === "string" ? series : ""),
    rarity: card.rarity,
...
export async function getStudentCollection(supabase, studentId) {
  ...
  return (owned || [])
    ...
    .map((row) => {
      ...
      return {
        ...mapCardForChild(card, card?.reward_card_series),
...
function sortCatalogCardsForDisplay(cards) {
  ...
  events.sort((a, b) => (a.nameHe || "").localeCompare(b.nameHe || "", "en"));
  return [...rest, ...events];
}
...
export async function getStudentCardsCatalogView(supabase, studentId) {
  ...
  const catalog = sortCatalogCardsForDisplay(
    allCards
      .map((card) => {
        const mapped = mapCardForChild(card, card.reward_card_series);
        const ownedRow = ownedMap.get(card.id);
        const isOwned = ownedIds.has(card.id);
...
export async function getStudentCardsSeriesView(supabase, studentId) {
  ...
  for (const series of seriesRows.data || []) {
    ...
    const mappedSeries = resolveGlobalRewardCardDisplay({
      seriesSlug: series.slug,
      seriesNameHe: series.name_he,
    });
    const seriesName = mappedSeries.seriesName;
    const cards = [];
    for (const card of inSeries) {
      ...
      const mapped = mapCardForChild(card, series);
      cards.push({
        cardId: card.id,
        cardKey: card.card_key,
        id: card.id,
        ...
        seriesNameHe: mapped.seriesNameHe,
        owned,
        isLocked: !owned,
...
export async function getStudentCardsView(supabase, studentId) {
  ...
  for (const card of allCards) {
    ...
    if (card.card_type === "achievement" || rules.length > 0) {
      locked.push({
        ...mapped,
        lockMessageHe: lockMeta?.lockMessageHe || rewardUiCopy("fallback", "keepLearning"),
      ...
      locked.push({
        ...
        lockMessageHe:
          ...
          (card.card_type === "event"
            ? rewardUiCopy("fallback", "notAvailableNow")
            : rewardUiCopy("fallback", "availableInShop")),
        requirementHe: lockMeta?.requirementHe || null,
        progressHe: lockMeta?.progressHe || null,
      });
    } else if (!isOwned) {
      locked.push({
        ...mapped,
        lockMessageHe: lockMeta?.lockMessageHe || rewardUiCopy("fallback", "notAvailableNow"),
        requirementHe: lockMeta?.requirementHe || null,
        progressHe: lockMeta?.progressHe || null,
      });
    }
...
export async function getDemoCardsCatalogView(supabase, gradeLevel) {
  ...
  const { getLockMeta } = loadDemoLockMetaContext(allCards, rulesByCard, ownedIds, studentGradeBand);
  ...
  const catalog = sortCatalogCardsForDisplay(
    allCards
      .map((card) => {
        ...
        return {
          ...mapped,
          requirementHe: lockMeta?.requirementHe || "",
          lockMessageHe: lockMeta?.lockMessageHe || "",
          progressHe: lockMeta?.progressHe || "",
          progressCurrent: lockMeta?.progressCurrent ?? null,
          progressTarget: lockMeta?.progressTarget ?? null,
...
export async function getDemoCardsSeriesView(supabase, gradeLevel) {
  ...
  for (const series of seriesRows.data || []) {
    ...
    seriesProgress.push({
      seriesId: series.id,
      seriesSlug: series.slug,
      seriesNameHe: mapCardForChild({ card_key: series.slug }, series).seriesNameHe || series.slug,
      totalCards: cards.length,
      ownedCards: cards.filter((c) => c.owned).length,
      cards,
    });
  }
