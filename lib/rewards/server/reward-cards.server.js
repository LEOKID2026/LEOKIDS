...
export async function getStudentCardsView(supabase, studentId) {
  ...
  for (const card of allCards) {
    ...
    if (card.card_type === "achievement" || rules.length > 0) {
      locked.push({
        ...mapped,
        lockMessageHe: lockMeta?.lockMessageHe || "Keep learning to unlock this card",
      ...
      locked.push({
        ...
        lockMessageHe:
          ...
          (card.card_type === "event" ? "Not available right now" : "Available in the shop"),
        requirementHe: lockMeta?.requirementHe || null,
        progressHe: lockMeta?.progressHe || null,
      });
    } else if (!isOwned) {
      locked.push({
        ...mapped,
        lockMessageHe: lockMeta?.lockMessageHe || "Not available right now",
        requirementHe: lockMeta?.requirementHe || null,
        progressHe: lockMeta?.progressHe || null,
      });
    }
