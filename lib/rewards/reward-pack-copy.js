...
export function resolveRewardCardEntry(cardKey, contentLocale) {
  ...
  if (!key) return null;
  // Lazy import avoided: unsupported keys are filtered in global display.
  // Also block active catalog resolution so overlays cannot expose Israeli titles.
  // GLOBAL catalogs must not resolve Israel-only keys (holidays / hebrew / social-studies).
  if (isIsraelOnlyRewardCardKey(key)) {
    return null;
  }
