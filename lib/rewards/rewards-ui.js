...
import { formatLocaleNumber } from "../i18n/format-locale.js";

/**
 * Child-facing reward UI labels — resolved from rewards content packs via locale catalog.
 */
...
export function formatRarityLabel(rarity, contentLocale) {
  const copy = ui(contentLocale);
  return copy("rarity", rarity) || copy("rarity", "regular");
}

export function formatCardTypeLabel(cardType, contentLocale) {
  const copy = ui(contentLocale);
  return copy("cardType", cardType) || copy("cardType", "shop");
}

export function formatCardSourceLabel(source, contentLocale) {
  const copy = ui(contentLocale);
  return copy("cardSource", source) || source || "";
}

export function formatCoinAmountLabel(amount, contentLocale) {
  const n = Math.floor(Number(amount) || 0);
  const copy = ui(contentLocale);
  return copy("shop", "coinsLabel", {
    amount: formatLocaleNumber(n, contentLocale || "en"),
  });
}

export function formatCoinAmountNumber(amount, contentLocale = "en") {
  const n = Math.floor(Number(amount) || 0);
  return formatLocaleNumber(n, contentLocale);
}

export function shopCardAlreadyOwnedLabel(contentLocale) {
  return ui(contentLocale)("shop", "alreadyOwned");
}

export function shopCardSellDuplicateLabel(contentLocale) {
  return ui(contentLocale)("shop", "sellDuplicate");
}

export function catalogCardOwnedLabel(contentLocale) {
