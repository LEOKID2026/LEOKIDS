...
function validateSoftcut(value) {
  if (!Array.isArray(value) || !value.length) {
    return { ok: false, code: "invalid_softcut", message: "softcut" };
  }
...
function validateStageBlocks(value) {
  if (!Array.isArray(value) || !value.length) {
    return { ok: false, code: "invalid_stage_blocks", message: "stage_blocks" };
  }
...
export function validateLeoMinersSettingsPatch(patch, current = {}) {
  if (!patch || typeof patch !== "object" || Array.isArray(patch)) {
    return { ok: false, code: "invalid_settings", message: "settings" };
  }
  ...
  for (const [key, raw] of Object.entries(patch)) {
    if (!(key in LEO_MINERS_DEFAULT_SETTINGS) && key !== "softcut" && key !== "stage_blocks") {
      return { ok: false, code: "unknown_setting_key", message: `  : ${key}` };
    }

    if (BOOLEAN_KEYS.includes(key)) {
      if (typeof raw !== "boolean") {
        return { ok: false, code: "invalid_boolean", message: `${key}   boolean` };
      }
      merged[key] = raw;
      continue;
    }
    ...
    if (key === "coins_rounding") {
      ...
      if (!COINS_ROUNDING_VALUES.includes(val)) {
        return { ok: false, code: "invalid_coins_rounding", message: "coins_rounding" };
      }
      merged[key] = val;
      continue;
    }
    ...
    if (NON_NEGATIVE_NUMBER_KEYS.includes(key)) {
      ...
      if (!Number.isFinite(num) || num < 0) {
        return { ok: false, code: "invalid_number", message: `${key}     ` };
      }
      merged[key] = num;
      continue;
    }
