...
import { schoolsubjectLabel } from "../school-portal/school-ui.js";
...
async function buildSchoolPhysicalClassReportSummaryPayload(input) {
  ...
  const subjectBreakdown = subjectRows.map((row) => ({
    ...
    subjectLabel: schoolsubjectLabel(row.subjectFocus),
...
function buildPhysicalSubjectGuidanceBlock(studentPayloads, subjectFocus, row) {
  ...
  try {
    ...
    return {
      subjectFocus,
      subjectLabel: schoolsubjectLabel(subjectFocus),
      classId: row.classId,
      teacherId: row.teacherId,
      teacherName: row.teacherName || null,
  ...
  return data
    ...
    .map((row) => {
      ...
      return {
        ...
        subjectLabel: schoolsubjectLabel(meta?.subjectFocus || row.subject),
