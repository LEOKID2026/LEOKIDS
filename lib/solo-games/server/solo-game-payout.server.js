...
export function calculateSoloGameCoins(gameKey, difficulty, metrics, rules) {
  ...
  if (isArcadeGame(gameKey)) {
    ...
    if (scoreUnits > 0) breakdown.push(`Score: +${scoreUnits * perUnit}`);
    if (levelBonus > 0) breakdown.push(`Level ${levelReached}: +${levelBonus}`);
    ...
    if (didWin) {
      ...
      breakdown.push(`Win (${difficultyLabelHe(diff)}): ${winBase}`);
      const extra = Math.floor(score / scoreDiv);
      if (extra > 0) breakdown.push(`Score bonus: +${extra}`);
      ...
      breakdown.push(`Stars (${stars}): ${stars * starRate}`);
      ...
      breakdown.push(`Blocks (${placed}): ${placed * blockRate}`);
      const extra = Math.floor(score / scoreDiv);
      if (extra > 0) breakdown.push(`Score bonus: +${extra}`);
    } else {
      coins = Math.floor(score / scoreDiv);
      breakdown.push(`Progress: +${coins}`);
    }
    ...
    if (didWin) {
      ...
      breakdown.push(`Win (${difficultyLabelHe(diff)}): ${winBase}`);
      if (mistakeCost > 0) breakdown.push(`Mistakes (${mistakes}): -${mistakeCost}`);
      if (timeBonus > 0) breakdown.push(`Time left: +${timeBonus}`);
    } else if (gameKey === "memory") {
      const pairs = clampInt(metrics?.pairsMatched, 0, 50);
      coins = pairs * pairRate;
      breakdown.push(`Pairs (${pairs}): ${pairs * pairRate}`);
    } else {
...
export function validateSoloGameMetrics(metrics, gameKey, difficulty) {
  ...
  if (gameKey === "memory") {
    ...
    if (mistakes > expectedMaxMistakes + 2) {
      return { ok: false, message: "Mistake count out of range" };
    }
  }
  ...
  if (gameKey === "leo-jump") {
    ...
    if (score > 5000) return { ok: false, message: "Score out of range" };
  }

  if (gameKey === "balloons") {
    if (score > 600) return { ok: false, message: "Score out of range" };
    const levelReached = Number(metrics?.levelReached);
  ...
  if (gameKey === "target-tap") {
    ...
    if (score > cap + 20) return { ok: false, message: "Score out of range" };
    const mistakes = Number(metrics?.mistakes);
...
export async function finalizeSoloGameSession(supabase, params) {
  ...
  if (payout.coins > 0) {
    ...
    if (!coinResult.ok) {
      return {
        ...
        message: coinResult.message || "Could not award coins",
      };
    }
  ...
  if (diamondPayout.diamonds > 0) {
    ...
    if (!diamondResult.ok && !diamondResult.skipped) {
      return {
        ...
        message: diamondResult.message || "Could not award diamonds",
      };
    }
  ...
  if (updErr) {
    return { ok: false, code: "db_error", message: updErr.message || "Could not save result" };
  }
