...
export function validateEngineReviewAdminToken(req, headerNames = ["x-engine-review-token", "x-admin-token"]) {
  ...
  if (!sent) {
    return {
      ...
      status: 401,
      code: "missing_token",
      error: globalBurnDownCopy("lib__security__admin-token", "missing_admin_token_header"),
    };
  }
  if (!timingSafeCompareStrings(sent, expected)) {
    return {
      ...
      status: 401,
      code: "invalid_token",
      error: globalBurnDownCopy("lib__security__admin-token", "admin_token_does_not_match_server_configuration"),
    };
  }
  return { ok: true };
}
