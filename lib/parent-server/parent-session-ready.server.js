...
import { ensureGlobalProductMembership } from "../global/product-membership.server.js";

const BLOCKED_PERSONA_MESSAGE =
  "Google sign-in is available for parents only. Use the teacher or school portal.";
 ...
 * - English locale / report language on membership
...
export async function finalizeParentSessionReady(serviceRole, user, options = {}) {
  ...
  if (!eligible.ok) {
    return {
      ...
      message: BLOCKED_PERSONA_MESSAGE,
    };
  }
  ...
  if (requireGoogle && !userHasGoogleIdentity(user)) {
    return {
      ...
      message: "We could not verify Google sign-in. Please try again.",
    };
  }
