...
/** Global default when policy acceptance locale is missing or unsafe. */
export const POLICY_ACCEPTANCE_DEFAULT_LOCALE = "en";

/**
 * Resolve locale for parent_policy_acceptances rows.
 * Explicit valid locale preserved; missing/invalid → `en`.
 * @param {string|null|undefined} raw
 * @returns {string}
 */
export function resolvePolicyAcceptanceLocale(raw) {
  const trimmed = String(raw ?? "").trim();
  if (!trimmed) return POLICY_ACCEPTANCE_DEFAULT_LOCALE;
  if (trimmed.length < 2 || trimmed.length > 16) return POLICY_ACCEPTANCE_DEFAULT_LOCALE;
  try {
    const normalized = normalizeLocaleId(trimmed);
    if (!normalized || normalized.length < 2 || normalized.length > 16) {
      return POLICY_ACCEPTANCE_DEFAULT_LOCALE;
    }
    return normalized;
  } catch {
    return POLICY_ACCEPTANCE_DEFAULT_LOCALE;
...
export async function recordParentPolicyAcceptance(serviceRole, input) {
  ...
  const locale = resolvePolicyAcceptanceLocale(input.locale);
