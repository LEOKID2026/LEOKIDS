/**
 * Parent-facing insights and home recommendations from aggregated report data.
...
import { subjectLabel, topicLabelHe } from "../teacher-portal/teacher-ui.js";
...
} from "../../utils/parent-report-language/parent-report-copy-spec.js";
import { createReportTranslator } from "../reports/report-locale.js";
...
/**
 * High-traffic parent insight headlines via report locale bundles.
 * @param {string} [reportLocale]
 */
function createParentInsightCopy(reportLocale = "en") {
  const { t } = createReportTranslator(reportLocale);
  return {
    noPracticeData: () => t("reports.parentInsights.noPracticeData"),
    insufficientData: () => t("reports.parentInsights.insufficientData"),
    recentInactivity: () => t("reports.parentInsights.recentInactivity"),
    dailyImprovement: () => t("reports.parentInsights.dailyImprovement"),
    noUrgentTopic: () => t("reports.parentInsights.noUrgentTopic"),
    homeFallback: () => t("reports.parentInsights.homeFallback")
  };
}
 ...
 * @returns {string[]}
 */
export function buildParentInsightsHe(payload, reportLocale = "en") {
  const copy = createParentInsightCopy(reportLocale);
  ...
  if (!guarded.length) {
    guarded.push(copy.noUrgentTopic());
  }
 ...
 * @returns {string[]}
 */
export function buildHomeRecommendationsHe(payload, insights = [], reportLocale = "en") {
  const copy = createParentInsightCopy(reportLocale);
  const subjects = payload?.subjects || {};
  const summary = payload?.summary || {};
  const subjectQuestionCounts = subjectQuestionCountsFromPayload(payload);
...
/**
 * @param {Record<string, unknown>} payload
 * @param {{ reportLocale?: string }} [opts]
 * @returns {{ insights: string[], homeRecommendations: string[] }}
 */
export function buildParentFacingBlocks(payload, opts = {}) {
  const reportLocale =
    opts.reportLocale ||
    (typeof payload?.meta?.reportLocale === "string" ? payload.meta.reportLocale : "en");
  const withQuality =
  ...
  let insights = buildParentInsightsHe(withQuality, reportLocale);
  ...
  const homeRecommendations = buildHomeRecommendationsHe(withQuality, insights, reportLocale);
 ...
 * @param {{ reportLocale?: string }} [opts]
 */
export async function enrichPayloadWithParentFacing(serviceRole, payload, studentId, opts = {}) {
  const reportLocale =
    opts.reportLocale ||
    (typeof payload?.meta?.reportLocale === "string" ? payload.meta.reportLocale : "en");
  bindReportPackLocale(reportLocale);
  bindPlatformDisplayLocale(reportLocale);
  const withQuality = attachParentContextEvidenceQuality(payload);
  const blocks = buildParentFacingBlocks(withQuality, { reportLocale });
  const subjectQuestionCounts = subjectQuestionCountsFromPayload(withQuality);
