...
const SCIENCE_TOPIC_LABELS = {
  body: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "human_body"),
  animals: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "animals"),
  plants: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "plants"),
  materials: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "materials"),
  experiments: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "experiments"),
  earth_space: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "earth_and_space"),
  environment: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "environment"),
};
...
function userFacingActivityLevelLabel(levelKeyOrLabel, subjectId, levelsMap) {
  ...
  if (!raw) return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  // Hebrew + English aliases for incoming curriculum/UI labels; always return English.
  if (raw === "" || raw === globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard") || raw === "regular" || raw === "Easy" || raw === globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "medium")) {
    return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  }
  if (raw === "" || raw === globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced") || raw === "advanced" || raw === "Hard") {
    return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced");
  }
  if (isScienceSubjectId(subjectId)) return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  const dl = activityDbEnumToDisplayLevel(raw.toLowerCase());
  if (dl === "advanced") return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced");
  if (dl === "regular") return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  // Geometry LEVELS still ship Hebrew names (//); map those to English UI.
  const legacyName = levelsMap?.[raw]?.name || levelsMap?.[raw.toLowerCase()]?.name;
  if (legacyName === "" || legacyName === "Hard") return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced");
  if (
    legacyName === "" |
    legacyName === "" |
    legacyName === "" |
    legacyName === "Easy" |
    legacyName === globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "medium") |
    legacyName === globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard")
  ) {
    return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  }
  if (legacyName === "" || legacyName === globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced")) return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced");
  return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
}
...
const SCIENCE_TOPIC_MAP = {
  ...
  // Legacy Hebrew aliases (unicode keys — not displayed)
  "": "body",
  "": "body",
  "": "animals",
  "": "animals",
...
const GEOMETRY_TOPIC_LABELS_EN = {
  shapes_basic: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "basic_shapes"),
  area: "Area",
  perimeter: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "perimeter"),
  volume: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "volume"),
  angles: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "angles"),
  parallel_perpendicular: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "parallel_and_perpendicular"),
  triangles: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "triangles"),
  quadrilaterals: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "quadrilaterals"),
  transformations: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "transformations"),
  rotation: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "rotation"),
  symmetry: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "symmetry"),
  diagonal: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "diagonals"),
  heights: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "heights"),
  tiling: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "tiling"),
  circles: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "circles"),
  solids: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "solid_shapes"),
  pythagoras: globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "pythagorean_theorem"),
  mixed: "Mixed",
};
...
function activityLevelLabelForErrors(plan, subjectId) {
  if (isScienceSubjectId(subjectId)) return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  if (plan.displayLevel === "advanced") return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced");
  if (plan.displayLevel === "regular") return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
  const sds = Array.isArray(plan.sourceDifficulties) ? plan.sourceDifficulties : [];
  if (sds.length === 1 && sds[0] === "hard") return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "advanced");
  return globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "standard");
}
...
function geometryLevelConfig(sourceDifficulty) {
  ...
  const nameEn = key === "hard" ? "Hard" : key === "medium" ? globalBurnDownCopy("lib__classroom-activities__generate-activity-questions-client", "medium") : "Easy";
...
export async function generateActivityQuestionSetClient({
  ...
  const localeOpts = { contentLocale, interfaceLocale, instructionLocale };
  ...
  if (sub === "math") {
    ...
    for (let attempt = 0; attempt < maxAttempts && questions.length < n; attempt += 1) {
      ...
      const q = generateQuestion(levelConfig, operation, grade, null, localeOpts);
  ...
  if (sub === "science") {
    ...
    const { localizeScienceQuestionsForContentLocale } = await import(
      "../learning/question-content-locale.js"
    );
    const rawPool = Array.isArray(bank?.SCIENCE_QUESTIONS) ? bank.SCIENCE_QUESTIONS : [];
    const pool = localizeScienceQuestionsForContentLocale(
      rawPool,
      contentLocale || "en");
  ...
  if (sub === "geometry") {
    ...
    for (let attempt = 0; attempt < maxAttempts && questions.length < n; attempt += 1) {
      ...
      const q = generateQuestion(levelConfig, topicKey, gradeKey, null, localeOpts);
  ...
  if (sub === "english") {
    ...
    for (let attempt = 0; attempt < maxAttempts && questions.length < n; attempt += 1) {
      ...
      const q = generateEnglishQuestion(levelConfig, topicKey, gradeKey, null, sourceDifficulty, localeOpts);
