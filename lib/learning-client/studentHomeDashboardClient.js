...
export function buildStudentHomeView({ student, homePayload }) {
  ...
  for (const sk of STUDENT_HOME_SUBJECT_ORDER) {
    ...
    for (const b of list) {
      ...
      if (!t || seen.has(t)) continue;
      seen.add(t);
      badges.push({ label: t, subjectKey: sk, subjectLabel: SUBJECT_UI[sk]?.labelHe ?? sk });
    }
  }
  ...
  if (topKey && topAnswers > 0 && SUBJECT_UI[topKey]) {
    recommendations.push({
      id: "practice-focus",
      titleHe: burnDownCopy("lib__learning-client__studentHomeDashboardClient", "suggested_next_step"),
  ...
  recommendations.push({
    id: "pick-subject",
    titleHe: burnDownCopy("lib__learning-client__studentHomeDashboardClient", "pick_up_where_you_left_off"),
    descriptionHe:
      topAnswers > 0
