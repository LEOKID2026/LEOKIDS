...
const SUBJECT_UI = {
  math: { label: "Math", href: "/learning/math-master", storageKey: "math" },
  english: { label: "English", href: "/learning/english-master", storageKey: "english" },
  science: { label: "Science", href: "/learning/science-master", storageKey: "science" },
  geometry: { label: "Geometry", href: "/learning/geometry-master", storageKey: "geometry" },
};
...
export function buildStudentHomeView({ student, homePayload }) {
  ...
  /** @type {Array<{ label: string, subjectKey: string, subjectLabel: string }>} */
  const badges = [];
  const seen = new Set();
  ...
  if (achievementNames.length) {
    for (const label of achievementNames) {
      ...
      seen.add(t);
      badges.push({ label: t, subjectKey: "", subjectLabel: "" });
    }
  }
  for (const sk of STUDENT_HOME_SUBJECT_ORDER) {
    ...
    for (const b of list) {
      ...
      if (!t || seen.has(t)) continue;
      seen.add(t);
      badges.push({ label: t, subjectKey: sk, subjectLabel: SUBJECT_UI[sk]?.label ?? sk });
    }
  }
  ...
  if (topKey && topAnswers > 0 && SUBJECT_UI[topKey]) {
    recommendations.push({
      id: "practice-focus",
      title: burnDownCopy("lib__learning-client__studentHomeDashboardClient", "suggested_next_step"),
      description: `You have the most activity in ${SUBJECT_UI[topKey].label}. Tap to continue there.`,
      href: SUBJECT_UI[topKey].href,
      ctaHe: `Go to ${SUBJECT_UI[topKey].label}`,
      truthState: StudentDisplayTruthState.estimated,
    });
  }
  recommendations.push({
    id: "pick-subject",
    title: burnDownCopy("lib__learning-client__studentHomeDashboardClient", "pick_up_where_you_left_off"),
    description:
      topAnswers > 0
        ? "Choose a subject from the list below to keep learning."
        : burnDownCopy("lib__learning-client__studentHomeDashboardClient", "no_activity_recorded_yet_choose_a_subject_to_get_started"),
    href: "/learning",
    ctaHe: burnDownCopy("lib__learning-client__studentHomeDashboardClient", "go_to_learning_hub"),
    truthState: StudentDisplayTruthState.estimated,
  });
  ...
  return {
    identity: {
      ...
      avatarCustomDataUrl,
      avatarBackgroundKey,
      friendlyLineHe: burnDownCopy("lib__learning-client__studentHomeDashboardClient", "great_to_see_you_every_practice_session_brings_you_closer_to_your_goal"),
    },
    accountStats: {
        ...
        : minutesThisMonth == null
          ...
          : progressPct != null && progressPct >= 100
            ? celebrationShownForMonth
              ...
              : burnDownCopy("lib__learning-client__studentHomeDashboardClient", "the_monthly_goal_is_usually_marked_after_you_reach_the_minutes_goal_keep")
