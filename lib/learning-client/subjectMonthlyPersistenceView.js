...
export function buildSubjectMonthlyPersistenceView(derived, monthlyPersistenceStatus, economyConfig) {
  ...
  const tiers = monthlyTiers.map((tier) => {
    const label = `${tier.coins.toLocaleString("en-US")} coins`;
  ...
  if (alreadyAwarded && awardedTierMinutes != null) {
    encouragementHe = burnDownCopy("lib__learning-client__subjectMonthlyPersistenceView", "you_got_the_monthly_persistence_reward_well_done");
  } else if (highestReached && currentMinutes >= (monthlyTiers[0]?.minutes ?? 0)) {
    encouragementHe = `You reached ${highestReached.minutes} minutes — a ${highestReached.coins.toLocaleString("en-US")}-coin reward is waiting`;
  } else if (minutesRemaining != null && minutesRemaining > 0) {
    encouragementHe = `${Math.round(minutesRemaining)} more minutes left (~${Math.ceil(Math.round(minutesRemaining) / 60)} hrs)`;
  } else {
    encouragementHe = burnDownCopy("lib__learning-client__subjectMonthlyPersistenceView", "you_reached_the_monthly_goal_keep_learning");
  }
