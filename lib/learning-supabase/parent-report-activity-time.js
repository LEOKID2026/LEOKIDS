...
/**
 * Parent-report activity timestamps — storage stays UTC/ISO;
 * display uses explicit IANA timezone when provided, otherwise UTC.
 */

export const PARENT_REPORT_DISPLAY_TIMEZONE = DEFAULT_CALENDAR_TIMEZONE;
 ...
 * @returns {string}
 */
export function formatParentReportActivityIsrael(ms, explicitTimeZone) {
  if (!Number.isFinite(ms) || ms <= 0) return "Unavailable";
  const tz = resolveCalendarTimeZone(explicitTimeZone ?? PARENT_REPORT_DISPLAY_TIMEZONE);
  const formatted = formatInstantInTimeZone(ms, tz, {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  // en-GB yields DD/MM/YYYY, HH:MM — normalize comma spacing
  return String(formatted || "").replace(",", "");
}

/** @deprecated Alias — prefer formatParentReportActivityIsrael with optional timezone. */
export const formatParentReportActivityLocal = formatParentReportActivityIsrael;
