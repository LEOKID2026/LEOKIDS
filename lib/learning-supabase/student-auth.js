...
export async function getAuthenticatedStudentSession(req) {
  ...
  const studentSelectFull =
    "id,full_name,grade_level,is_active,account_kind,leo_number,guest_status,product_id,student_coin_balances(balance)";
  const studentSelectFallback =
    "id,full_name,grade_level,is_active,product_id,student_coin_balances(balance)";
  ...
  if (studentResult.error && isMissingColumnError(studentResult.error)) {
    const msg = String(studentResult.error.message || "").toLowerCase();
    if (msg.includes("product_id")) {
      // Product isolation schema not applied — cannot prove Global scope.
      return null;
    }
  ...
  // Global site: only students tagged leokids_global may use learning APIs.
  const { PRODUCT_GLOBAL } = await import("../global/product-context.server.js");
  if (student.product_id !== PRODUCT_GLOBAL) {
    return null;
  }
