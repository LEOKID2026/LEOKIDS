 ...
 */
export function runParentReportGenerationFromApiBody(reportApiBody, uiPeriodLabel, opts = {}) {
  if (typeof window === "undefined") {
    return { ok: false, error: "server" };
  }

  const reportLocale =
    String(opts.reportLocale || reportApiBody?.meta?.reportLocale || "").trim() || "en";
  bindReportPackLocale(reportLocale);
  bindLearningBurnDownLocale(reportLocale);
  ...
  try {
    const { base, detailed } = runWithIsolatedReportStorage(store, () => {
      ...
      if (!generated.meta || typeof generated.meta !== "object") generated.meta = {};
      generated.meta.reportLocale = reportLocale;

      const rawBaseReport = generated;
      const gradeFilteredBase = buildRegularReportViewModel(rawBaseReport)?.report ?? rawBaseReport;
      if (gradeFilteredBase && typeof gradeFilteredBase === "object") {
        if (!gradeFilteredBase.meta || typeof gradeFilteredBase.meta !== "object") {
          gradeFilteredBase.meta = {};
        }
        gradeFilteredBase.meta.reportLocale = reportLocale;
      }
