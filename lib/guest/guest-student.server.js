...
export async function resumeGuestStudent(supabase, resumeToken) {
  ...
  if (!raw) {
    return { ok: false, status: 400, code: "missing_resume_token", message: "Missing resume token." };
  }

  const config = await loadGuestRuntimeConfig(supabase);
  if (!config.enabled) {
    return { ok: false, status: 403, code: "guest_mode_disabled", message: "Guest mode is currently disabled." };
  }

  const tokenHash = hashStudentSecret(raw);
  const binding = await findActiveGuestBinding(supabase, tokenHash);
  if (!binding?.student_id) {
    return { ok: false, status: 401, code: "guest_resume_invalid", message: "Could not resume the guest on this device." };
  }
  ...
  if (isLinkedGuestStudent(student)) {
    return {
      ...
      message: globalBurnDownCopy("lib__guest__guest-student.server", "this_number_is_already_linked_to_a_parent_sign_in_with_a_username_and_pi"),
    };
  }
  ...
  if (!session.ok) {
    return { ok: false, status: 500, code: session.code, message: "Server error." };
  }
...
export async function startGuestStudent(supabase, options = {}) {
  ...
  if (!parent.ok) {
    return { ok: false, status: parent.status, code: parent.code, message: "The guest system is not ready." };
  }

  const leoNumber = await generateUniqueLeoNumber(supabase);
  const displayName = `Guest ${leoNumber}`;
  const nowIso = new Date().toISOString();
  ...
  if (studentErr || !student?.id) {
    if (isMissingGuestColumnError(studentErr)) {
      return { ok: false, status: 503, code: "db_schema_not_ready", message: "The guest system is not ready." };
    }
    return { ok: false, status: 500, code: "guest_create_failed", message: "Could not create a guest." };
  }
  ...
  if (bindErr) {
    return { ok: false, status: 500, code: "guest_binding_failed", message: "Could not create a guest." };
  }
