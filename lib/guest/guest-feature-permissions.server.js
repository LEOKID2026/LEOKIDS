...
export const GUEST_FEATURE_LABELS_HE = Object.freeze({
  room_public_create: "Create public room",
  room_private_create: "Create private room",
  room_join_by_code: "Join with code",
  quick_game: "Quick game",
  invites_send: "Send game invite",
  invites_receive: "Receive game invite",
  friends: "Add friend",
  safe_messages: "Preset messages",
  shop: "Buy cosmetics",
  events: "Daily event",
  tournaments: "Tournaments",
  missions: "Arcade missions",
...
export async function assertGuestArcadeFeature(supabase, studentId, featureKey) {
  ...
  if (!studentRow?.id) {
    return { ok: false, status: 404, code: "student_not_found", message: "Player not found." };
  }
  ...
  if (!isGuestFeatureAllowedForStudent(studentRow, map, featureKey)) {
    return {
      ...
      message: "This action is not currently available for guests.",
    };
  }
