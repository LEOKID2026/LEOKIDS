import {
  ...
  GUEST_GAME_LOCK_MESSAGE_KEY,
  GUEST_LOCK_MESSAGE_KEY,
  GUEST_LOCKED_HOME_PANELS,
  GUEST_STATUS_ACTIVE,
  GUEST_STATUS_LINKED,
  GUEST_TOPIC_LOCK_MESSAGE_KEY,
} from "./constants.js";
...
export async function buildGuestPolicyPayload(supabase, student) {
  ...
  return {
    ...
    displayName: formatGuestDisplayNameHe(student),
    defaultGradeLevel: GUEST_DEFAULT_GRADE_LEVEL,
    lockedHomePanels: [...GUEST_LOCKED_HOME_PANELS],
    lockMessageKey: GUEST_LOCK_MESSAGE_KEY,
    gameLockMessageKey: GUEST_GAME_LOCK_MESSAGE_KEY,
    topicLockMessageKey: GUEST_TOPIC_LOCK_MESSAGE_KEY,
...
export function applyGuestLockToGameAccess(baseAccess, catalogRow, guestPlayableByKey) {
  ...
  return {
    ...
    messageKey: GUEST_GAME_LOCK_MESSAGE_KEY,
  };
}
...
export async function assertGuestLearningTopicAllowed(supabase, studentId, subject, topic) {
  ...
  return {
    ...
    messageKey: GUEST_TOPIC_LOCK_MESSAGE_KEY,
  };
}
...
export function assertNotGuestForReports(student) {
  if (isGuestStudent(student)) {
    return {
      ...
      messageKey: "ui.parent.errors.guestNotEligible",
    };
  }
  return { ok: true };
}
