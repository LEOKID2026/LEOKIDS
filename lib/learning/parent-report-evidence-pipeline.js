...
export function buildParentEvidenceStatements(p) {
  ...
  /** @type {Array<{ kind: ParentEvidenceStatementKind, text: string, evidenceRef: object }>} */
  const statements = [];

  statements.push({
    kind: "FACT",
    text: `Answered ${q} questions, ${c} correct${w > 0 ? `, ${w} incorrect` : ""}${q > 0 ? ` (${acc}% accuracy)` : ""}.`,
  ...
  if (evRec && evRec.evidenceCount > 0) {
    ...
    if (tag === "omitted_addend") {
      statements.push({
        ...
        text: `In ${evRec.evidenceCount} of ${evRec.relevantQuestions} three-addend problems, only the first two addends were added.`,
        evidenceRef: {
          tag,
      ...
      statements.push({
        kind: "OBSERVED_PATTERN",
        text: `In ${evRec.evidenceCount} of ${evRec.relevantQuestions} subtraction problems, the answer matched adding the two numbers.`,
        evidenceRef: { tag, evidenceCount: evRec.evidenceCount, examples },
      });
    } else if (tag) {
      statements.push({
        kind: "OBSERVED_PATTERN",
        text: `A repeating pattern (${evRec.evidenceCount}/${evRec.relevantQuestions}) was seen for error type "${tag}".`,
        evidenceRef: { tag, evidenceCount: evRec.evidenceCount, examples },
      });
    }
    ...
    if (evRec.state === "suspected") {
      statements.push({
        kind: "HYPOTHESIS",
        text: `There may be a pattern in error "${tag || tax?.patternHe || "unknown"}"; a check question is needed.`,
    ...
    if (evRec.confirmed || evRec.state === "confirmed") {
      statements.push({
        ...
        text: `The pattern "${tax?.patternHe || tag}" repeated ${evRec.evidenceCount} times with matching evidence${evRec.matchingEvents.some((e) => e.metadata?.probeConfirmed) ? " and was confirmed on a check question" : ""}.`,
  ...
  if (recentCorrect && p.de2Unit?.recurrence?.evidenceRecurrence?.state === "resolved") {
    statements.push({
      kind: "PROGRESS",
      text: "Recent questions show improvement relative to a pattern identified earlier.",
