...
assert.equal(topicMaps.mathOperations.addition.mode, "Learning");
assert.equal(topicMaps.mathOperations.addition.level, "Regular");
assert.ok(topicMaps.mathOperations.addition.lastSessionAt);
assert.ok(topicMaps.mathOperations.addition.grade && topicMaps.mathOperations.addition.grade !== "Unavailable");
assert.equal(topicMaps.geometryTopics.shapes.timeMinutes, 16);
assert.equal(topicMaps.geometryTopics.shapes.mode, "Learning");
...
assert.ok(!String(report.summary.diagnosticOverviewHe.notPracticedSubjectsSummaryHe || "").includes(""));
assert.ok(!String(report.summary.diagnosticOverviewHe.notPracticedSubjectsSummaryHe || "").includes(""));
