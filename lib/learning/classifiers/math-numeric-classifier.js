...
import { classifyStructuralMulDivError } from "../fuzzy-tolerance-muldiv.js";
...
export function classifyMathNumericAnswer(userAnswer, expectedAnswer, params, kind) {
  ...
  // Fuzzy-tolerance fallback tier (active subjects only: math/geometry).
  // Runs only when no exact rule above matched; never overrides an exact hit.
  if (isTopic3FracKind(k) || k.includes("frac")) {
    const fracHit = classifyFractionAnswer({ kind: k, ...p, userAnswer, expectedAnswer });
    if (fracHit) {
      return {
        tag: fracHit.tag,
        evidenceType: fracHit.evidenceType || EVIDENCE_TYPES.DIRECT_EVIDENCE,
        details: fracHit.details,
        confidence: fracHit.confidence ?? 0.85,
        ruleId: fracHit.ruleId,
      };
    }
  }

  if (isCoreOpsKind(k)) {
    const coreHit = classifyCoreOpsAnswer({ kind: k, ...p, a, b, c, userAnswer: n, expectedAnswer: expected });
    if (coreHit) {
      return {
        tag: coreHit.tag,
        evidenceType: coreHit.evidenceType || EVIDENCE_TYPES.DIRECT_EVIDENCE,
        details: coreHit.details,
        confidence: coreHit.confidence ?? 0.85,
        ruleId: coreHit.ruleId,
      };
    }
  }

  if (isGeometryNumericKind(k) || k.startsWith("heights_")) {
    const geoHit = classifyGeometryAnswer({ kind: k, ...p, a, b, c, userAnswer: n, expectedAnswer: expected });
    if (geoHit) {
      return {
        tag: geoHit.tag,
        evidenceType: geoHit.evidenceType || EVIDENCE_TYPES.DIRECT_EVIDENCE,
        details: geoHit.details,
        confidence: geoHit.confidence ?? 0.85,
        ruleId: geoHit.ruleId,
      };
    }
  }

  const structuralAddSub = classifyStructuralAddSubError({ kind: k, a, b, c, userAnswer: n, expectedAnswer: expected });
  if (structuralAddSub) {
    return {
      tag: structuralAddSub.tag,
      evidenceType: EVIDENCE_TYPES.DIRECT_EVIDENCE,
      details: structuralAddSub.details,
      confidence: structuralAddSub.confidence ?? 0.8,
      ruleId: structuralAddSub.ruleId,
    };
  }

  const structuralMulDiv = classifyStructuralMulDivError({ kind: k, params: p, a, b, userAnswer: n, expectedAnswer: expected });
  if (structuralMulDiv) {
    return {
      tag: structuralMulDiv.tag,
      evidenceType: EVIDENCE_TYPES.DIRECT_EVIDENCE,
      details: structuralMulDiv.details,
      confidence: structuralMulDiv.confidence ?? 0.8,
      ruleId: structuralMulDiv.ruleId,
    };
  }
