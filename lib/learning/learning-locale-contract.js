 ...
 * Uses registered pack catalog (client + server); does not hard-import content-packs/en.
 */
...
export function loadLearningContentPack(contentLocale, ...segments) {
  ...
  return resolveRegisteredContentPack(locale, "learning", ...segments);
}
...
export function loadTaxonomyBundle(subject, contentLocale) {
  const locale = resolveContentLocale({ contentLocale, subject });
  const structure = resolveRegisteredContentPack(
    locale,
    "learning",
    "taxonomy",
    `${subject}.structure.json`,
  );
  const content = resolveRegisteredContentPack(
    locale,
    "learning",
    "taxonomy",
