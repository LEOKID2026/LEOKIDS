...
export function detectStemLeak(stem, correctValue) {
  ...
  if (tokenCount === 1 && correctStr === "" && /(?!)/i.test(s)) {
    return false;
  }
  if (/(?!)/u.test(s) && /^-?\d+([.]\d+)?$/.test(correctStr)) {
    const nums = s.match(/-?\d+(?:[.]\d+)?/g) || [];
    if (nums.some((n) => n.replace(",", ".") === correctStr.replace(",", "."))) {
      return false;
    }
  }
  if (/(?!)/u.test(s) && /(?!)/u.test(s)) {
    const subjectNum = s.match(/(?!)/u)?.[1];
  ...
  if (/(?!)/u.test(s) && /(?!)/u.test(s) && /^-?\d+([.]\d+)?$/.test(correctStr)) {
    const nums = s.match(/-?\d+(?:[.]\d+)?/g) || [];
    if (nums.some((n) => n.replace(",", ".") === correctStr.replace(",", "."))) return false;
  }
  if (/(?!)/u.test(s) && s.includes(correctStr)) {
    return false;
  }
  if (/(?!)/u.test(s) && s.includes(correctStr)) {
    return false;
  }
  if (/(?!)/u.test(s) && /(?!)/u.test(s) && s.includes(correctStr)) {
    return false;
  }
  const quotedChunk = s.match(/(?!)/u);
  ...
  if (/(?!)/u.test(s)) {
    ...
    if (s.includes(correctStr) && /(?!)/.test(s)) return false;
  }
  if (tokenCount >= 2 && s.includes(correctStr) && /(?!)/.test(s)) {
    return false;
  }
  ...
  if (correctStr === "" && /(?!)/.test(s) && /(?!)/.test(s)) {
    return false;
  }
  if (tokenCount === 1 && /(?!)/.test(s) && /(?!)/.test(s)) {
    const alts = s
      .split(/(?!)/)
