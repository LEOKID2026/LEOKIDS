 ...
 */
export function buildDemoParentReportPayload(childId, fromYmd, toYmd, opts = {}) {
  const reportLocale = opts.reportLocale || "en";
  ...
  const blocks = buildParentFacingBlocks(withBooks, { reportLocale });
  ...
  const scaffoldBody = filterDemoReportSubjects(
    stripInternalReportPayloadFields({
      ...
      demo: true,
      meta: {
        ...(withUnifiedTime.meta && typeof withUnifiedTime.meta === "object" ? withUnifiedTime.meta : {}),
        reportLocale,
      },
    }),
  );
