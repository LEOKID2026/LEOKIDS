...
import { normalizeHebrewHyphensForTts } from "./normalize-hyphens-for-tts.js";
...
export function convertMathExpressionsForTts(text, options = {}) {
  ...
  replaceTracked(
    /(\d+)\s*\+\s*__\s*=\s*(\d+)/g,
    (a, b) => `${speakNumberToken(a)}     ${speakNumberToken(b)}`,
    "addition_blank"
  );
  replaceTracked(
    /(\d+)\s*\+\s*(\d+)\s*=\s*__/g,
  ...
  replaceTracked(
    /(\d+)\s*\+\s*(\d+)\s*=\s*(\d+)/g,
    (a, b, c) => `${speakNumberToken(a)}  ${speakNumberToken(b)}  ${speakNumberToken(c)}`,
    "addition_equality"
  );

  replaceTracked(
    /(\d+)\s*([-−])\s*(\d+)\s*=\s*\?/g,
    (a, _op, b) => `${speakNumberToken(a)}  ${speakNumberToken(b)}   `,
    "subtraction_unknown"
  );
  replaceTracked(
    /(\d+)\s*([-−])\s*(\d+)\s*=\s*(\d+)/g,
    (a, _op, b, c) =>
      `${speakNumberToken(a)}  ${speakNumberToken(b)}  ${speakNumberToken(c)}`,
  ...
  replaceTracked(/(?!)/g, (n) => ` ${speakNumberToken(n)}`, "from_number");
  replaceTracked(/(?!)/g, (n) => ` ${speakNumberToken(n)}`, "until_number");
