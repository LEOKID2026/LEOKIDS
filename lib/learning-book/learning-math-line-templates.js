...
/**
 * X% -Y = Z   /   ❌ 25% -80 = 55 —  25   /   50% -60 = 30 ()
 * Percent stays its own math island, "-" stays prose, the quantity/equation
...
export function parseMakeTenSequenceBody(body) {
  ...
  return [mathRun(eq1), proseRun(""), mathRun(eq2)];
}
...
export function parseComparisonConclusionRuns(text) {
  ...
  const tryBody = (rest) => {
    const arrow = rest.match(
      new RegExp(
        `^(${NUM})\\s+()\\s+-?\\s*(${NUM})\\s*→\\s*(${NUM})\\s*([<>＝=])\\s*(${NUM})\\s*$`
      )
    );
    const hence = rest.match(
      new RegExp(
        `^(${NUM})\\s+()\\s+-?\\s*(${NUM})\\s*,?\\s*:\\s*(${NUM})\\s*([<>＝=])\\s*(${NUM})\\s*$`
      )
    );
    const m = arrow || hence;
    if (!m) return null;
...
export function parseLabeledDigitComparisonRuns(text) {
  ...
  // The arrow tail can itself contain equations (e.g. " → 10 − 5 = 5").
...
export function buildComparisonConclusionRuns({ left, right, relation }) {
  ...
  return [proseRun(`${a}  -${b}, : `), mathRun(`${a} = ${b}`)];
}
