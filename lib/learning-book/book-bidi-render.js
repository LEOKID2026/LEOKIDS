...
import { splitMixedHebrewMathRuns } from "../bidi/mixed-rtl-math-runs.js";
...
/** Hebrew single-letter prefixes before hyphen + number (-, -, -, -, -, -, -) */
export const HEBREW_MATH_AFFIX =
  /(?!)/u;

/**
 * Split a prose fragment for safer BiDi rendering when it sits between math runs.
 * Keeps Hebrew affixes like "-" attached to RTL prose, never merged into LTR math.
 * @param {string} text
...
/**
 * Split ": 68 − 24 = ?" into Hebrew prefix + one LTR equation island.
 * @param {string} text
...
export function splitInlineHebrewTaskEquation(text) {
  ...
  if (/(?!)/u.test(equation)) return null;
  return { prefix: stripStrayMarkdown(match[1]).trim(), equation };
}

/**
 * Split comma-separated Hebrew formula rows like "2  = 200, 3  = 30".
 * @param {string} body
 * @returns {string[]|null}
 */
...
/**
 * Split "… 68 = 60 + 8, -24 = 20 + 4." into two display rows (renderer only).
 * @param {string} body
 * @returns {string[]|null}
 */
