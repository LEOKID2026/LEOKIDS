...
import { bookUiCopy } from "./book-pack-copy.js";

/** @typedef {{ type: string, options: Record<string, string> }} GeometryDiagramSpec */

/** @param {string} key */
export function getGeometryDiagramLabel(key) {
  return bookUiCopy("diagramLabels", key);
}

/** Back-compat — lazy label map via pack (English source). */
export const GEOMETRY_DIAGRAM_LABELS = new Proxy(
  {},
  {
    get(_t, prop) {
      if (typeof prop !== "string") return undefined;
      return getGeometryDiagramLabel(prop);
    },
  },
);
