/**
 * Flatten MixedRtlMathText output to child-visible strings (verification + export).
 */
...
/**
 * Flatten one book line the way MixedRtlMathText renders it.
 * @param {string} line
 */
...
export function detectSpacingRegression(source, rendered) {
  ...
  if (sourceNoSpace === renderedNoSpace && source !== rendered) {
    ...
    const longHebrewRun = rendered.match(/(?!)/u);
