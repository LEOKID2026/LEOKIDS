...
/**
 * @typedef {{
 *   batches: { id: string, title: string, pages: string[] }[],
 *   pageOrder: string[],
...
/**
 * Prefer locale tree `docs/learning-book/{locale}/{subject}/{grade}/drafts`,
 * then legacy `docs/learning-book/{subject}/{grade}/drafts` via resolveLearningBookDraftsDir.
 * Registry `meta.draftsDir` is legacy fallback metadata only.
 *
...
export function createLearningBookPageLoader(registry, opts = {}) {
  ...
  const contentLocale = resolveContentLocale({
    contentLocale: opts.contentLocale || registry.meta.contentLocale || DEFAULT_LOCALE,
    subject,
  });

  let draftsDir = null;
  if (subject && grade) {
    draftsDir = resolveLearningBookDraftsDir(contentLocale, subject, grade);
  }
  if (!draftsDir) {
    draftsDir = registry.meta.draftsDir;
  }
 ...
 * @param {{ contentLocale?: string|null }} [opts]
 */
export function loadLearningBookPage(registry, pageId, opts = {}) {
  return createLearningBookPageLoader(registry, opts).loadPage(pageId);
}
