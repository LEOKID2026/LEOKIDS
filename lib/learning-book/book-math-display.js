...
/** LTR label isolation inside mixed lines (Global English books) */
 ...
 * Keeps decimals (π ≈ 3.14) and commas inside carry lines (→ 5,  1).
 * @param {string} text
 */
...
/** LTR prose isolation between LTR math islands */
...
/** Hebrew measurement units attached to numbers (12 , 24 ). */
...
export function isFullEquationLine(text) {
  ...
  const eqPart = body.split(/(?!)/).shift()?.trim() || body;
  if (!eqPart) return false;

  if (/(?!)/.test(eqPart) && (eqPart.match(/=/g) || []).length >= 2) {
    return false;
  }

  if (/(?!)/.test(eqPart)) {
    // Place-value decompositions whose only Hebrew words are number/place words
    // (1  + 2  + 4  = 124) still read as a single LTR equation island.
 ...
 * left-to-right inside a numeric equation island (1  + 2  = 3 …).
 ...
 * into an LTR math island. (Place-value-only compounds like " " are OK.)
 * @param {string} text
 */
...
/**
 * Hebrew words between digit groups in place-value equations (1  + 2  …).
...
/** Hebrew measurement units that stay LTR-attached to a preceding number (12 ). */
...
function extendMathRunEnd(stripped, start) {
  ...
  while (i < len) {
    ...
    //  / "  
    if (stripped[i] === "") {
      const currencyMatch = stripped.slice(i).match(/(?!)/);
      if (currencyMatch) {
        let prev = i - 1;
        while (prev >= start && /\s/.test(stripped[prev])) prev -= 1;
        if (prev >= start && DIGIT.test(stripped[prev])) {
...
export function isFormulaLikeBody(text) {
  ...
  // Prose with word-internal hyphens (-) — not a formula row.
  if (WORD_INTERNAL_HYPHEN.test(s) && !/[()]/.test(s)) {
    return false;
  }

  // Explanatory Hebrew chains with multi-word segments ( =  =   ).
  if (!/[()]/.test(s) && /(?!)/.test(s)) {
    return false;
  }
...
/**
 * Tokenize Hebrew-formula bodies like " = ( × ) + ".
 * @param {string} text
 * @returns {{ type:  || "op", value: string }[]}
 */
/**
...
export function splitFormulaTokens(text) {
  ...
  /** @type {{ type:  || "op" || "symbol" || "space", value: string }[]} */
  const tokens = [];
  const re = /([=+−×÷()[\]])|([π≈°²³√∥⊥±])|(\s+)/g;
  let last = 0;
  let match;
  ...
  while ((match = re.exec(input)) !== null) {
    if (match.index > last) {
      ...
      if (chunk) tokens.push({ type: "rtl_text", value: chunk });
    }
    if (match[1]) {
