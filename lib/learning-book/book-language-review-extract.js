...
const EXCLUDED_BOOK_KEYS = new Set([]);
...
export function extractBookLanguageReview(root, entry, parseLearningPageMarkdown) {
  ...
  const result = {
    ...
    batches: entry.registry.batches.map((b) => ({
      id: b.id,
      title: resolveRegistryTitleKey(b.titleKey || `${subject}.${grade}.${b.id}`, "en"),
    })),
    pages: [],
    warnings: [],
  };
...
export function renderBookReviewMarkdown(book) {
  ...
  const subject = getLearningBookSubjectLabelCopy(book.subject, "en");

  /** @type {string[]} */
  const lines = [];
  lines.push(`# ${subjectLabel} ${gradeUpper} - Book Text Review`);
  lines.push("");
  lines.push(`Subject: ${subjectLabel} (${subject})`);
  ...
  for (const book of books) {
    ...
    for (const batch of book.batches) {
      blocks.push({
        ...
        language_detected: detectLanguage(batch.title),
        visible_text: batch.title,
        word_count: countWords(batch.title),
        character_count: batch.title.length,
        review_flags: computeReviewFlags(batch.title, book.grade),
        extraction_warning: null,
      });
    }
