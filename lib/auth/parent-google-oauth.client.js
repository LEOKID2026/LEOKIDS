...
export async function postParentSessionReady(accessToken, flow) {
  ...
  try {
    ...
    return { ok: false, messageKey: "auth.google.connectionError" };
  }
  ...
  try {
    ...
    return { ok: false, messageKey: "auth.google.serverError" };
  }
...
export async function completeParentGoogleSession(session) {
  ...
  if (!accessToken) {
    return {
      ok: false,
      messageKey: "auth.google.signInFailed",
      redirectTo: null,
    };
  }
  ...
  if (!ready.ok) {
    return {
      ok: false,
      messageKey: ready.messageKey || "auth.google.signInFailed",
      redirectTo: null,
    };
  }
