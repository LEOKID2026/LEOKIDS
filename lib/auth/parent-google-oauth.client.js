...
export async function postParentSessionReady(accessToken, flow) {
  ...
  try {
    ...
    return { ok: false, messageHe: "Connection problem. Check your internet and try again." };
  }
  ...
  try {
    ...
    return { ok: false, messageHe: "Server error. Please try again in a moment." };
  }
...
export async function completeParentGoogleSession(session) {
  ...
  if (!accessToken) {
    return {
      ok: false,
      messageHe: "We couldn't finish Google sign-in. Try again or sign in with email and password.",
      redirectTo: null,
    };
  }
  ...
  if (!ready.ok) {
    return {
      ...
      messageHe:
        ready.messageHe ||
        "We couldn't finish Google sign-in. Try again or sign in with email and password.",
      redirectTo: null,
    };
  }
