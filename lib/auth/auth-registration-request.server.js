...
import {
  REG_REQUEST_INTENT_IDS,
} from "./auth-registration.js";

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const REQUEST_INTENT_SET = new Set(REG_REQUEST_INTENT_IDS);
...
/**
 * Resolve preferred_language for a newly created teacher profile.
 * Explicit non-empty selection wins; otherwise global default `en`.
 * Existing profiles are never overwritten by ensureTeacherProfileAndLimits.
 * @param {string|null|undefined} preferredLanguage
 * @returns {string}
 */
export function resolveTeacherPreferredLanguageDefault(preferredLanguage) {
 ...
 * @param {string|null|undefined} [preferredLanguage]
 */
export async function ensureTeacherProfileAndLimits(
  serviceRole,
  teacherId,
  displayName,
  preferredLanguage
  ...
  if (!profileRes.profile) {
    const { error } = await serviceRole.from("teacher_profiles").insert({
      ...
      preferred_language: resolveTeacherPreferredLanguageDefault(preferredLanguage),
    });
    if (error) {
