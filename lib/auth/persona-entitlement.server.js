...
/**
 * Load parent account settings for the Global product table only (v3).
 * Does not read/write legacy parent_account_settings (IL exclusive).
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} serviceRole
...
export async function loadParentAccountSettings(serviceRole, parentUserId, productId) {
  const { loadProductParentAccountSettings } = await import(
    "../global/product-settings.server.js"
  );
  return loadProductParentAccountSettings(serviceRole, parentUserId, productId);
}
