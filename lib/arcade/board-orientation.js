...
export function boardSideLabels(seatLabels, mySeat, sides) {
  const fallbackTop = { name: seatLabels[sides[0].seat] || `Player ${sides[0].seat + 1}`, color: sides[0].color };
  const fallbackBottom = { name: seatLabels[sides[1].seat] || `Player ${sides[1].seat + 1}`, color: sides[1].color };
  ...
  const me = {
    name: seatLabels[mySeat] || `Player ${mySeat + 1}`,
    color: sides.find((s) => s.seat === mySeat)?.color || "",
    isMe: true,
  };
  const otherSeat = /** @type {0|1} */ (mySeat === 0 ? 1 : 0);
  const them = {
    name: seatLabels[otherSeat] || `Player ${otherSeat + 1}`,
