...
export async function applyChessAction(supabase, params) {
  ...
  if (!from || !to || from.length < 2 || to.length < 2) {
    return { error: { code: "invalid_move", message: "Invalid move", status: 400 } };
  }
  ...
  if (room.game_key !== "chess") {
    return { error: { code: "wrong_game", message: "This room is not Chess", status: 400 } };
  }
  ...
  if (expectedRevision != null && Number(expectedRevision) !== rev) {
    return {
      error: {
        ...
        message: gamePackCopy("lib__arcade__server__chess-game", "outdated_version_refresh"),
        revision: rev,
        status: 409,
      },
    };
  ...
  try {
    ...
    return { error: { code: "bad_state", message: "Corrupt board state", status: 500 } };
  }
