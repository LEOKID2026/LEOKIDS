...
export async function validateEntryCost(supabase, gameRow, entryCost) {
  ...
  if (!catalog.includes(c)) {
    return { error: { code: "invalid_entry_cost", message: "Invalid entry cost" } };
  }
  const allowed = Array.isArray(gameRow.allowed_entry_costs) ? gameRow.allowed_entry_costs : [];
  if (!allowed.includes(c)) {
    return { error: { code: "entry_cost_not_allowed", message: "Cost not available for this game" } };
  }
...
export async function createArcadeRoom(supabase, params) {
  ...
  if (rt !== "public" && rt !== "private" && rt !== "quick") {
    return {
      error: {
        ...
        message: gamePackCopy("lib__arcade__server__arcade-rooms", "unsupported_room_type_allowed_public_private_or_quick"),
      },
    };
  }

  if (rt === "public" && game.supports_public_rooms !== true) {
    return { error: { code: "room_type_not_supported", message: "Public rooms are not supported for this game" } };
  }
  if (rt === "quick" && game.supports_quick_match !== true) {
    return { error: { code: "room_type_not_supported", message: "Quick match is not supported for this game" } };
  }
  if (rt === "private" && game.supports_private_rooms !== true) {
    return { error: { code: "room_type_not_supported", message: "Private rooms are not supported for this game" } };
  }
  ...
  if (rt === "private") {
    ...
    if (!joinCode) {
      return { error: { code: "join_code_failed", message: "Could not create room code" } };
    }
  }
  ...
  if (insertRoom.error || !insertRoom.data) {
    return { error: { code: "room_create_failed", message: insertRoom.error?.message || "Error" } };
  }
...
export async function joinArcadeRoomById(supabase, studentId, roomId) {
  ...
  if (rErr || !room) return { error: { code: "room_not_found", message: "Room not found" } };
  if (room.status !== "waiting") {
    return { error: { code: "room_not_joinable", message: "Cannot join this room" } };
  }
  ...
  if (existing?.id) {
    return { error: { code: "already_joined", message: "Already in room" } };
  }
  ...
  if (ins.error) {
    ...
    return {
      error: {
        ...
        message: slotConflict ? "Seat taken or room filled" : ins.error.message,
      },
    };
  }
  ...
  if (ev.error) {
    ...
    return {
      error: {
        ...
        message: typeof e.message === "string" ? e.message : "Could not start the game",
      },
    };
  }
...
export async function joinArcadeRoomByCode(supabase, studentId, rawCode) {
  ...
  if (code.length < 4) {
    return { error: { code: "invalid_code", message: "Invalid code" } };
  }
  ...
  if (error || !room) return { error: { code: "room_not_found", message: "No room found for that code" } };
...
export async function leaveArcadeRoom(supabase, studentId, roomId) {
  ...
  if (!membership) {
    return { error: { code: "not_in_room", message: "Not in this room" } };
  }
