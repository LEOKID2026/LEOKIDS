/**
 *  OV2 —     (arcade_rooms + arcade_game_sessions).
 *     /  RPC -MLEO (10s  , row 10%, full = ).
 */
...
/**
 * Snapshot  normalizeOv2BingoAuthoritativeSnapshot (   ).
 *
...
/**
 *    -snapshot  `ov2_bingo_build_snapshot`  normalizeOv2BingoAuthoritativeSnapshot.
 *
  ...
  if (!winnerStudentId && entryCost > 0 && claimsPaidTotal <= 0) {
    for (const p of players || []) {
      ...
      const r = await refundArcadeEntry(
        ...
        { sourceId: roomId });
      if (!r.ok) return { error: { message: r.message || "Refund error" } };
    }
  }
...
export async function applyBingoArcadeAction(supabase, params) {
  ...
  if (room.game_key !== "bingo") {
    return { error: { code: "wrong_game", message: "Not a Bingo room", status: 400 } };
  }
  ...
  if (expectedRevision != null && expectedRevision !== rev) {
    return { error: { code: "revision_conflict", message: "Outdated version", revision: rev, status: 409 } };
  }
  ...
  if (action === "call_next") {
    if (String(session.status || "") !== "active") {
      return { error: { code: "game_finished", message: "Game is not active", status: 409 } };
    }
    ...
    if (String(bingo.caller_student_id || "") !== String(studentId)) {
      return { error: { code: "not_caller", message: "Only the caller can draw a number", status: 403 } };
    }
    ...
    if (Number.isFinite(nextAt) && now < nextAt) {
      return {
        error: {
          code: "call_too_soon",
          message: gamePackCopy("lib__arcade__server__bingo-game", "not_time_for_the_next_call_yet"),
          status: 400,
  ...
  if (action === "claim_prize") {
    ...
    if (!prizeKey) {
      return { error: { code: "invalid_action", message: "Missing prizeKey", status: 400 } };
    }

    if (String(session.status || "") !== "active") {
      return { error: { code: "game_finished", message: "Game is not in a winning state", status: 409 } };
    }
    if (String(bingo.phase || "") !== "playing") {
      return { error: { code: "not_playing", message: "Cannot claim right now", status: 409 } };
    }
    ...
    if (!card) {
      return { error: { code: "no_card", message: "No card", status: 400 } };
    }
    ...
    if (
      ...
      return { error: { code: "prize_not_earned", message: "Not eligible yet", status: 400 } };
    }

    if (existingClaims.some((c) => String(/** @type {Record<string, unknown>} */ (c).prize_key) === prizeKey)) {
      return { error: { code: "already_claimed", message: "Prize already taken", status: 400 } };
    }
    ...
    if (amount > 0) {
      ...
      if (!pay.ok) {
        return { error: { message: pay.message || "Credit failed", status: 500 } };
      }
    }
    ...
    if (prizeKey === "full") {
      ...
      if (fin.error) {
        return { error: { message: fin.error.message || "Finish failed", status: 500 } };
      }
    }
