...
export async function ensureDailyEventForToday(supabase) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      title: "Today's challenge — play an arcade game",
...
export async function getActiveDailyEvent(supabase, studentId = null) {
  ...
  return {
    ...
    title: data.title || data.slug || "Event",
...
export async function claimEventReward(supabase, studentId, eventId) {
  ...
  if (!event?.id) return { ok: false, code: "not_found", message: "Event not found" };
  ...
  if (!part?.completed_at) return { ok: false, code: "not_completed", message: "You haven't completed the event yet" };

  if (part.reward_claimed) return { ok: false, code: "already_claimed", message: "Reward already claimed" };
...
export async function registerForTournament(supabase, studentId, tournamentId) {
  ...
  if (!t?.id || t.status !== "registration") {
    ...
    return { ok: false, code: "not_open", message: "Tournament registration is closed" };
  ...
  if ((count || 0) >= t.max_players) {
    ...
    return { ok: false, code: "full", message: "Tournament is full" };
