...
export async function getPersonalRoom(supabase, studentId) {
  ...
  if (!data) {
    return {
      ...
      room: {
        studentId,
        roomName: gamePackCopy("lib__arcade__club__personal-room.server", "my_room"),
        backgroundId: null,
        decorationSlots: [],
      },
      featureLocked: false,
...
export async function updatePersonalRoom(supabase, studentId, patch) {
  ...
  if (error) {
    if (error.code === "42P01") return { ok: false, code: "unavailable", message: "Personal room unavailable", status: 503 };
