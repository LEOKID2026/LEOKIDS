...
export async function getArcadeDisplayName(supabase, studentId) {
  ...
  if (!studentRow?.id) return "Player";
...
export function validateDisplayNameInput(raw) {
  ...
  if (name.length < DISPLAY_NAME_MIN) {
    return { ok: false, code: "empty_name", message: "Display name cannot be empty" };
  }
  if (name.length > DISPLAY_NAME_MAX) {
    return { ok: false, code: "name_too_long", message: `Display name up to ${DISPLAY_NAME_MAX} characters` };
  }
  if (FORBIDDEN_DISPLAY_NAME.test(name)) {
    return { ok: false, code: "invalid_chars", message: "Display name contains invalid characters" };
  }
...
export async function updateArcadeDisplayName(supabase, studentId, displayName) {
  ...
  if (!studentRow?.id) {
    return { ok: false, code: "student_not_found", message: "Player not found", status: 404 };
  }
  ...
  if (current?.display_name_updated_at) {
    ...
    if (Number.isFinite(last) && Date.now() - last < DISPLAY_NAME_COOLDOWN_MS) {
      return {
        ok: false,
        code: "rate_limited",
        message: gamePackCopy("lib__arcade__club__player-profile.server", "you_can_change_your_display_name_once_every_24_hours"),
        status: 429,
      };
    }
  }
  ...
  if (error) {
    if (error.code === "42P01") {
      return { ok: false, code: "profile_unavailable", message: "Profile unavailable", status: 503 };
    }
