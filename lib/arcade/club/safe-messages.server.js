...
const DEFAULT_SAFE_MESSAGES = [
  { text: "Nice one!", emoji: "👏", category: "praise", min_permission_level: "guest" },
  { text: "Play again?", emoji: "🔄", category: "invite", min_permission_level: "guest" },
  { text: "Fun playing with you!", emoji: "😊", category: "fun", min_permission_level: "guest" },
  { text: "You won!", emoji: "🏆", category: "praise", min_permission_level: "registered" },
  { text: "Good game!", emoji: "⭐", category: "praise", min_permission_level: "registered" },
];
...
export async function ensureDefaultSafeMessages(supabase) {
  ...
  for (const m of DEFAULT_SAFE_MESSAGES) {
    const { text, ...rest } = m;
    await supabase.from("arcade_safe_messages").insert({ ...rest, text_he: "", active: true });
  }
}
...
export async function listSafeMessagesForStudent(supabase, studentId) {
  ...
  const messages = (data || [])
    ...
    .map((m) => ({
      id: m.id,
      text: (DEFAULT_SAFE_MESSAGES.find((d) => d.emoji === m.emoji && d.category === m.category) || {}).text || "",
      emoji: m.emoji,
      category: m.category,
    }));
...
export async function sendSafeMessageToRoom(supabase, studentId, roomId, messageId) {
  ...
  if (!msg?.id) return { ok: false, code: "message_not_found", message: "Message not found" };
  ...
  if (!memberRow?.id) {
    return {
      ...
      message: gamePackCopy("lib__arcade__club__safe-messages.server", "cannot_send_a_message_in_this_room"),
    };
  }
  ...
  if (!session?.id) return { ok: false, code: "no_session", message: "No active game" };

  const state = typeof session.state === "object" && session.state ? { ...session.state } : {};
  const emotes = Array.isArray(state.emotes) ? [...state.emotes] : [];
  emotes.push({
    studentId,
    text: (DEFAULT_SAFE_MESSAGES.find((d) => d.emoji === msg.emoji && d.category === msg.category) || {}).text || "",
    emoji: msg.emoji,
