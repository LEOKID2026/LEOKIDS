...
import reportsBurnDownIndexEsEs from "../../content-packs/es-ES/reports/burn-down-index.json" with { type: "json" };
 ...
 * @param {Record<string, { copy?: Record<string, string> } | Record<string, string>>} leaves
 * @returns {Readonly<Record<string, Record<string, string>>>}
 */
function composeBurnDownIndexOverlay(leaves) {
  /** @type {Record<string, Record<string, string>>} */
  const out = {};
  for (const [slug, pack] of Object.entries(leaves)) {
    const copy =
      pack && typeof pack === "object" && "copy" in pack && pack.copy && typeof pack.copy === "object"
        ? /** @type {Record<string, string>} */ (pack.copy)
        : /** @type {Record<string, string>} */ (pack || {});
    out[slug] = copy;
  }
