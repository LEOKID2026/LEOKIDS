 ...
 * Default: GLOBAL_DATA_WRITES_ENABLED=true — writes enabled for deployed global product.
 */
...
export function isGlobalDataWritesEnabled() {
  const raw = String(process.env.GLOBAL_DATA_WRITES_ENABLED ?? "true").trim().toLowerCase();
...
export function isGlobalMockModeEnabled() {
  const raw = String(process.env.GLOBAL_MOCK_MODE ?? "false").trim().toLowerCase();
  if (raw === "0" || raw === "false" || raw === "no" || raw === "off") return false;
