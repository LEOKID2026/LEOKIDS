...
function heuristicHeToEn(he) {
  ...
  s = s
    .replace(/present tense/gi, "in the present tense")
    .replace(/in questions/gi, "in questions")
    .replace(/ask about /gi, "Ask about ")
    .replace(/question about /gi, "Ask about ")
    .replace(/using /gi, "with ")
    .replace(/with /gi, "with ")
    .replace(/we use /gi, "we use ")
    .replace(/place description/gi, "Place description")
    .replace(/comparative/gi, "Comparative")
    .replace(/adjective/gi, "Adjective")
    .replace(/adjectives describe nouns\.?/gi, "Adjectives describe nouns.")
    .replace(/common for grade 3\.?/gi, "common for grade 3.")
    .replace(/\u200e/g, "")
    .replace(/\s+/g, " ")
    .trim();
  return s;
