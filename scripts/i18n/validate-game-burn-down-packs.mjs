/**
 * Validate game burn-down content packs (index + per-file packs).
 * Run: node scripts/i18n/validate-game-burn-down-packs.mjs
 */
...
if (index) {
  ...
  if (packFiles.length === 0) {
    errors.push("expected at least one burn-down pack file, found 0");
  }
...
if (errors.length) {
  console.error("validate-game-burn-down-packs FAILED");
  for (const e of errors) console.error(" -", e);
  process.exit(1);
}

const keyCount = index ? Object.values(index).reduce((n, p) => n + Object.keys(p).length, 0) : 0;
console.log(`validate-game-burn-down-packs OK (${Object.keys(index || {}).length} packs, ${keyCount} keys)`);
