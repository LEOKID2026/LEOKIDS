/**
 * Hybrid residue excluding README/status/infra chrome and Hebrew notes.
 */
...
// Avoid German loanwords identical in spelling: Addition, Multiplikation, Division, etc.
const EN_MARK =
  /\b(the|and|with|that|which|without|because|through|their|they|these|those|would|could|should|from|have|been|being|does|make|help|need|what|when|where|how|why|for|over|under|after|before|during|only|more|most|other|than|then|each|every|will|are|were|you|your|we|our|is|like|has|not|all|sides|corners|shape|length|width|equal|Imagine|Check|Yes|Did|Does|called|hold|move|moved|flip|mirror|everywhere|tiles|windows|games|board|sticker|classroom|Identifying|different|opposite|usually|longer|shorter|everything|three-dimensional|flat|page|hand|polygon|triangular|faces|round|base|point|top|shoe|box|rectangular|same|curved|soda|bases|Today|Number line|Look for|Try to|In practice|How many|How much|A square|A rectangle|A triangle|Example|between|names|rows|plane|about the|about a|Important|don't|stop|questions|comes|neighbor|Break|Find|step|first|Coins|Total|amount|groups|Regrouping|carrying|shekels|shekel|agorot|altogether)\b/;

function isInfra(rel, enLine) {
  if (/(^|\/)README\.md$/i.test(rel)) return true;
  if (/[\u0590-\u05FF]/.test(enLine)) return true;
  if (/registry|Not in scope|practice routing|owner-approved|approval_status|runtime|commit|push|deploy|Hebrew titles|Child-facing copy uses|signoff|style guide|spine `maxGrade`/i.test(enLine))
    return true;
  if (/^:::/.test(enLine) || /^```/.test(enLine) || /^type:\s*\w/.test(enLine)) return true;
  return false;
}
...
fs.writeFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), "_de-DE-book-hybrid-student.json"), JSON.stringify(arr, null, 2));
