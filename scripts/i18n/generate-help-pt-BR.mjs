/**
 * Generate data/help-center/pt-BR/* from English Help articles.
 * Preserves slugs, ids, hrefs, keywords structure, screenshot/video paths.
 *
 * Run: node scripts/i18n/generate-help-pt-BR.mjs
 */
...
const OUT_DIR = path.join(ROOT, "data/help-center/pt-BR");
const CACHE_PATH = path.join(__dirname, "_mt-cache-pt-BR-help.json");
...
function emitModule(exportName, value) {
  return `/** Auto-generated pt-BR Help — do not hand-edit structure/slugs. */\nexport const ${exportName} = ${JSON.stringify(value, null, 2)};\n`;
}
...
async function main() {
  ...
  const sectionsPt = await transform(SECTIONS, null, cache);
  // Restore non-translatable section keys / hrefs
  for (const key of Object.keys(SECTIONS)) {
    sectionsPt[key].key = SECTIONS[key].key;
    sectionsPt[key].href = SECTIONS[key].href;
    sectionsPt[key].emoji = SECTIONS[key].emoji;
    sectionsPt[key].hubGradientKey = SECTIONS[key].hubGradientKey;
  }
...
export const SECTIONS_PT_BR = ${JSON.stringify(sectionsPt, null, 2)};
...
`;
  ...
  console.log("Wrote Help Center pt-BR", {
