...
const SURFACE_META = {
  ...
  detailedNarrative: {
    roleHe: "    (snapshot / homeLine / caution)",
    reportTypes: ["detailedReport"],
  ...
  staticTemplateCatalog: {
    roleHe: "  owner —  live;     templateId+slots",
    reportTypes: ["templateCatalog"],
    contractSource: "parent-report-owner-copy-templates.js",
  },
  gradeAwareTemplate: {
    roleHe: " grade-aware —  live;  action/goal  taxonomy+grade band",
    reportTypes: ["templateCatalog"],
...
function buildMarkdown(report) {
  ...
  const lines = [
    ...
    "",
    "- ** ** (`parentReportScreen`) —  `pages/learning/parent-report.js`: diagnosticOverview, insights, subjectSummary, topicExplain",
    "- ** ** (`shortReport`) — PDF/ : subjectSummary, topicInsightLine, topicExplain",
    "- ** ** (`detailedReport`) — subjectRollup, parentLetter, homeAction, recommendationCard, detailedNarrative, executiveSummary, crossSubjectInsights, homePlan",
    "- ** ** (`templateCatalog`) — static fixtures + grade-aware templates ( live)",
    "",
