#!/usr/bin/env node
/**
 * Moledet / geography metadata review pack.
 * npm run qa:question-metadata:geography-review-pack
 */
...
const ENRICHMENT_JSON = join(OUT_DIR, "geography-enrichment-suggestions.json");
const OUT_JSON = join(OUT_DIR, "geography-review-pack.json");
const OUT_MD = join(OUT_DIR, "geography-review-pack.md");
...
async function main() {
  ...
  const packMod = await import(
    new URL("../utils/question-metadata-qa/geography-enrichment-review-pack.js", import.meta.url).href
  );
  const { buildMoledetGeographyReviewPack } = packMod;
  ...
  const { generateMoledetGeographySuggestions } = sugMod;
  ...
  if (existsSync(ENRICHMENT_JSON)) {
    ...
    enrichmentPayload = {
      ...
      scope: { rowCountByFile, geographyRowTotal },
      summary: {
  ...
  const loadErrors = [];
  const pack = buildMoledetGeographyReviewPack(enrichmentPayload, { loadErrors });
  await writeFile(OUT_JSON, JSON.stringify(pack, null, 2), "utf8");

  const s = pack.summary;
  const md = [
    "# Moledet / geography — review pack",
    "",
    ...
    "## Outputs",
    "",
    `- \`reports/question-metadata-qa/geography-review-pack.json\``,
    "",
  ].join("\n");
  ...
  console.log("");
  console.log("═══════════════════════════════════════════════════════════════");
  console.log("  Moledet/geography review pack — rows:", s.geographyRowTotal);
