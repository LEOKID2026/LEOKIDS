/**
 * Science inventory/runtime rows vs conservative curriculum map.
 */
...
function renderMd(p, reg) {
  const lines = [];
  lines.push("# Science runtime / inventory vs official source");
  lines.push("");
  lines.push(`Generated: ${p.generatedAt}`);
  lines.push("");
  lines.push("## Registry (Science)");
  lines.push("```json");
  lines.push(JSON.stringify(reg, null, 2));
  lines.push("```");
  lines.push("");
  lines.push("## Summary");
  lines.push(`- Inventory Science rows: **${p.meta.inventoryScienceRows}**`);
...
async function main() {
  ...
  const base = buildScienceRuntimeVsOfficialSummary(inventory);
  const reg = summarizeScienceRegistry(scienceRegistryRows());
  const payload = {
    ...base,
    registrySummary: reg,
    meta: {
  ...
  await writeFile(join(OUT_DIR, "science-runtime-vs-official-source.json"), JSON.stringify(payload, null, 2), "utf8");
  await writeFile(join(OUT_DIR, "science-runtime-vs-official-source.md"), renderMd(payload, reg), "utf8");
  console.log("Wrote science-runtime-vs-official-source.{json,md}");
}
