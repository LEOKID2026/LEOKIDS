/**
 * Download MoE kita1–kita6.pdf and extract a section catalog (per-page text + derived phrases).
 * Output: reports/curriculum-audit/math-pdf-section-catalog.{json,md}
 */
...
async function main() {
  ...
  for (let grade = 1; grade <= 6; grade++) {
    ...
    byGrade.push({
      ...
      extractionNotes: [
        ...
        "התאמת קטלוג ידני לחלקים ספציפיים דורשת השוואה חזותית ל-PDF.",
      ],
  ...
  const payload = {
    ...
    phase: "math-pdf-section-catalog",
    meta: {
  ...
  await writeFile(join(OUT_DIR, "math-pdf-section-catalog.json"), JSON.stringify(payload, null, 2), "utf8");

  const md = renderMarkdown(payload);
  await writeFile(join(OUT_DIR, "math-pdf-section-catalog.md"), md, "utf8");
  console.log(`Wrote ${join(OUT_DIR, "math-pdf-section-catalog.json")}`);
  console.log(`Wrote ${join(OUT_DIR, "math-pdf-section-catalog.md")}`);
  if (!allOk) {
...
function renderMarkdown(p) {
  const lines = [];
  lines.push("# Math PDF section catalog (MoE kita1–kita6)");
  lines.push("");
