...
add({ category: "evidence_gate", state: "no_data", file: EQ, cond: "classifyDataSufficiency: n===0 → NO_DATA", in: "summary.totalAnswers, diagnosticAnswers", out: "meta.evidenceQuality.dataSufficiency", tpl: "(suppresses strong diagnosis)", ex: "   —    ", pol: "neutral", short: DISPLAY.no, detailed: DISPLAY.partial, hidden: "gates downstream copy", spec: "specific", dup: "activity_gap_zero", conflict: "server weak-subject insight", notes: "Distinguish zero activity vs zero diagnostic" });
add({ category: "evidence_gate", state: "insufficient_data", file: EQ, cond: "n<=INSUFFICIENT_MAX_DIAGNOSTIC → INSUFFICIENT", in: "diagnosticAnswers count", out: "dataSufficiency", tpl: "(hedged only)", ex: "     —      .", pol: "neutral", short: DISPLAY.insights, detailed: DISPLAY.yes, hidden: "patternDiagnostics suppressed", dup: "server thin insight", conflict: "strong topic line", notes: "Student-level hedge" });
add({ category: "evidence_gate", state: "preliminary_signal", file: EQ, cond: "n<SUPPORTED_MIN && !recurrenceMet → PRELIMINARY", in: "diagnosticAnswers, recurrenceMet", out: "dataSufficiency", tpl: "(tentative conclusions)", ex: "{subject}      —      .", pol: "neutral", short: DISPLAY.insights, detailed: DISPLAY.yes, hidden: "strong diagnosis blocked", dup: "insufficient_data", conflict: "supported_diagnosis same topic", notes: "Use tentative wording" });
add({ category: "evidence_gate", state: "supported_diagnosis", file: EQ, cond: "n>=SUPPORTED_MIN && recurrenceMet → SUPPORTED", in: "diagnosticAnswers, recurrenceMet", out: "allowsStrongParentDiagnosisAtStudent=true", tpl: "   {subject}...", ex: "   ,    .", pol: "negative", short: DISPLAY.insights, detailed: DISPLAY.yes, spec: "specific", dup: "engine topic insight", conflict: "preliminary same subject", notes: "Server path; may be overridden by engine" });
add({ category: "evidence_gate", state: "no_recurrence", file: EQ, cond: "reasonCode: PRELIMINARY && !recurrenceMet → no_recurrence", in: "recurrenceMet", out: "evidenceReasonCode", tpl: "(blocks strong pattern claims)", ex: "        ", pol: "neutral", short: DISPLAY.explain, detailed: DISPLAY.yes, hidden: "fake recurring mistake blocked", dup: "insufficient_mistake_evidence", conflict: "recurring mistakes insight", notes: "Critical honesty gate" });
add({ category: "evidence_gate", state: "too_few_questions", file: EQ, cond: "reasonCode: INSUFFICIENT → too_few_questions", in: "diagnosticAnswers", out: "evidenceReasonCode", tpl: "(topic insights withheld)", ex: "       ", pol: "neutral", short: DISPLAY.partial, detailed: DISPLAY.yes, hidden: "confidence badge low", dup: "min_questions_low_confidence", conflict: "knowledge_gap strong", notes: "" });
add({ category: "evidence_gate", state: "allowsStrongParentDiagnosisAtTopic", file: EQ, cond: "allowsStrongParentDiagnosisAtTopic: suff===SUPPORTED && topic gates", in: "payload, subject, topicKey", out: "topic copy authority", tpl: "(enables specific topic lines)", ex: "   {topic} —    .", pol: "negative", short: DISPLAY.insights, home: DISPLAY.yes, detailed: DISPLAY.yes, dup: "engine rowNeedsAttention", conflict: "insufficient_dependency_evidence", notes: "" });
...
const legacyRules = [
  ["min_questions_low_confidence", "q < cfg.minQuestionsLowConfidence && q>0", "maintain_and_strengthen", "  {q}  «{name}» —       .", "negative"],
  ...
  ["evidence_sufficiency_cap", "suppressAggressiveStep post-cap", "maintain/remediate", "   —       ", "neutral"],
];
...
const phase2Guards = [
  ["hint_dependence_block_advance", "hintDependenceRisk && isAdvance", "maintain_and_strengthen", "     —   / ically.", "neutral"],
  ["fragile_success_block_advance", "fragile_success && (!progressSupportsAdvance||hint||!sufficiencyStrong)", "maintain_and_strengthen", "      —  ...", "neutral"],
  ["stable_mastery_guard_advance", "stable_mastery && (insufficientEvidenceRisk||falsePromotionRisk||unclearTrend)", "maintain_and_strengthen", "   —     ...", "positive"],
  ["false_promotion_guard", "falsePromotionRisk && aggressive step", "maintain_and_strengthen", "      —   .", "neutral"],
  ["unclear_trend_cap_aggressive", "unclearTrend && aggressive && !strongPerformanceNoTrend", "maintain_and_strengthen", "      ...", "neutral"],
  ["accuracy_up_independence_down", "fragileProgressPattern && advance", "maintain_and_strengthen", " ,      ...", "neutral"],
  ["speed_only_block_drop", "speedOnlyRisk && drop step", "maintain_and_strengthen", "      —   ...", "neutral"],
  ["instruction_friction_soften_drop", "instruction_friction && drop && !strongKnowledgeGap", "remediate_same_level", "    ...", "neutral"],
  ["careless_pattern_before_drop", "careless_pattern && drop", "remediate_same_level", "  —     .", "negative"],
  ["knowledge_gap_respect_positive_trend", "knowledge_gap && drop && positiveAccuracy", "remediate_same_level", "  ,   ...", "neutral"],
  ["false_remediation_guard", "falseRemediationRisk && drop", "remediate_same_level", "     —   .", "neutral"],
  ["recent_transition_caution", "negativeTrendAfterRecentDifficultyIncrease && drop_grade", "remediate_same_level", "    ...", "neutral"],
  ["fluency_positive_speed_context", "fluencySupportWithoutAccuracyDrop && speedOnly && drop_level", "maintain_and_strengthen", "     ...", "positive"],
  ["insufficient_evidence_cap_phase2", "insufficientEvidenceRisk && aggressive", "maintain_and_strengthen", "     .", "neutral"],
  ["risk_profile_prefer_remediate_over_maintain", "fragileLike && maintain && q>=10 && wrongRR>=0.15 && acc<88", "remediate_same_level", "  / —   ...", "negative"],
...
// ─── 6. Phase7 restraint ────────────────────────────────────────────────────
const phase7 = [
  ["phase7_restraint_cap_advance", "shouldAvoidStrongConclusion && advance step", "maintain_and_strengthen", "   —       ."],
  ["phase7_restraint_soften_drop", "shouldAvoid && drop step", "remediate_same_level", "       —  ."],
  ["phase7_insufficient_evidence_drop", "rootCause===insufficient_evidence && drop", "remediate_same_level", "    —   ;   ."],
  ["phase7_early_stage_soften_drop", "rootCause===early_stage_instability && drop && conclusionStrength!=='strong'", "remediate_same_level", "    —    ."],
  ["phase7_soft_copy_withheld", "mergePhase7SoftHebrewCopy: cs==='withheld'||level==='insufficient'", "parentHe append", "     —      ."],
  ["phase7_soft_copy_tentative", "cs==='tentative'||shouldAvoidStrongConclusion", "parentHe append", " :   ,  ."],
];
...
const rootCauses = [
  ["insufficient_evidence", "level==='insufficient'||conclusionStrength==='withheld'", "         ", "neutral"],
  ["knowledge_gap", "behaviorType knowledge_gap + strong evidence OR default", "       ", "negative"],
  ["speed_pressure", "speedOnlyRisk OR speed_pressure behavior", "        ", "negative"],
  ["instruction_friction", "instruction_friction || hintDependenceRisk", "     ", "negative"],
  ["careless_execution", "careless_pattern OR acc>=68 wr band", "      ", "negative"],
  ["weak_independence", "fragile_success+trend OR independenceDeteriorating", "     ", "negative"],
  ["early_stage_instability", "q<12 || conclusionStrength==='tentative'", "   —   ", "neutral"],
  ["mixed_signal", "restraint.level==='mixed' OR default", "    ", "neutral"],
];
...
const behaviors = [
  ["knowledge_gap", "wrongRatio>=0.32 OR maxFamCount>=3 OR acc<62", " ", "negative"],
  ["speed_pressure", "modeKey speed OR medWrongMs<2200", " ", "negative"],
  ["instruction_friction", "hintRate>=0.42", "       ", "negative"],
  ["careless_pattern", "acc 55-85 + fast wrong responses", "    ", "negative"],
  ["fragile_success", "acc 72-92 + firstTryMiss/avgRetry/changedRate", " ", "negative"],
  ["stable_mastery", "q>=8 acc>=88 wrongRatio<=0.14", "    ", "positive"],
  ["undetermined", "best score <1.15 && q<4", " ", "neutral"],
  ["mixed_low_signal", "(UI label only)", "   —    ", "neutral"],
];
...
const whyMetrics = [
  ["firstTryMissRate", "fragile_success && firstTryMiss>=0.4", "    -{pct}%  ", "negative"],
  ["changedRate", "fragile_success && changedRate>=0.3", "     ({pct}%)", "negative"],
  ["avgRetry", "fragile_success && avgRetry>=1.15", " {avgRetry}   ", "negative"],
  ["fragile_success_default", "fragile_success else branch", "         ", "negative"],
  ["mid_accuracy_60_74", "acc in remediate band (legacy+row)", "  —   ", "negative"],
  ["low_accuracy", "acc<55 or acc<62 knowledge_gap", "  — ", "negative"],
  ["high_accuracy_85_plus_errors", "acc>=85 && wrongs recurring", "     — fragile/careless", "neutral"],
  ["maintained_accuracy", "stable_mastery in buildWhy", "     ( {acc}%  {q} )", "positive"],
];
...
const deps = [
  ["insufficient_dependency_evidence", "weak||gateReadiness==='insufficient'", "         ", "neutral"],
  ["likely_local_issue", "speedOnly&&!fragile OR localScore>=2", "   ", "neutral"],
  ["likely_foundational_block", "foundationScore>=3 OR (>=2 && fragile/persistent)", "    —     ", "negative"],
  ["mixed_dependency_signal", "foundationScore&&localScore mixed", "      ", "neutral"],
  ["accuracy_foundation_gap", "knowledgeGap+persistentMistakes blocker", "   ", "negative"],
];
for (const [id, cond, tpl, pol] of deps) {
  add({
    ...
    pdfShort: DISPLAY.yes,
    detailed: DISPLAY.yes,
    hidden: "vague ' ' filtered by VAGUE_FOUNDATION_PHRASE",
    spec: "specific",
...
const patterns = [
  ["insufficient_mistake_evidence", "mC===0&&q<10 OR weakSignal", "        ", "neutral"],
  ["speed_driven_error", "speed_pressure root/behavior", "  ", "negative"],
  ["instruction_misread", "instruction_friction/hintDependence", "    ", "negative"],
  ["support_dependent_success", "weak_independence/independenceDeteriorating", "    ", "negative"],
  ["careless_flip", "careless_execution/careless_pattern", "      ", "negative"],
  ["concept_confusion", "knowledge_gap strong + wr>0.38", "  ", "negative"],
  ["procedure_break", "knowledge_gap strong + wr<=0.38", "  ", "negative"],
  ["mixed_mistake_pattern", "fragileProgress OR default", "  —    ", "neutral"],
  ["early_learning_noise", "q<12 unclear trend", "    ", "neutral"],
];
...
const steps = [
  ["advance_level", "   —   ", "positive"],
  ["advance_grade_topic_only", "  —   ", "positive"],
  ["maintain_and_strengthen", "  ", "neutral"],
  ["remediate_same_level", "  ", "negative"],
  ["drop_one_level_topic_only", "   —   ", "negative"],
  ["drop_one_grade_topic_only", "   —   ", "negative"],
];
...
// ─── 13. Risk flags ─────────────────────────────────────────────────────────
const risks = [
  ["falsePromotionRisk", "    ", "neutral"],
  ["falseRemediationRisk", "  ", "neutral"],
  ["speedOnlyRisk", " ", "negative"],
  ["hintDependenceRisk", "   ", "negative"],
  ["insufficientEvidenceRisk", "  ", "neutral"],
  ["recentTransitionRisk", "  ", "neutral"],
];
for (const [id, label, pol] of risks) {
  add({
    ...
    out: "riskFlags; appended to whyThisRecommendationHe as '  '",
    tpl: label,
    ex: `  : ${label}.`,
    pol,
...
const interventions = [
  ["insufficient_evidence", "collect_evidence", "  :  ,  ,     .", "neutral"],
  ["speed_pressure", "accuracy_over_speed", "    ,    .", "neutral"],
  ["instruction_friction", "clarity_first", " ,  ,     .", "negative"],
  ["weak_independence", "fade_support", "   « »  «  ».", "negative"],
  ["knowledge_gap", "core_skill_gap", "       .", "negative"],
  ["careless_execution", "execution_habits", "       .", "neutral"],
  ["mixed_signal", "observe_and_stabilize", "        .", "neutral"],
...
add({ category: "overview_strength", state: "strongestAreaLineHe", file: V2, cond: "strengthCandidates[0]: positiveAuthorityLevel excellent/very_good/good OR actionState maintain/expand", in: "diagnosticEngineV2.units canonicalState", out: "summary.diagnosticOverviewHe.strongestAreaLineHe", tpl: "{subject}: {name}: {q} ,  {acc}%", ex: ": : 45 ,  92%", pol: "positive", short: DISPLAY.overview, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "rawMetricStrengthsHe", conflict: "requiresAttentionPreview same subject", notes: "" });
add({ category: "overview_strength", state: "readyForProgressPreviewHe", file: V2, cond: "strengthCandidates[1..] up to 3 lines", in: "V2 units sorted by strength", out: "diagnosticOverviewHe.readyForProgressPreviewHe[]", tpl: "{subject}: {name}: {q} ,  {acc}%", ex: ": : 30 ,  88%", pol: "positive", short: DISPLAY.overview, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "advance_level step", conflict: "falsePromotionRisk on same topic", notes: "Preview only — not same as approved advance" });
add({ category: "overview_weakness", state: "mainFocusAreaLineHe", file: V2, cond: "attentionSorted[0] priority P1-P4 OR diagnosis.allowed", in: "V2 units priority", out: "diagnosticOverviewHe.mainFocusAreaLineHe", tpl: "{subject}: {name}: {pattern OR q/acc}", ex: ": :   ", pol: "negative", short: DISPLAY.overview, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, hidden: "appendThinOverviewHedge when thin", spec: "specific", dup: "engine topic_attention_insight", conflict: "strongestAreaLineHe same subject AAA5", notes: "" });
add({ category: "overview_weakness", state: "requiresAttentionPreviewHe", file: V2, cond: "attentionSorted[1..] up to 5", in: "V2 attention units", out: "diagnosticOverviewHe.requiresAttentionPreviewHe[]", tpl: "(same as mainFocus format)", ex: ": : -12 ,  58%", pol: "negative", short: DISPLAY.overview, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "insights topic lines", conflict: "mainFocusAreaLineHe", notes: "" });
add({ category: "overview_neutral", state: "insufficientDataSubjectsHe", file: V2, cond: "subjects with thin evidence list", in: "insufficientDataSubjectsHe", out: "diagnosticOverviewHe.insufficientDataSubjectsHe[]", tpl: "(subject names)", ex: "", pol: "neutral", short: DISPLAY.overview, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "thin_student_data", conflict: "", notes: "" });
...
const explainSections = [
  ["explain_identified", "buildTopicDiagnosticExplainSectionsHe identified", " : {stepLabel} «{label}».", "topic table + PDF"],
  ["explain_data", "q/acc/wr", ": {q} ,  {acc}%, {wr}% .", "topic table + PDF"],
  ["explain_pattern_known", "parentFacingPatternLineHe present", ": {patternLabel/narrative}.", "topic table + PDF"],
  ["explain_pattern_unknown", "rowNeedsAttention && !pattern", ":         ...", "topic table + PDF"],
  ["explain_meaning", "parentFacingMeaningLineHe + foundation", ": {step/reason}. {foundation?}", "topic table + PDF"],
  ["explain_action", "doNowHe||interventionPlanHe||recommendedParentActionHe", " : {action}", "topic table + PDF"],
  ...
  ["trendCompactLineHe", "trend.summaryHe OR accuracy/independence directions", "   : {dir}", "topic table truncated 100"],
  ["learningMemoryLineHe", "memoryNarrativeHe||learningStageLabelHe", " : ...", "detailed partial truncate 150"],
  ["reviewBeforeAdvanceLineHe", "reviewBeforeAdvanceHe", "   ...", "detailed truncate 160"],
  ["transferReadinessLineHe", "transferReadiness not unknown", ": ...", "detailed truncate 130"],
];
for (const [id, cond, tpl, where] of explainSections) {
  add({
    ...
    tpl,
    ex: tpl.replace("{q}", "24").replace("{acc}", "68").replace("{label}", ""),
...
const trends = [
  ["row_trend_up", "accuracyDirection==='up' OR progressSupportsAdvance", " /    ", "positive"],
  ["row_trend_down", "accuracyDirection==='down'", "", "negative"],
  ["row_trend_flat", "accuracyDirection==='flat'", " ", "neutral"],
  ["row_trend_unknown", "unclearTrend", "   /  ", "neutral"],
  ["fragileProgressPattern", "accuracy up + independence down", " ,      ", "neutral"],
  ["progressSupportsAdvance", "trend supports advance", "    ,    ", "positive"],
...
const homeRecs = [
  ["home_math_geometry", "weakest subject math|geometry", " 10     ...", "neutral"],
  ["home_hebrew", "weakest hebrew", "     ...", "neutral"],
  ["home_english", "weakest english", " 5–10    ...", "neutral"],
  ["home_generic_weak", "other weak subject", "     {label}  ", "neutral"],
  ["home_word_problems", "weakTopic word problems", "    ...", "negative"],
  ["home_review_mistakes", "totalAnswers>0", "    ...", "neutral"],
  ["home_start_short", "totalAnswers===0", "    5–10 ...", "neutral"],
  ["home_routine", "always (fallback)", "     ...", "neutral"],
  ["home_hebrew_reading", "insights mention ", "   ...", "neutral"],
];
...
add({ category: "strength_detection", state: "stable_mastery_why", file: P2, cond: "buildWhy behaviorType===stable_mastery", in: "acc, q", out: "whyThisRecommendationHe", tpl: "     ( {acc}%  {q} ).", ex: "     ( 91%  32 ).", pol: "positive", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "maintained_accuracy", conflict: "mistake pattern on same row", notes: "Good strength template to reuse in insights" });
add({ category: "strength_detection", state: "advance_level_positive", file: TNE, cond: "final step advance_level after guards pass", in: "high volume strong performance", out: "recommendedStepLabelHe + why", tpl: "   —   ", ex: ":    —  ...", pol: "positive", short: DISPLAY.explain, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "readyForProgressPreviewHe", conflict: "falsePromotionRisk", notes: "Only when guards allow" });
add({ category: "strength_detection", state: "advance_grade_topic_only", file: TNE, cond: "advance_grade_topic legacy rule + guards", in: "hard level high acc stability", out: "step label", tpl: "  —   ", ex: "(same)", pol: "positive", short: DISPLAY.explain, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "readyForProgressPreviewHe", conflict: "phase7_restraint_cap_advance", notes: "" });
add({ category: "strength_detection", state: "strength_not_stable_enough", file: P2, cond: "fragile_success_block_advance OR stable_mastery_guard_advance", in: "good acc but hint/unclearTrend/insufficientEvidence", out: "step→maintain", tpl: "      —  ", ex: "      —  ...", pol: "neutral", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "fragile_success", conflict: "advance_level + high acc", notes: "Critical parent message" });
...
add({ category: "why_recommendation", state: "knowledge_gap_why", file: P2, cond: "buildWhy behaviorType===knowledge_gap", in: "acc,q,wr,pattern,dependency", out: "whyThisRecommendationHe", tpl: "    ... + pattern/foundation honesty", ex: "       ( 38%, 136 , 62% ).", pol: "negative", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "explain meaning", conflict: "likely_local_issue dependency", notes: "" });
add({ category: "why_recommendation", state: "undetermined_why", file: P2, cond: "behaviorType undetermined|insufficient_evidence", in: "q", out: "whyThisRecommendationHe", tpl: "       ...", ex: "          ( 4  ).", pol: "neutral", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "no_pattern_honest", conflict: "", notes: "" });
add({ category: "why_recommendation", state: "default_behavior_why", file: P2, cond: "else branch", in: "behaviorType label", out: "whyThisRecommendationHe", tpl: "  : {diagnosticTypeLabelHe}", ex: "  :   ( 64%  18 ).", pol: "negative", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "behavior profile label", conflict: "", notes: "" });
add({ category: "why_recommendation", state: "legacy_rule_mention", file: P2, cond: "legacyRuleId truthy", in: "legacyRuleId", out: "why append", tpl: "   .", ex: "   .", pol: "neutral", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "generic", dup: "phase2 blocker", conflict: "", notes: "" });

// ─── 23. Learning stage labels (Phase 9) ────────────────────────────────────
const stages = [
  ["early_acquisition", "   —   ", "neutral"],
  ["partial_stabilization", " ,   ", "neutral"],
  ["stable_control", "    ", "positive"],
  ["fragile_retention", "  ", "negative"],
  ["regression_signal", "   —   ", "negative"],
  ["transfer_emerging", "     ", "positive"],
  ["insufficient_longitudinal_evidence", "    ", "neutral"],
];
...
add({ category: "v2_tier_label", state: "tierStableStrengthHe", file: V2C, cond: "positiveAuthorityLevel excellent/very_good/good OR maintain action", in: "diagnosticEngineV2 unit canonicalState", out: "diagnostic card tierHe / detailed summaryHe", tpl: " ", ex: " :  ", pol: "positive", short: DISPLAY.partial, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "stable_mastery_why", conflict: "tierWeakness same topic", notes: "Badge without q/acc unless caller adds" });
add({ category: "v2_tier_label", state: "tierWeaknessRecurringHe", file: V2C, cond: "weakness unit && wrongCountForRules>=5", in: "unit.recurrence.wrongCountForRules", out: "diagnostic card tierHe", tpl: "   ", ex: " :    ", pol: "negative", short: DISPLAY.partial, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "recurring_mistakes_subject", conflict: "no_recurrence gate", notes: "Needs wrong count evidence" });
add({ category: "v2_tier_label", state: "tierWeaknessSupportHe", file: V2C, cond: "weakness unit && wrongCount<5", in: "V2 weakness unit", out: "diagnostic card tierHe", tpl: " ", ex: " :  ", pol: "negative", short: DISPLAY.partial, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "generic", dup: "remediate_same_level", conflict: "", notes: "" });
add({ category: "v2_tier_label", state: "v2ShortOverviewCannotConcludeHe", file: V2C, cond: "unitRequiresShortThinOverviewHedge(unit)", in: "thin V2 unit evidence", out: "appended to mainFocusAreaLineHe", tpl: "(hedge phrase from v2ShortOverviewCannotConcludeHe)", ex: "—       ", pol: "neutral", short: DISPLAY.overview, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "generic", dup: "preliminary_signal", conflict: "mainFocusAreaLineHe assertive", notes: "" });
add({ category: "v2_tier_label", state: "v2SubjectDiagnosticRestraintHe", file: V2C, cond: "subject-level restraint copy", in: "diagnostic restraint", out: "subject diagnostic snippet", tpl: "       ...", ex: "(same)", pol: "neutral", short: DISPLAY.partial, detailed: DISPLAY.yes, spec: "generic", dup: "phase7_soft_copy_tentative", conflict: "", notes: "" });

// ─── 25. whatCouldChangeThisHe ──────────────────────────────────────────────
add({ category: "topic_meta_copy", state: "whatCouldChangeThisHe", file: P2, cond: "buildWhatCouldChangeThisHe always after why", in: "q, behaviorType", out: "topicEngineRowSignals.whatCouldChangeThisHe", tpl: "   {max(q,12)} ...    ...   ...", ex: "   24   ,    ...", pol: "neutral", short: DISPLAY.no, detailed: DISPLAY.yes, pdfDetailed: DISPLAY.yes, hidden: "often collapsed in short UI", spec: "specific", dup: "insufficient_evidence narrative", conflict: "", notes: "" });

// ─── 26. Hint dependence metric ─────────────────────────────────────────────
add({ category: "behavior_metric", state: "hint_dependence", file: BEH, cond: "hintRate>=0.42 && hintKnown>=3 → instruction_friction; risk flag hintDependenceRisk", in: "mistake events hintUsed", out: "behavior scores + riskFlags.hintDependenceRisk", tpl: "    /   {pct}%", ex: "  :    .", pol: "negative", short: DISPLAY.why, pdfShort: DISPLAY.yes, detailed: DISPLAY.yes, spec: "specific", dup: "instruction_friction", conflict: "advance_level blocked", notes: "User-requested explicit mapping" });
add({ category: "behavior_metric", state: "speed_pressure_metric", file: BEH, cond: "modeKey speed OR medWrongMs<2200", in: "responseMsWrong, modeKey", out: "diagnosticType speed_pressure", tpl: " ", ex: "  :  ", pol: "negative", short: DISPLAY.explain, detailed: DISPLAY.yes, spec: "specific", dup: "speed_only_block_drop", conflict: "", notes: "" });
add({ category: "behavior_metric", state: "careless_execution_metric", file: RC, cond: "rootCause careless_execution", in: "acc 68-92, wr band", out: "rootCauseLabelHe", tpl: "    ", ex: "(same)", pol: "negative", short: DISPLAY.explain, detailed: DISPLAY.yes, spec: "specific", dup: "careless_pattern", conflict: "stable_mastery", notes: "" });

// ─── 27. Detailed report only ───────────────────────────────────────────────
const DET = "utils/detailed-parent-report.js";
add({ category: "detailed_report", state: "detailed_summary_strength", file: DET, cond: "strongPosD && leadPosD → tierStableStrengthHe in summaryHe", in: "rankedPositiveD[0]", out: "subject section summaryHe", tpl: " {name}:  ", ex: " :  ", pol: "positive", short: DISPLAY.no, detailed: DISPLAY.yes, pdfDetailed: DISPLAY.yes, spec: "specific", dup: "strongestAreaLineHe", conflict: "weak topic same subject in short", notes: "" });
...
const topicFields = [
  ...
  ["topic_row_questions", "row.questions", "{q} ", "neutral"],
  ...
  ["topic_row_confidence_badge", "confidenceBadgeLabelHe", "   / ...", "neutral"],
  ["topic_row_sufficiency_badge", "sufficiencyBadgeLabelHe", " : //", "neutral"],
];
