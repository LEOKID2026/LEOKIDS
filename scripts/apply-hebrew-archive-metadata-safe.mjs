#!/usr/bin/env node
/**
 * Apply safe Hebrew archive metadata from enrichment JSON (g1–g6).
 * npm run qa:hebrew-archive-metadata:apply-safe
 */
...
const suggestionsPath = join(ROOT, "reports", "question-metadata-qa", "hebrew-archive-enrichment-suggestions.json");
const reportDir = join(ROOT, "reports", "question-metadata-qa");
const reportJson = join(reportDir, "hebrew-archive-metadata-apply-report.json");
const reportMd = join(reportDir, "hebrew-archive-metadata-apply-report.md");
...
/**
 * @param {Record<string, unknown>} modPlain
 * @param {string} objectPath
 */
function getRowAtArchivePath(modPlain, objectPath) {
...
/**
 * Suffix after `G*_HARD_QUESTIONS = { ... };` — may include helper functions (g6.js).
 * @param {string} fullText
 * @param {string} gradeDigit
 */
...
/**
 * Banner = file header / comments before first EASY export.
 * @param {string} fullText
 * @param {string} relPath
 */
...
/**
 * @param {string} relPath
 * @param {Record<string, unknown>} clonePlain
 */
function serializeArchiveModule(relPath, clonePlain) {
...
/**
 * @param {Record<string, Record<string, unknown>>} clonesByRel
 */
async function scanArchiveCompleteness(clonesByRel) {
...
async function main() {
  const taxMod = await import(new URL("../utils/question-metadata-qa/question-metadata-taxonomy-hebrew-archive.js", import.meta.url).href);
  const { HEBREW_ARCHIVE_SKILL_IDS, HEBREW_ARCHIVE_SUBSKILL_ALLOWLIST_BY_SKILL } = taxMod;
  ...
  const statsBefore = await scanArchiveCompleteness(clonesByRel);
  ...
  for (const s of suggestions) {
    ...
    const row = getRowAtArchivePath(clone, rel);
    ...
    if (sk && HEBREW_ARCHIVE_SKILL_IDS.has(sk)) {
      const allow = HEBREW_ARCHIVE_SUBSKILL_ALLOWLIST_BY_SKILL[sk];
    ...
    if (prereqIds.length > 0) {
      let skipReason = "hebrew_archive_policy_skip_automated_prereq";
      prerequisiteSkillIdsSkipped += 1;
      prerequisiteSkipLog.push({ objectPath: fullPath, reason: skipReason });
    }

    if (touchedThis) rowsTouched += 1;
  }

  const statsAfter = await scanArchiveCompleteness(clonesByRel);

  const META =
    "// Metadata enrichment (safe pass): skillId, subtype (grade subskill), difficulty, cognitiveLevel, expectedErrorTypes. " +
    "Inline // comments inside arrays may be normalized when this file is rewritten from structured data. " +
    "See reports/question-metadata-qa/hebrew-archive-metadata-apply-report.json.\n";
  ...
  for (const rel of REL_PATHS) {
    ...
    const body = serializeArchiveModule(rel, clonesByRel[rel]);
  ...
  const md = [
    "# Hebrew archive — metadata apply (safe pass)",
    "",
  ...
  console.log("");
  console.log("═══════════════════════════════════════════════════════════════");
  console.log("  Hebrew archive metadata apply-safe complete");
