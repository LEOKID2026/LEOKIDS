...
assert.match(
  formatParentReportActivityDisplayLabelHe({
    ...
    parentActivityTitle: "Home practice",
  }),
  /activity|Home practice/i
);
assert.equal(
  formatParentReportActivityDisplayLabelHe({
...
const g1MixedReport = {
  ...
  mathOperations: {
    "addition::grade:g1": { questions: 5, accuracy: 90, gradeRelation: "same", contentGradeKey: "g1", displayName: "Addition" },
    "multiplication::grade:g2": { questions: 20, accuracy: 88, gradeRelation: "higher", contentGradeKey: "g2", displayName: "Multiplication" },
  },
...
assert.doesNotMatch(aiText, /Grade\s*2/u, "AI insight must not mention grade 2 for g1 student");
assert.doesNotMatch(aiText, /Multiplication.*Grade/u, "AI insight must not cite higher-grade topic as strength");
for (const s of aiNarr.strengths || []) {
  assert.doesNotMatch(String(s.textHe || ""), /Multiplication/u, "higher-grade multiplication must not appear in strengths");
}
...
const mixedEngineReport = {
  ...
  mathOperations: {
    ...
    "mult_g5::grade:g5": {
      ...
      contentGradeKey: "g5",
      displayName: "Multiplication - Grade 5",
      topicEngineRowSignals: { diagnosticType: "stable_mastery", recommendedNextStep: "maintain" },
    },
    "targil_g2::grade:g2": {
      questions: 20,
...
assert.doesNotMatch(insightText, /Grade\s*2/u, "what-to-know insights must not mention grade 2");
assert.doesNotMatch(insightText, /Grade\s*5/u, "what-to-know insights must not mention grade 5");
assert.doesNotMatch(insightText, /Multiplication - Grade/u, "must not cite higher-grade multiplication label");
assert.doesNotMatch(insightText, /Practice - Grade/u, "must not cite higher-grade practice label");
assert.match(insightText, /mixed picture|mixed grade|mixed/i, "same-grade mixed insight may still appear");
...
assert.doesNotMatch(whatToNoticeText, /Grade\s*2/u, "what to notice must not mention grade 2");
assert.doesNotMatch(whatToNoticeText, /Grade\s*5/u, "what to notice must not mention grade 5");
assert.ok(
