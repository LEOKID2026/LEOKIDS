 ...
 * Cabo Verde (pt-CV) sparse Help overlays on pt-PT Help.
 */
...
export function applyHelpArticleOverlay(article, overlay) {
  ...
  if (Array.isArray(overlay.tocPatches) && Array.isArray(article.toc)) {
    out.toc = article.toc.map((entry) => {
      /** @type {Record<string, unknown>} */
      const item = { .../** @type {Record<string, unknown>} */ (entry) };
      for (const patch of overlay.tocPatches) {
        if (patch.id != null && item.id !== patch.id) continue;
        if (patch.titleIncludes != null && !String(item.title || "").includes(patch.titleIncludes)) {
          continue;
        }
